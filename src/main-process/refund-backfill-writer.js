...
//        sheet1「回填模板」：第 1 行 REFUND_TEMPLATE_HEADERS（14 列 A~N，加粗 size10）
//           其后每行按 REFUND_TEMPLATE_HEADERS.map(h => row[h] ?? '') 投影（列顺序单一真相 = 常量）
//        sheet2「未匹配报错」：第 1 行 UNMATCHED_HEADERS（含「结果类型」列区分 报错-人工介入/未匹配-提示）
//           兼容引擎产出的两类异构行（详见下）：
//             ① 银行未匹配行 { 结果类型, ...REFUND_BANK_COLUMNS 9 列, 报错/提示信息 }
//             ② refund 提示行 { 结果类型, 退款单号, 报错/提示信息 }（无银行 9 列）
//           UNMATCHED_HEADERS 同时含「退款单号」列与 9 列银行字段 → 两类行各按 key 投影，缺 key → ''
...
//   「退款单号」列承接 refund 提示行（②形状），REFUND_BANK_COLUMNS 9 列承接银行未匹配行（①形状）；
//   异构两类各按 key 投影，缺 key → ''。
...
//   入参：
//     backfillRows: Array<row>（R5 场景4 buildBackfillRow 产出，每行含 REFUND_TEMPLATE_HEADERS 14 键）
