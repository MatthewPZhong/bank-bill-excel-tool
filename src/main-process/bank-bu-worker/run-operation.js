...
const {
  assertDatasetCurrent, commitRun, getCommittedRunByOperation, getDatasetEvidence, open
...
async function executeRun(input, context = {}) {
  ...
  try {
    ...
    try {
      dataset = getDatasetEvidence(db, yearMonth);
      ...
      replay = getCommittedRunByOperation(
        db, operationIdentity, yearMonth, dataset.dataset_hash
      );
      if (!replay) result = runReconciliation(db, yearMonth);
      db.exec('COMMIT');
    } catch (error) {
    ...
    if (replay) {
      ...
      assertDatasetCurrent(db, yearMonth, dataset.dataset_hash);
      replay = getCommittedRunByOperation(
        db, operationIdentity, yearMonth, dataset.dataset_hash
      );
