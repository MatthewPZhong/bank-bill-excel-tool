...
function financeSafeIdentityDigest(identity) {
  const digest = identity && typeof identity.identityDigest === 'string' &&
    /^[a-f0-9]{64}$/.test(identity.identityDigest)
    ? identity.identityDigest
    : null;
  return digest ? `sha256:${digest.match(/.{8}/g).join(':')}` : 'unavailable';
}
...
function identityError(stage, identity, message, cause = null) {
  ...
  const causeCode = cause && typeof cause.code === 'string' && /^[A-Z0-9_]+$/.test(cause.code)
    ...
    : 'UNKNOWN';
  error.detailLines = [
    `taskRootIdentityDigest=${financeSafeIdentityDigest(identity)}`,
