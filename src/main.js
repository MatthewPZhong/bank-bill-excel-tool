...
function registerAppHandlers() {
  ...
  trackedIpcHandle('bank-statement:run', '银行对账单处理', '开始运行', () => {
    try {
      ...
      //   **不受** reconSourceMid 二选一开关控制；故无条件单独 structuredClone 注入（与 workingReconRows 并列、互不影响）。
      //   R3.5 实际 gating 在编排器：dbsChargeFundCheck 桶非空（DBS-Charge 场景 enabled）才跑；本批未 seed → 桶空 → no-op。
      //   structuredClone 防引擎原地改字段污染 DB 还原对象（与 workingReconRows / workingMidRows 同口径）。
      const workingDispatchReconRows = structuredClone(database.readFundTransferReconRows() || []);
  ...
  trackedIpcHandle('linked-table:import', '链接表管理', '导入', async () => {
    ...
    for (const filePath of res.filePaths) {
      ...
      try {
        ...
        //   交割表体量小（远小于 65 万行网关账单），数组路径全量读无 OOM 风险。
        ...
        //   （linked_fund_transfer_recon）——一行中台调拨单按收/付方向拆成 FundTransfer-in + FundTransfer-out 两行，
        //   决策 D1 固化 big_account，作为需求2（R5s2 二选一回填）/ 需求3（DBS-Charge 资金校验）匹配引擎的标准化对手方数据源。
        //   ⚠️ 与上方 ADM 派生不同：调拨对账单只依赖 mid-allocation 本身（不需 bank-deposit 数据存在），故无条件重建。
        //   函数内部保留 try/catch 隔离：派生抛错记 fundTransferReconDerive.created:false（不阻断 mid-allocation 导入本身）。
        if (repoKey === 'mid-allocation') {
