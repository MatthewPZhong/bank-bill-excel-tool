...
- 本地余额种子保存在 `文档/网银账单生成小助手/balance-seeds/`；文件按银行拆分，记录键为 `MerchantId + Currency + BillDate`。
...
- 所有用户侧报错都会生成详细报错文件，状态框可点击导出；报错文件名规则为 `YYYYMMDD-HHMMSS-模版名-错误步骤.txt`。
- 支持“新开账户生成网银账单”模块，可按开户日期和月末日期批量生成零余额账单。
- 支持另存为导出生成文件。
- 将模版和映射关系持久化到 SQLite。
- 异常按日期写入日志文件，并会在应用根目录生成 `app_activity_log.txt` 记录关键操作与报错。
...
```bash
dist/网银账单小助手-1.2.9-setup.exe
dist/网银账单小助手-1.2.9-portable.exe
```
...
- 应用运行日志：应用根目录下的 `app_activity_log.txt`
