...
- `导入文件` 支持一次选择多个原始账单文件，并将同一批次文件合并处理。
...
- 当 `MerchantId` 选择 `自己输入` 时，会直接进入“维护大账号”模式，由维护的大账号配置统一接管 `MerchantId + Currency`。
- 当维护大账号只展开出 1 条 `大账号 + 币种` 组合时，导入时会自动直通，不再弹出选择框。
- 同一模板在当前软件打开期间已导入过 2 次及以上时，点击 `导出明细` / `导出余额` 会先提示用户选择“导出当前文件”还是“导出所有”结果；该统计范围只按模板，不再按大账号 / 币种拆分。
- `导出所有明细` / `导出所有余额` 会把同一模板的多个大账号、多个币种结果整合到同一个文件里；这类文件命名不再带大账号。
...
- 混合币种账单会按币种分别计算余额，再把所有币种结果整合到同一个余额文件、同一个 sheet 中导出。
...
- 模板管理页新增 `大账号` 列和 `重命名`；`MerchantId` 选择 `自己输入` 后，可直接通过“维护大账号”维护多个“大账号 + 币种”配置，并在导入时自动使用唯一组合或选择本次使用的组合。
- 映射关系管理保存失败时，会先弹出错误提示；确认后回到原编辑内容继续修改，不会丢失当前草稿。
...
- “新开账户生成网银账单”模块支持单币种下拉选择与多币种账户模式；单币种未选择时会显示空白占位，多币种下拉支持搜索、顺序编号和点击外部收起保留结果。
- 左下角背景调色盘按钮右侧提供 `使用手册` 文本按钮，可将内置用户手册另存为 Markdown 文件。
- 所有用户侧报错都会生成详细报错文件，状态框可点击导出；报错文件名规则为 `YYYYMMDD-HHMMSS-模板名-错误步骤.txt`。
- 支持“新开账户生成网银账单”模块，可按开户日期和月末日期批量生成零余额账单。
- 1.4.6 起，新开账户余额账单改为开户日到当日逐日生成（上限 3650 天）。
- 1.4.6 新增“余额管理”弹窗，可按 `大账号 + 币种 + 日期` 维护余额附加值并在余额导出时累加注入。
- 1.4.6 新增“导入银行账号”入口，支持从 Excel 解析客资 / 自有账号到大账号表和独立 JSON 存储。
- 1.4.7 大账号选择对话框重写为左右分栏（文件顺序 + 勾选序位），并新增账号顺序固定 / 不固定模式与“记住顺序”能力。
- 1.4.7 收尾的 CSV 导入改用纯文本解析器 `parseCsvText`，长数字（20 位以上交易流水号）保持完整字符串不再被截断。
- 1.4.7 收尾把 `Currency` 字段从映射对话框的多选拼接里排除，下拉不再出现“需要拼接字段”。
- 1.4.7 收尾修复 `splitTemplateName` 多段 `-` 时所在地取第二段（`BNI-ID-SG` → `ID`，不再是 `ID-SG`）。
- 1.4.8 映射关系管理新增 `按字段区分发生额`，可通过“条件字段 + 条件值 → 目标字段”的方式覆盖单金额列 + 类型列等之前两种映射模式都无法覆盖的银行流水结构；条件值支持字面值精确匹配和 `/pattern/flags` 形式的正则匹配。
- 1.4.8 启用 `按字段区分发生额` 时，与 `Credit Amount` / `Debit Amount` 直接映射、`按正负号拆分的发生额` 形成三方互斥；切回空白时弹框配置草稿独立保留，再切回 `是` 时回显。
- 1.4.8 引入 `bundleVersion` 顶层字段（v2），导出 entry 中包含 `amountSplitRules`；导入时 `bundleVersion > 当前支持版本` 会被拒绝。
- 1.4.9 映射关系管理新增 `账单拆分合并管理` 分组，支持把一行原始账单按不同币种或金额列拆分成多笔输出行，并可按规则再合并成一条净值行；与直接映射 / 按正负号拆分 / 按字段区分发生额 形成四方互斥。
- 1.4.9 拆分账单的非金额字段可通过独立的 `拆分/合并账单映射关系设置` 弹框配置（`复用模块字段的映射关系 = 否`），并支持 `导入当前映射关系` 从主模板复制配置（自动排除 `Currency` / `Credit Amount` / `Debit Amount`）。
- 1.4.9 拆分行为按行级落库：`需要拆分成几份账单` 输入数字 N 后点 `拆` 生成 N 行，每一行的 `完成` 按钮单独锁定该行配置；删除合并组内的行时会先弹出受影响合并组列表的二次确认。
- 1.4.9 合并行的输出按 `Σ(Credit) − Σ(Debit)` 净值方向写入 `Credit Amount` 或 `Debit Amount`；单行 / 合并组净值为 0 的明细行会被静默过滤；合并组 `Currency` 不一致会报错阻断。
- 1.4.9 多文件导入时新增“以下文件全部未命中拆分/合并规则”聚合告警，与 1.4.8 的 `按字段区分发生额` 全部未命中告警平行独立。
- 1.4.9 `bundleVersion` 升级到 v3，导出 entry 新增 `billSplitMappings` / `billSplitRows` / `billSplitAmountRules` / `billSplitMeta` 四个字段；旧 v2 bundle 自动按默认值兼容。
- 支持另存为导出生成文件。
- 将模板和映射关系持久化到 SQLite。
...
```bash
dist/网银账单小助手-1.4.9-setup.exe
dist/网银账单小助手-1.4.9-portable.exe
```
