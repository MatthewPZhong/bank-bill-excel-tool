...
async function runExceljsWriterSmokeTests() {
  ...
  // F8-W2：传 unmatchedRows = [] → 2 sheet（sheet1 未命中场景、sheet2 命中场景）
  {
    ...
    // 未命中场景：v3.0.8 #9 表头上移到第1行（与 A1 提示同行）；W2 表头从 B 列起（A 列除 A1 外留空）。
    const s1 = wb.worksheets[0];
    assert.strictEqual(s1.getCell(1, 1).value, '请检查，导入前请删除该sheet', 'F8-W2 未命中场景 A1 提示');
    assert.strictEqual(s1.actualRowCount, 1, 'F8-W2 #9 空未命中 → 仅第1行(A1提示+表头)，无数据行');
    assert.strictEqual(s1.getCell(1, 2).value, 'col1', 'F8-W2 #9 第1行表头 col1（B 列）');
    assert.strictEqual(s1.getCell(1, 3).value, 'col2', 'F8-W2 #9 第1行表头 col2（右移1）');
  }
  ...
  // F8-W3：传 unmatchedRows = N 行 → 未命中场景 sheet 含 A1 + 表头 + N 行数据
  {
    ...
    // 第1行(A1提示+表头) + 2 数据行(第2-3行) = 3 行（v3.0.8 #9 表头上移一行，与 A1 同处第1行）
    assert.strictEqual(s1.actualRowCount, 3, 'F8-W3 未命中场景行数 = 第1行(A1+表头) + 2 数据');
    // v3.0.8 W2 + #9：表头在第1行 B 列起；数据从第2行起、B 列起——A 列除 A1 外留空。
    assert.ok(s1.getCell(2, 1).value === null || s1.getCell(2, 1).value === undefined, 'F8-W3 数据行 A 列留空');
    assert.ok(s1.getCell(3, 1).value === null || s1.getCell(3, 1).value === undefined, 'F8-W3 数据行 A 列留空');
    // 表头在第 1 行、从 B 列（第2列）起
    assert.strictEqual(s1.getCell(1, 2).value, 'col1', 'F8-W3 #9 表头 col1（第1行 B 列）');
    assert.strictEqual(s1.getCell(1, 3).value, 'col2', 'F8-W3 #9 表头 col2（右移1）');
    assert.strictEqual(s1.getCell(1, 4).value, 'col3', 'F8-W3 #9 表头 col3（右移1）');
    // 数据从第 2 行起、从 B 列起
    assert.strictEqual(s1.getCell(2, 2).value, 'um1-a', 'F8-W3 #9 r1.col1 = um1-a（第2行 B 列）');
    assert.strictEqual(s1.getCell(2, 4).value, 'um1-c', 'F8-W3 #9 r1.col3 = um1-c（右移1）');
    assert.strictEqual(s1.getCell(3, 2).value, 'um2-a', 'F8-W3 #9 r2.col1 = um2-a（第3行 B 列）');
    // 防泄漏：表头（第1行，B 列起 = 第2~4列）不含 _ 前缀字段（headers 投影自动过滤）
    for (let c = 2; c <= 4; c++) {
      const cellHeader = String(s1.getCell(1, c).value || '');
