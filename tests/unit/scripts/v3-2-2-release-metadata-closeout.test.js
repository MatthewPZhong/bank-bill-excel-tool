...
test('v3.2.2 closeout同步当前版本、冻结文档与三份发布说明', () => {
  ...
  assert.equal(
    read('changes/3.2.2/spec.md'),
    read('changes/background-execution-v3.2.x-contract-baseline/changes/3.2.2/spec.md')
  );
  assert.equal(
    read('changes/3.2.2/techdoc.md'),
    read('changes/background-execution-v3.2.x-contract-baseline/changes/3.2.2/techdoc.md')
  );

  const currentChangelog = section(changelog, '## 3.2.2 - 2026-08-30（版本分支技术收口，未发布）');
  const currentHistory = section(history, '## v3.2.2（2026-08-30，版本分支技术收口，未发布）');
