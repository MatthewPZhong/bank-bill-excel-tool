...
test('A-09 unarchive gate 固定按 inconsistent → active → later → allowed', () => {
  ...
  assert.equal(evaluateUnarchiveGate(legacy, { ...allBlocked, taskActive: false }).code, 'unarchive-not-tail');
...
test('A-10 active/later 只改变 gate，旧 unresolved evidence 不再阻断', () => {
  ...
  assert.deepEqual(
    ...
    ['active-vcc-task', 'active-vcc-task', '', 'unarchive-not-tail']
  );
