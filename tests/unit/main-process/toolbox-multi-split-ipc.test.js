...
test.describe('toolbox:split:export 多文件 IPC 接线', () => {
  ...
  test('成功日志记录有界日期样例、样式预计/实际和发布前校验结果', () => {
    ...
    assert.equal(
      ...
      5,
      '按行拆分、合并、普通单拆、大文件单拆、多文件拆分都应记录统一审计详情'
    );
    assert.equal(
      handlerSource.match(/输入有效行数：/g).length,
      4,
      '按行拆分、普通单拆、大文件单拆和多文件拆分都应记录输入有效行数'
    );
  });
  ...
  test('单文件字段和值校验只约束单文件模式，且全部发生在 reserve 前 prepare', () => {
    assert.match(prepareSource, /payload\.mode !== 'multiple' && payload\.mode !== 'rows' && !field/);
    assert.match(prepareSource, /payload\.mode !== 'multiple' && payload\.mode !== 'rows' && \(!Array\.isArray\(values\)/);
  ...
  test('只选择一次目录，冲突统一确认后才创建临时目录', () => {
    const multipleSource = handlerSource.slice(handlerSource.indexOf("} else if (payload.mode === 'multiple')"));
    const directoryIndex = multipleSource.indexOf("showImportOpenDialog('toolbox-split-export-directory'");
    const invalidTargetIndex = multipleSource.indexOf('const invalidTargets = targetPlans.filter');
    const conflictIndex = multipleSource.indexOf("buttons: ['返回', '覆盖全部']");
    const tempIndex = multipleSource.indexOf("fs.mkdtempSync(path.join(outputDirectory, '.toolbox-split-'))");
    ...
    assert.equal(multipleSource.match(/showImportOpenDialog\('toolbox-split-export-directory'/g).length, 1);
  });
  ...
  test('同名冲突确认使用“返回 / 覆盖全部”，返回仍在 FilePlan 与临时目录前整批取消', () => {
    ...
    const filePlanIndex = prepareSource.indexOf('prepared.filePlan = rows ? rows.filePlan : {', cancelledIndex);
