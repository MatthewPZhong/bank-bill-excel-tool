    ...
    const result = await generateValidateAndPublishReconFixExport({
      ...
      publishPublication: async (payload) => {
        ...
        assert.deepEqual(
          ...
          filePlan.outputs.map((output) => output.targetParentIdentity)
        );
        assert.equal(
...
test('artifact set/collision/target alias/symlink staging均在Publisher前拒绝', async () => {
  ...
  try {
    ...
    const staleParentRoot = path.join(harness.root, 'stale-parent-plan');
    const staleParentPlan = planFor(staleParentRoot, 2);
    const originalParent = path.dirname(staleParentPlan.outputs[0].filePath);
    fs.renameSync(originalParent, `${originalParent}.moved`);
    fs.mkdirSync(originalParent);
    assert.throws(
      () => createReconFixExportInput({
        result: harness.run,
        filePlan: staleParentPlan,
        artifactBindings: artifactBindingsFor(staleParentPlan, harness.run),
        stagingDirectory: emptyStaging
      }),
      (error) => error.code === 'RECON_FIX_EXPORT_FILE_PLAN_INVALID'
    );
