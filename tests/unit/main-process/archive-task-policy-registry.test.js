...
test('main 与独立 V327 注册器的 IPC 加内部任务，与 policy/support 精确相等', () => {
  ...
  assert.equal(actual.length, 268);
  ...
  assert.equal(registry.channels('exclude').length, 132);
...
test('71 file 与 63 no-file mutation 逐项显式分类且精确闭合', () => {
  ...
  assert.equal(excludeInventory.length, 132);
  assert.equal(excludeChannels.size, 132);
  ...
  assert.equal(registry.channels('exclude').length, 132);
  assert.equal(registry.list().length, 71 + 63 + 132);
});
