...
const stage = (name) => fs.writeSync(1, JSON.stringify({ phase, terminal, cut, hold: holdFlag === 'true', stage: name, elapsedMs: Date.now() - startedAt }) + '\n');
  ...
  const f = await createHost({ after(fn) { hooks.push(fn); } }, {
    ...
    wrapRecoveryControl(control) { return { runInControlTransaction(work) {
      return control.runInControlTransaction((tx) => {
        ...
        if (armed && anchored && cut === 'bundle') { stage('control-bundle-written'); process.exit(76); }
        return result;
      });
    } }; }
  });
