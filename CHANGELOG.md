...
> v3.1.8 发布候选重做 VCC 财务OP校验的输入完整性、结果调整、解归档、历史导出和模板生成规则。本节仅描述待发布行为，不表示 Windows 产物或资金人工验收已完成。
...
- **仍待人工确认**：发布前仍须在 Windows installer/portable 中实际打开两份模板，在 Excel/WPS 检查字体、填充、动态行、打印区与 M/N 可见性，验证受保护后台任务关闭时序，并由财务人员对真实月份逐主体、逐币种复核基础值、调整、差异、归档和下月期初。
