...
- **样式预算与写后校验**：输出按最终样式签名去重，并对 cell XF、字体、填充、边框和自定义数字格式设置安全预算。临时文件发布前会复核 ZIP/worksheet/styles 结构、实际样式数量、文件大小和摘要；活动日志记录预计/实际样式数、校验结果及有界日期降级样例。
- **整批可恢复发布**：所有产物先生成和校验，再通过同目录 staging、外部 journal 和固定恢复索引整批发布；覆盖过程中断会恢复旧目标，恢复受权限阻断时会给出人工恢复路径。启动和下一次工具箱任务都会重放未完成 journal。
- **性能与行数守恒**：30 万行级低样式文件走流式读取/写出，Worker 仅传控制消息，不把逐行数据送回主进程；合并、拆分、分页和混合 XLSX/XLS/CSV 路径均核对表头、行序、行数与长编号。
...
- 本版不修改资金对账、平盘、模板映射或业务存档规则。由于工具箱可搬运银行账单，发布前仍需用真实或脱敏的 `.xlsx`/`.xls` 在 Windows Excel/WPS 中核对日期、长账号、颜色、边框、行列布局和行数；自动测试不能替代人工数据复核。
