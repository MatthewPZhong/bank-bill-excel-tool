...
- **侧库防丢与备份门禁**：主库在侧库写入前持久登记基准 checkpoint 和 operation token；侧库以实例身份、generation、事务 token、父 token 与 operation token 历史校验同批数据。侧库缺失、空库、旧主库单独恢复、旧库回滚、同代或更高代次分叉都会阻断；旧 initialized 版本可验证迁移，备份恢复须包含整个软件数据目录中的主库和 `run-data/`。
- **运行审计防篡改**：运行来源由原始银行行重新推导，已确认消费冲突数由逐行血缘重新统计；同时修改 snapshot 和 summary 也不能绕过 stale 与审计门禁。
- **零数据文本格式**：静态模板与运行时 writer 共用标识符判定；结果、链接和原始表的业务 ID、账号、卡号、ReconID、CustomerRef、客户编号、客户号和 `accountReference` 等字段使用列级文本格式，只有表头的空表也能保留长整数和前导零。
...
- 平盘业务返回成功前会完成本次存档或记录可重试失败；管理页遇到侧库恢复门禁时会同时显示具体 `detailLines`。
