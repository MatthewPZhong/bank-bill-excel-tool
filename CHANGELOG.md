...
- **侧库防丢与备份门禁**：主库记录平盘侧库初始化状态；已初始化后若侧库缺失或被替换为空库会阻断使用，不再静默新建空库。备份恢复须包含整个软件数据目录中的主库和 `run-data/`。
- **零数据文本格式**：结果、链接和原始表的业务 ID、账号、卡号、ReconID 等字段使用列级文本格式，只有表头的空表也能保留长整数和前导零。
