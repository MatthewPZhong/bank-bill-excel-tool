...
- **小助手功能收纳弹窗**（PRD T3）：主页面右下角新增 🔄 按钮，点击弹出「小助手功能收纳」弹窗
  - 双区域布局：左侧「闲置功能」（按模块名 name.length 升序）+ 右侧「启用功能」（=左上角模块切换菜单的显示顺序）
  - 中间 ➡️ / ⬅️ 按钮：把选中模块在两区域间移动
  - 右侧每行末尾拖拽手柄 ⋮⋮：HTML5 原生 draggable，松手即落库
  - 即时落库（PRD O6 拍板）：无"保存/取消"按钮，每次变更立即写 SQLite `app_settings.enabled_modules`（JSON 数组）
