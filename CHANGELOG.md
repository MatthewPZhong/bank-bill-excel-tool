...
> ### v3.0.11 · 新增
>
> - **银行对账单处理统一 operation lock**（需求 3 批1 · 🔴 资金红线 · `main.js`）：新增模块级单例锁 `bankStatementOperationLock` + `tryAcquireBankStatementOpLock` / `releaseBankStatementOpLock`，import(`bank-statement:batch-import`) / run(`bank-statement:run`) / export(`bank-statement:export`) 三 handler 入口 acquire（争用即返回 `{status:'failed', message:'正在处理中…'}`）、`finally` 释放——三动作共享全局会话态（`bankStatementSession` / `processingResult` / `refundOrderSession`），统一一把互斥锁串行化防并发撕裂。新增 `bank-statement-op-lock.test.js`（源码接线 grep + 抽真实实现 `new Function` 验三动作两两互斥 / 释放后可重入）。
...
> - **需求 3 批2 run 数据准备让出（含 codex-P2 修复）**（🔴 资金红线 · `main.js` / `renderer.js`）：`bank-statement:run` 在 `yieldRun('prepare')`→`yieldRun('reconcile')` 之间「银行行深拷后」插 `yieldRun('prepare-clone-bank')` 让出（🔴 绝不在 `structuredClone` 中途让出、不删 clone）。**codex review P2 修复**：原拟在 linked-table 多步读取之间再插 `prepare-gw` / `prepare-linked` 两处让出，但 `linked-table:import` / `delete-by-date-range` 不在 `bankStatementOperationLock` 覆盖内 → 中途让出会让 run 把「改动前 gw」与「改动后 deposit/mid/recon」拼成从未真实存在的状态、存错 `processingResult`；故移除这两处内嵌让出，**linked-table 多步读取保持原子**（`prepare-clone-bank` 在 linked-table 读取之前、银行 session 受 op-lock 护，安全保留）。纯控制流，数据值 / 产物零变化。
