...
- **在打 tag 前定稿用户指南**：随应用打包的 `docs/USER_GUIDE.md` 现在使用不会随发布阶段过期的版本说明，不再把当前安装包写成“未发布候选”；当前公开稳定版、资产和发布时间始终以 GitHub Releases 为准。
- **保留 v3.1.11 的真实发布事实**：v3.1.11 的 tag、Release、资产和人工验收记录继续保留，同时明确其安装包内用户指南仍包含发布前状态文字；不替换、不覆盖已发布的同版本资产。
- **使用更高补丁版本交付修正**：应用与 lockfile 版本统一提升到 `3.1.12`，让修正后的指南进入新的 Setup 与 portable。除版本和随包文档外，不修改业务源码、数据库 schema、存档合同、金额、币种、方向、匹配或文件输出。
