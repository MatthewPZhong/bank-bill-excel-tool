...
- **侧库防丢与备份门禁**：主库与平盘侧库通过实例身份、generation、事务 token 和父 token 历史链校验同批数据；侧库缺失、空库、合法旧库回滚、同代分叉或更高代次分叉都会阻断。只有可证明主库 checkpoint 是当前侧库祖先的异常退出窗口才会自动追平；备份恢复须包含整个软件数据目录中的主库和 `run-data/`。
- **零数据文本格式**：静态模板与运行时 writer 共用标识符判定；结果、链接和原始表的业务 ID、账号、卡号、ReconID、CustomerRef、客户编号等字段使用列级文本格式，只有表头的空表也能保留长整数和前导零。
