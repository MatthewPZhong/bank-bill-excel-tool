...
> v3.0.14 交付 **1 项资金对账输出口径收紧 + 1 个新模块**：① 「资金对账数据处理」只对至少一个字段真实改值的行检测、保留「异常说明」，仅异常说明/no-op 行不再进「命中场景」；② 新增「前置资金对账」，3.0.14 交付「缺网关账单」：导入标准银行对账单，联合临时 MPT 网关账单和现有网关链接表，按非空对账 ID + 渠道 + 精确十进制金额 + 币种严格 1:1 对账，按银行渠道导出 5-sheet 结果。「缺渠道账单」、`MPT_CHANNEL_OTHERS` 和临时银行对账单延后到 3.0.15。`npm run release-check` 全绿（lint + smoke + unit **3452/3452** + integration **1817/1817**（39 脚本））。
...
3. **新导出文件为固定 5-sheet**：使用 `assets/资金对账导出不平.xlsx`，按渠道生成 `资金对账不平_{渠道名}_XXXX年XX月XX日.xlsx`，sheet 顺序固定为「不平结果 / 平账结果 / 网关账单 / 渠道账单 / 订单修复」。C4 网关对账单修复读取器同时兼容旧 4-sheet 和新 5-sheet 文件。
...
- **导出与结果可观测性**：网关空 ID、无效行、完全重复折叠、未使用候选和多候选 ID 组均显示统计；网关联合池无非空 ID 候选时阻断运行，结果库丢失/损坏或数据源变化时禁止导出。多文件导出中途失败会清理新文件并恢复原文件。
