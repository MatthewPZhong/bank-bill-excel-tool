...
> v3.1.8 重做 VCC 财务OP校验的输入完整性、结果调整、解归档、历史导出和模板生成规则。六项人工发布门禁已获用户明确确认通过，当前已批准进入受控正式发布流程；本节仍保持 Unreleased，不表示 `v3.1.8` tag 或 GitHub Release 已创建。
...
- **仍未正式发布**：以上人工签字允许继续 annotated tag、Windows Release workflow 与公开资产复核；当前不提前写入正式发布日期，也不宣称 `v3.1.8` tag、Release 或四项公开资产已经存在。
