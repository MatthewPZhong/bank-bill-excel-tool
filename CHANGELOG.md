...
> v3.1.9 当前为本地发布候选，尚未合并或正式发布。PR1—PR6 的本地实现和分阶段自动验证已经完成；独立代码评审、用户人工验收和最终发布总门禁仍按发布清单推进。
...
- 本地自动证据不能替代人工环境与资金复核。Windows installer/portable 实际运行、目标生产库的标准旧归档与触发行为、约 16 GB 数据库性能、约 700 万行多 Sheet 工具箱、跨盘符和网络路径、独立 copy/只读/容量行为、Excel/WPS 打开与另存，以及真实主体×九币种、跨月、调整、归档/解归档/删除和审计结果仍待用户人工验收。
- 上述 Windows、Excel/WPS、目标生产库和资金人工门禁当前均未记为通过；本条目不代表已经合并、创建 tag 或发布安装包。
