...
- `MPT_CHANNEL_OTHERS`、临时银行对账单和「缺渠道账单」调整到 **3.0.16**；现有导入提示同步指向 3.0.16。

---

## 3.0.14 - 2026-07-12

> v3.0.14 交付 **1 项资金对账输出口径收紧 + 1 个新模块**：① 「资金对账数据处理」只对至少一个字段真实改值的行检测、保留「异常说明」，仅异常说明/no-op 行不再进「命中场景」；② 新增「前置资金对账」，3.0.14 交付「缺网关账单」：导入标准银行对账单，联合临时 MPT 网关账单和现有网关链接表，按非空对账 ID + 渠道 + 精确十进制金额 + 币种严格 1:1 对账；完全重复仍按 `reconciliationId + 10 字段指纹` 折叠，但按渠道动态追加第 6 个「重复网关账单」审计页。「缺渠道账单」、`MPT_CHANNEL_OTHERS` 和临时银行对账单当时延后到 3.0.15，后续在 3.0.15 规划中再次调整到 3.0.16。最终 `npm run release-check` 全绿（lint + smoke + unit **3483/3483** + integration **1842/1842**（39 脚本）），前置资金 side DB 端到端 **62/62**。
