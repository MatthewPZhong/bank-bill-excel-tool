...
- 业务 OP：导入失败且有可用错误诊断时自动将原格式 XLSX 报告保存到 `error-reports/{日期}/`，分别显示业务结果和报告保存结果；移除手动导出错误报告和已提交任务的取消入口，保留提交前的返回/取消。
- 存档保留期限：默认期限与 14 个存档模块可分别设置，模块支持“跟随默认”和永久；新配置只影响之后新建批次，历史批次和已预约任务的期限保持原值。
- 存档永久删除：先预检再确认，仅清理存档原件及有持久归属证据的受管文件；运行、锁定、业务引用和恢复保护继续生效。清理尚未完成时明确提示并可重试，不把删除记录等同于文件已删除。
- 外观设置：新增默认关闭的定时深色模式，默认 18:30–06:00，按本机当地时间切换；支持跨午夜、唤醒校正及有效编辑自动保存。修复原生时间输入丢字和关闭排队后新时间丢失问题。
- 工具箱：拆分设置新增“按行拆分”，每份最多 N 条有效数据行、跨 Sheet 连续计数，一次最多生成 1000 份；保留来源格式及整批安全发布、覆盖确认和回滚保护。修复资源总配额不足时无效等待 5 秒的问题，改为立即显示中文原因及资源额度；临时占用仍可排队重试。
- VCC 财务 OP：支持同一 XLSX 的同类型或混合类型多 Sheet 预检与导入，通道主体按文件和 Sheet 指定；结果确认页可导出全部主体的待确认表和差异关联原表，未归档结果显示“待确认”；修复安装包内置模板读取及 FilePlan 首次来源绑定。
- VCC 兼容：存储结构升级到 v3，共享冻结输入并保留逐 Sheet 来源；旧记录继续按其原有身份核验。升级后不可用旧程序写 v3 数据，回退须使用升级前完整备份或既定维护恢复流程。
- 项目资料：统一版本文档目录为 `changes/v3.2.9/`，纳入项目通用规则与 Skill。自动检查、平台检查和人工验收分别记录在本版发布记录中。

## 3.2.8 - 2026-09-07

- Windows 启动修复：新版业务 OP 首次激活和恢复时，目录同步改用 Windows 所需的可写句柄，解决升级后因目录落盘权限错误无法打开主窗口的问题；完整新版模块及原有失败保护继续生效。
- 落盘链路：候选分片封存、旧文件回收、导出发布和存档中心相关目录同步采用正确的句柄权限。只读归档生成的内部候选副本仅调整自身权限，原件权限、内容及历史存档保持不变。
- 导出兼容：修复 Windows 文件编号超过安全整数范围时校验凭据保存失败的问题；文件身份无损校验，并继续兼容旧版本的安全数值凭据，工作簿内容不变。
- 资源提示：升级所需的 1 GiB 后台预算保持不变，资源不足时显示实际可用预算和预算上限，及时结束等待并保留原任务与迁移进度。
- 验收门禁：新增 Windows 正常应用与实际打包应用的首次激活、重启、导入、数据管理、核对、七类导出、删除及导入中断恢复验证；业务 OP 成功路径不再允许因目录能力不足跳过。
- 兼容：已进入新版的输入、历史结果、金额公式和输出列不变，无需因本次补丁重新导入。尚未完成首次升级的安装继续原有检查和升级步骤，不新增清理范围。

## 3.2.7 - 2026-09-07

- 启动与退出保护：业务 OP 首次升级及中断恢复资源不足时及时失败并保留原任务；补充合法验收证据不会使已激活安装失效，非法日历授权日期仍被拒绝。其他业务 OP Main 阶段对固定预算不足立即拒绝、暂时资源竞争最多等待 5 秒，启动错误保留具体原因。修复退出清理 Promise 链错误，等待实际清理完成后再退出。
- 业务 OP 新区间模块：复用 XLSX 富类型读取与既有后台执行平台，使用公共同步 SQLite 候选 writer；按已确认的账号、日期与高精度金额规则导入，以磁盘分片完成区间计算并保留可追溯历史结果。
- 六类业务表与错误报告独立流式导出、校验及发布；页面支持日期预检、版本选择、数据管理、删除影响预览和两种删除模式，原件由存档中心保管。
- 一次性升级使用持久迁移任务、阶段收据与严格旧文件白名单；旧恢复/孤儿/入口/DDL 按模式退役。失败保留原任务和资源保护，重启继续原阶段；旧客户端写路径有持久防写约束。
- 2026-09-06 按用户“全部开启”的明确要求，配置开启业务 OP 的十二项后台动作。授权记录为 USER_AUTHORIZED，未完成的 Windows 目录耐久、目标规模、真实业务/资金及 Excel/WPS 验收保持 NOT_RUN，不伪造 release-pass 或规模证明。
- 版本元数据收口为 3.2.7。实际切换须通过现有内存、磁盘、目录耐久、旧任务收口与五阶段升级检查；仅配置开启不等于用户数据已进入 ACTIVE。首次从旧业务 OP 模块升级会清理旧明细、旧结果和专属月度侧库，需从原件重新导入；历史存档、原件、用户锁、其他模块及外部文件保留，E5 恢复不变，详情见 `changes/v3.2.7/enablement`。
- 页面布局纠偏：主面板恢复 VCC 财务 OP 的两行四按钮结构，“导出数据”移入数据管理底部；数据管理恢复左侧分类、右侧表格。保留业务 OP 的列序、操作月份、四类输入导出及删除/取消合同。
- 页面细节：初始状态显示“欢迎使用小助手”，导入时原位显示“取消导入”；结果导出月份与表名横排缩窄，标题对齐分割线。数据管理底部改为“删除 / 导出 / 返回”，多页使用页码选择；业务 OP“导出原表”和 VCC“查看结果”均为左对齐文字按钮，与“操作”列标题对齐。
- 导出收口：差异文件不附“核对说明”工作表，保留 19 列异常摘要并指向完整原表；零差异只输出表头页。“导出数据”的账期/目标也横排对齐；导出原表时去掉管理页进度框，选取删除时去掉整版本处理注释，确认影响与取消仍可用。
- 运行日期：点击起止日期框后，月历默认展示最近成功导入的 OP 数据账期月份，只能选择当前有 OP 数据的日期；支持前后有数据月份切换。运行窗口标题对齐分割线，日期框缩至原宽的 2/3，【检查所需数据】移到左下角，原区间说明移除；运行前仍检查所需流水及数据版本。
- 导出弹窗细节：移除“只导出该类型、该账期当前可用的版本。”；导出数据的日期框缩至调整前的 4/5，目标下拉框缩至 1/2。两个导出弹窗的提交按钮统一为【导出】；“确认删除影响”标题与下方分割线左对齐。
- 紧凑弹窗：开始运行、导出数据、导出校验结果表、日期选择、删除确认和成功提醒参照 VCC 的字体、结构与小按钮，缩小整体尺寸和留白；表单与底部按钮左右对齐，长清单在内容区滚动。运行预检保留所需输入表格，移除额外的“所需输入齐全”说明。
- 导出反馈：导出结果原表期间，数据管理底部【删除／导出／返回】保持原位，【取消】与【删除】同尺寸并独立放在左侧。结果原表、OP 校验原表及流水校验原表仅在实际导出成功后弹出提醒，取消或失败不误报成功。
- 合计流水口径：新结果的【合计流水发生额】改为【流水入账合计 − 流水出账合计】，下一列改为【终止期末－合计流水】；倒推余额、核对差额、结论、原因及差异行集合保持一致。正负冲正、高精度金额及 `0.01` 容差规则不变。
- 计算版本兼容：已经导入新版区间模块的数据无需重导，同一区间首次重跑生成新口径版本，之后重复运行复用该版本。历史封存结果保留原数值、原公式说明与原表头，导出不改写旧结果。68 项相关自动测试及 17 组旧代码到新代码的跨版本回放通过；该证据不替代真实资金、Windows 目录耐久、2200 万输入/最大结果并集和 Excel/WPS 人工验收，详见 `changes/v3.2.7/pr3/flow-net-validation.md`。

## 3.2.6 - 2026-09-05（正式发布）

- **银行对账单赋值自身**：对账字段的 `vs` 改为“等于 / 包含”单选框，默认“等于”；“包含”按左侧包含右侧、去除首尾空白、区分大小写执行，任一侧为空不命中，数值 0 有效。配置、确认详情、复制及场景导入导出保持操作符一致，非法值在保存和执行前拒绝。
- **对账字段方向**：多条 AND 条件按各自选择的账单类型取值，同一类型对可正反向混用；修复后续反向“包含”导致的误赋值、误锁定和漏匹配，并验证未命中行可继续进入后续场景。
- **交叉候选歧义**：一对多左行的全部命中仍计入右侧多对一检查，仅双方唯一的配对可以赋值和锁定；交叉歧义完整告警，条件换序不改变修改及锁定对象，独立合法配对继续执行。
- **提取大账号顺序**：`MerchantId=自己输入` 模板在点击【提取大账号顺序】时，检查整批文件全部分段中明确识别出的账号是否已维护，包括已手动绑定、未参与提取及已保留的空交易分段。提示全部未维护账号和来源位置，用户确认后结束本批导入，不生成本批账单或保存本批账号顺序，历史结果保留。
- **空分段定位**：未维护提醒优先显示本段表头行，修复首段或中间空段误显示下一段交易行号的问题；无表头证据时保留预览行回退。
- **交互反馈**：提取期间禁止重复提取和完成提交；未维护提醒确认后的取消失败可重试。完全未识别、已维护多币种及普通手动选择仍沿用现有流程；长账号列表可滚动查看全部来源。
- **兼容**：旧 C2 操作符缺省按“等于”读取，无数据库结构迁移；旧的反向类型且左右异字段的等于条件会修正取值来源，命中结果可能改变，需纳入人工验收。降版前停用“包含”场景或改回“等于”。后台执行平台 production 开关和执行模式保持当前配置。
- **验收记录**：实现、自动测试与界面验证详见 `changes/3.2.6/implementation-notes.md`；脱敏真实样本的包含命中、账号归属及整批取消仍待人工复核。
...
- **生产仍关闭**：legacy seam 全部保留；冻结 R3.2.5 evidence 中 Windows packaged 的 `NOT_RUN`，真实业务样本和资金/恢复的 `PENDING_HUMAN_REVIEW`，以及 Excel/WPS、RSS 与稳定观察窗口的未执行状态保持不可篡改。自动测试和本次发布授权都不能据此启用 production。
- **历史证据与当前授权分层**：发布负责人 `MatthewPZhong` 于 2026-09-05 另行确认本次资金、恢复、真实业务样本及稳定窗口人工验收通过；该签字只覆盖本次正式技术发布，不改写冻结 R3 快照，也不授权 production。
- **发布授权与生产边界**：本版已获授权经受保护 PR、唯一 annotated tag 与 Windows Release workflow 发布技术 stable Release；workflow 内置 `release-check` 获授权，application production strategy、feature flag 与 effective worker 继续 disabled/legacy。最终 v3.2.5 资产产生后的 Windows 10/11 Setup/portable、SmartScreen、离线覆盖安装与 `production/latest` canary 按 Issue #220 在发布后逐项补做；本地 `release-check` / `check-vars` / `scan:vars` 未运行且不得记录为 PASS。
...
- **修复大批量收尾卡顿**：为 staging 的 `comparison_import_row_id` 增加 partial index，SQLite 清理暂存行时可直接定位自引用子行，不再反复扫描整张 staging 表。新库和已有 storage contract-v2 库都会在正常初始化中幂等补齐索引。
- **展示真实数据库阶段**：明细原表读取期间继续显示“正在导入”；某类明细完成全部文件读取、逐文件 SHA 核对和最终读取提交后，状态切换为“正在校验并写入”。充值清退与 Pending 分别显示自己的阶段和最终行数，系统财务 OP 行为不变。
