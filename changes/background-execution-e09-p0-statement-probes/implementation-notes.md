...
| `lastPendingBalanceSeedConfirmation`作为第六个legacy global独立投影 | 真实Main global持有完整plan/records/import/session与`assertFresh` callback；后续Service可用record + records digest + session/batch ref重新取证，不应跨线程复制重对象 | 把legacy confirmation整体送Worker；增加第四种token purpose；在P0实现token store | overwrite继续复用`manual-balance`单token；private context仅保留bounded record/digest/ref/count，公开值走既有interaction handle，`tokenId`不在prompt中重复 |
...
| mismatch message由固定摘要、总数和最多8个alias preview构造 | Main真实公式可拼接1024个raw filename；该原文必定被重建，任何预清洗字段上限都会在alias前误拒合法输入 | 提高最终message到无界；截断raw filename；放宽240 KiB ceiling | raw message只检查非空字符串；1024个255字符basename仍构造bounded aliases，最终interaction/Protocol继续受240/256 KiB ceiling |
| overwrite confirmation是`manual-balance`的exact prompt variant而非孤立transport | v3.2.3只允许`job:done -> interaction-required`公开interaction，外层handle已是唯一token authority；孤立`{status,message,tokenId}`绕过action result validator | 保留第二种transport；增加purpose/tokenId；提前实现E09-B waiting-user | 仅`statement:resolve-manual-balance`的exact result接受`{status:'confirm-overwrite',message}` prompt；真实Registry/Protocol/validateResult/Supervisor统一校验 |
...
| overwrite曾冻结孤立`{status,message,tokenId}` continuation | 删除该export与projection，改为`manual-balance` exact prompt + 完整canonical interaction-required result；仅resolve-manual-balance action接受 | Reviewer Round2 P1：孤立transport绕过真实result binding并形成第二token authority | probe public bytes由完整result计量；P0仍不实现token store/live Worker | 不需要，收敛到既有TechDoc result合同 |
| raw legacy mismatch message曾以240 KiB字段上限预先校验 | 改为仅校验非空字符串，alias重建后仍执行完整240 KiB与Protocol 256 KiB检查 | Reviewer Round2 P1：1024个合法255字符basename按Main公式已超过该预清洗上限，但raw值不会进入public graph | 不改变三purpose业务shape、alias/fileName单项边界或最终wire ceiling | 不需要，消除不可达字段上限 |
...
| 50k/4批次/1 token probe v2（default + `TMPDIR=/tmp`） | Round2前deterministic exact-equal：state raw/estimated 23,614,188/35,422,208 B；big pending 5,912,626/8,871,936 B；overwrite pending 2,012/4,096 B；public interaction 795 B；孤立overwrite DTO 125 B（Round2已由完整result替代） | 六globals、独立overwrite footprint与TMPDIR确定性；动态heap/RSS仅校准，不作为跨runner golden |
...
| Reviewer Round2 focused / affected unit | focused 22/22 PASS；affected 179/179 PASS（9 files） | exact overwrite prompt/result、真实frozen Registry/Protocol/validateResult/Supervisor正反路径、Main真实mismatch公式、inner +1与256KiB真实over-wire，以及全部相关Platform/legacy回归 |
| Reviewer Round2 integration / smoke | `statement-generation-pipeline.js` 45/45 PASS；`npm run smoke` PASS | Statement production generation seam与全仓主要集成场景未因P0合同收敛回归 |
| Reviewer Round2 50k probe（default + `TMPDIR=/tmp`） | deterministic exact-equal：state raw/estimated 23,614,188/35,422,208 B；big pending 5,912,626/8,871,936 B；overwrite pending 2,012/4,096 B；public interaction/完整overwrite result 795/368 B；两类预算均268,435,456 B | 六globals、完整canonical overwrite result与TMPDIR确定性；default和`/tmp`动态heap/RSS不同且不进入golden |
| Reviewer Round2 static | full `src/` ESLint PASS（只读主仓依赖）；全部changed JS `node --check`、fixture JSON parse、`git diff --check` PASS | 语法、lint、JSON与空白边界；未调用release-check/check-vars/scan:vars |
...
| filename/message隐私 | BigAccount constructor对raw basenames按跨数组首次出现顺序生成alias，mismatch只输出固定摘要/总数/8项preview；raw message仅做非空类型检查但不复制，message/fileName没有finance-safe例外；result validator拒绝任何未规范化结果 | full-account/raw客户名正例构造后wire JSON零命中；Main真实公式的1024个255字符basename可先形成超240KiB raw message，再产出bounded Registry/Protocol DTO；普通raw mutation仍被binding拒绝 |
