...
| 最小泛化E07-C exact-runtime external-parser finalization seam；BankBU在`runtime.start`后首个await前登记owner | app shutdown必须看见外置Parser terminal、parent terminal与task-private spool cleanup，且cleanup rejection必须由同一runtime重试 | BankBU复制第二套shutdown状态机；把BankBU policy接入global runtime | runtime clean严格晚于Parser→parent→cleanup；E07-C薄wrapper保留既有错误码/调用面。 |
...
| 证据 | 结果 | 覆盖的行为/风险 |
| --- | --- | --- |
| E08-B focused unit | `12/12 PASS` | 真worker、role/month/source/row血缘、乱序parity、source-change、failure/cancel、tamper、gate/topology、静态禁DB/matching/receipt/mirror；Round1 delayed abort terminal与cleanup retry owner |
| E08-A + BankBU affected unit | `117/117 PASS` | E08-A policy/singleton/Supervisor/Inspector/side-main identity、reader/DB repository与dual-source旧路径 |
| E07-C external Parser shutdown affected unit | `20/20 PASS`；Duplicate runtime/policy `5/5 PASS` | 通用seam保持E07-C真实Worker shutdown timeout、cleanup lock/retry、receipt/lease与exact-runtime兼容 |
...
| Duplicate end-to-end integration | `31/31 PASS` | E07-C wrapper泛化未改变Duplicate业务、恢复或shutdown相邻链路 |
| 全量smoke | PASS；BankBU `41/41` | BankBU资金链与全仓相邻业务smoke无回归 |
| 静态检查 | full `src/` ESLint、changed test/fixture `node --check`、`git diff --check`均PASS | 通用seam、dispatcher、测试fixture无语法/规则/空白错误 |
...
| app shutdown只看Supervisor、遗漏外置Parser与spool owner | exact runtime统一收集external finalizer；BankBU同步登记，shutdown先等真实Worker barrier，再等parent Supervisor，最后等finalization | 真实delayed Parser worker在60ms观察窗内保持shutdown pending、parent active、staging存在；terminal后才clean |
| cleanup rejection后owner丢失或runtime误报clean | retryable cleanup记录保留在exact runtime；每次shutdown继续同一owner，真实删除成功才释放记录 | 模拟外部`EBUSY`时首次/连续shutdown均errors+leak，解除后第三次真实删除且clean |
| 为shutdown race延后创建Parser controller后，外层signal可能在窗口内取消 | dispatcher记忆pre-start abort，controller创建即回放；不改变原取消错误/恢复语义 | failure/cancel focused case改为pre-start abort并通过，真实in-flight abort由delayed Worker回归覆盖 |
