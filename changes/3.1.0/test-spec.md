...
- 主库无批量明细，持久侧库重启可恢复；主库 checkpoint、预登记 operation token 与侧库实例身份、generation、token、父链和历史 operation token 全部相容时才能继续。覆盖缺失/空库、旧 `initialized` 标记迁移、合法旧库回滚、仅恢复旧主库且无待完成操作、同 generation 分叉、更高 generation 分叉、完整同批恢复，以及待完成操作可证明的侧库领先窗口安全追平。
