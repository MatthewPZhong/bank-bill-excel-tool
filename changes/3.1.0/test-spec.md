...
- 每条运行行完整性 hash、原始/结果行、唯一允许变更字段、结果去向、差异精确集合、来源消费标记、血缘及当前银行/链接引用分别覆盖篡改测试；旧非空草稿缺少新完整性字段时拒绝读取。
...
- 覆盖正式存档建批失败后的持久 outbox、outbox 完整性校验与启动重放；异常退出发生在侧库提交、业务成功标记或输出文件发布之后时，必须恢复存档意图。正式批次和 outbox 都无法登记时返回“业务可能已完成、请勿直接重试”的明确错误。
