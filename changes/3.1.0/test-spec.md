...
- 主库无批量明细，持久侧库重启可恢复；主库 checkpoint 与侧库实例身份、generation、token 及父 token 历史链相容时才能继续。覆盖缺失/空库、合法旧库回滚、同 generation 分叉、更高 generation 分叉、完整同批恢复，以及父链可证明的侧库领先窗口安全追平。
