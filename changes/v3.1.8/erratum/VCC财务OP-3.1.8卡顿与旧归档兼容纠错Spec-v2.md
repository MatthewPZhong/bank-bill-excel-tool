# VCC 财务 OP v3.1.8 卡顿与旧归档兼容纠错 Spec v2.1
...
> amends: `Spec v2.0（2026-08-10）；仓库路径为兼容既有链接保持不变`<br>
> status: `v3.1.9 产品口径已锁定；本地实现与自动验证完成，待真实环境和财务人工验收`
...
### 0.1 2026-08-13 CNY 与异常数据过滤修订

本修订只前向约束 v3.1.9，不追溯改写 v3.1.8 已发布二进制、tag、冻结 Spec 或发布证据。以下合同是最新口径，并覆盖本文 §1.3“币种不变”及 §2.2“结果模板、导入幂等规则不变”中与之冲突的部分：

1. VCC 财务 OP 的九币种唯一标准集合改为 `AUD/CAD/CNY/EUR/GBP/HKD/JPY/SGD/USD`；当前结果、校验、期初、归档和导出不得再生成 `CNH` 坐标。
2. “系统财务OP”取消 `CNY → CNH` 归一化。新文件中的 `CNY` 保持 `CNY`；新文件中的 `CNH` 按不支持币种过滤，不得静默改写后入库。
3. VCC 财务 OP 校验结果表的固定币种列由 `CNH` 改为 `CNY`；模板文件及其完整性 hash 同步更新。
4. 明细导入按“行”处置；系统财务 OP 按“主体 × 精确九币种快照”处置。可归因到单行或单主体快照的异常只过滤对应单位，其余正常数据必须继续落入有效表。
5. 文件损坏、工作簿/表头结构不可信、取消、数据库错误、账期已归档等组级失败仍整组失败关闭；不得把结构不可信或业务门禁失败降级为“异常过滤”。
6. 每次导入必须满足数量守恒，并在 UI/导入审计中分别展示新增、幂等跳过、异常过滤和回滚数量；不得以“新增 0、跳过 0”掩盖未启动或失败的任务。
...
- 除 §0.1 与 §15 明确冻结的 `CNH → CNY` 合同修订外，不改变金额、主体、九币种余额公式、调整公式、跨月期初或五表计算规则。
...
- VCC 九币种标准集合改为 CNY、系统财务 OP 取消 CNY→CNH、结果模板列名修订、异常单位过滤与正常数据继续落库。
...
- 除 §0.1 与 §15 明确修改的结果模板币种列和异常处置外，不改变金额精度、业务键、内容 hash 或幂等比较语义。
- 不把工具箱改动混入 VCC 纠错 PR。
...
| D13 | 九币种标准集合 | `AUD/CAD/CNY/EUR/GBP/HKD/JPY/SGD/USD`；当前事实与结果不得生成 CNH。 |
| D14 | 系统财务 OP 币种 | CNY 原样保留；取消 CNY→CNH；新 CNH 为异常，不自动改写。 |
| D15 | 结果模板 | 固定币种列使用 CNY；模板 SHA-256 随受控模板更新。 |
| D16 | 明细异常处置 | `invalid_key`、`format_error`、`idempotent_conflict` 只过滤对应行；其余 accepted 行正常落库。 |
| D17 | 系统 OP 异常处置 | 单个主体必须形成精确九币种原子快照；异常主体快照过滤，其他完整主体快照正常落库。 |
| D18 | 历史 CNH | 原始大表/审计原文不批量改写；派生小表受控迁移，读取/导出边界仅把历史精确 CNH 解释为 CNY；冲突失败关闭。 |
...
| AC-21 | 新系统财务 OP 文件使用 CNY | 导入并计算 | CNY 原样落库并进入 CNY 结果坐标，系统链路不生成 CNH。 |
| AC-22 | 新明细或系统财务 OP 文件使用 CNH | 导入 | 对应行或主体快照计入异常过滤；不得自动改写为 CNY，其他正常单位继续落库。 |
| AC-23 | 同一明细文件同时包含正常行与异常行 | 导入 | 正常行 accepted 并进入 effective；异常行保留 disposition/error；状态为 `success_with_skips`，数量守恒。 |
| AC-24 | 同一系统文件含一个完整主体和一个异常主体 | 导入 | 完整主体的九币种快照落库；异常主体快照过滤并保留错误证据；不得因局部异常回滚完整主体。 |
| AC-25 | 工作簿损坏、表头/Sheet 不可信、取消或账期已归档 | 导入 | 整组失败关闭或取消；不得以局部过滤绕过结构/状态门禁。 |
| AC-26 | 当前结果、归档和三类导出 | 展示/写文件 | 固定九币种中只出现 CNY，不出现 CNH；结果模板固定列为 CNY。 |
| AC-27 | 历史合法 CNH 派生事实 | 启动升级并读取/导出 | 派生小表迁为 CNY；大表原始事实不批量重写；读取结果统一为 CNY，原始审计仍可追溯。 |
| AC-28 | 同一资金坐标同时存在 CNH 与 CNY，或历史 JSON 无法无损迁移 | 启动升级 | `vcc-currency-migration-blocked`，事务零部分提交，不合并两种币种金额。 |
| AC-29 | lifecycle 未启动、busy、failed 或 success 但无结构化 record | renderer 处理返回值 | 明确显示失败，不得显示“新增 0 行、幂等跳过 0 行”的假成功。 |
...
- 结果 DTO、结果表、期初、调整、归档、校验表和导出均使用上述坐标；禁止同时出现 CNY/CNH，也禁止将 CNY 显示为 CNH。
- 结果模板固定币种列使用 CNY；v3.1.9 模板 SHA-256 为 `48c8161484128e63a6e3e60724336f2433a8f23687695d980720c59a9dec2053`。
- “系统财务OP”币种读取为 identity：输入 CNY 得到 CNY。`CNH` 不属于新导入允许集合，按异常单位过滤并记录原值和来源位置。

### 15.2 明细导入处置

明细导入的最小处置单位为一行：

- `accepted`：业务键、格式和币种均合法，且未命中冲突；写入 effective。
- `idempotent_skip`：同键同内容；不重复写 effective。
- `invalid_key`、`format_error`、`idempotent_conflict`：只过滤该行，原始行、原因和对比血缘保留在导入审计。
- `rolled_back`：读取未完成、取消、数据库或组级失败导致本组事务不能安全完成。

导入记录必须满足：

```text
rawCount = insertedCount + skippedCount + invalidKeyCount
         + conflictCount + formatErrorCount + rolledBackCount
```

有 accepted 行且同时存在 skip/异常时为 `success_with_skips`；零 accepted 且存在异常时为失败；全部同内容时可为 `all_skipped`。异常行不得阻止同一可信文件中其他正常行落库。

### 15.3 系统财务 OP 处置

系统财务 OP 的最小资金处置单位不是单行，而是同一账期的“主体 × 精确九币种快照”：

- 同一主体只有九币种全部存在、币种唯一、字段和金额有效时，快照才可 accepted。
- 任一行异常、币种缺失/重复、含 CNH 或余额无效会过滤整个主体快照，防止把残缺九币种余额写入有效表。
- 同文件或同导入组内其他完整主体快照继续落库；错误表保留异常主体、文件、Sheet、行号和字段。
- 工作簿无法读取、业务 Sheet/表头不唯一、1904 日期系统或其他无法建立可信候选集合的错误属于 hard failure，整组失败关闭。可精确归因到某一数据行的模板外额外列属于该行/主体异常，按本节隔离。
- 已归档账期禁止新增快照；该门禁不是可过滤的行级异常。

### 15.4 历史 CNH 兼容与迁移

- `currency_contract_version=2` 是 CNY 合同的幂等升级标记。
- 启动迁移只改写可枚举且规模可控的派生结果/余额/调整事实及相应 content hash；不批量更新千万级 `vcc_fin_op_import_rows`、`vcc_fin_op_effective_rows`，也不改写 `raw_json` 原始审计文本。
- calculator 与数据导出在历史读取边界仅将精确大写 `CNH` 解释为 `CNY`；不得把大小写错误或其他币种静默归一化。
- 同一资金坐标同时存在 CNY/CNH、balances JSON 同时含两键、缺字段、未知币种或无法重算 hash 时，升级必须以 `vcc-currency-migration-blocked` 事务回滚；禁止相加、覆盖或择一。
- 升级后低于 3.1.9 的版本不理解该币种合同，不支持继续写库；回退只允许恢复升级前完整备份。

### 15.5 可观测性与人工验收

- UI 完成摘要必须分别显示新增、幂等跳过和异常过滤；异常项提供进入数据管理查看审计的入口。
- 顶层 lifecycle 非成功、无 records 或业务失败必须显示真实错误，不能用零计数兜底为成功。
- ⚠️ 人工复核至少覆盖：一个 CNY 系统主体九币种快照、一个含异常行和正常行的明细文件、一个含异常主体和正常主体的系统文件、历史 CNH 月的计算/归档/导出，以及迁移前后逐主体逐币种金额守恒。
