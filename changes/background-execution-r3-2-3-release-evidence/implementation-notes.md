...
| 历史 exact suite 继续仅由当前外层 wrapper 兼容 Windows worktree：按每个进程当前 `HEAD` 只投影已跟踪普通文件的 POSIX permission bits，并把 exact/candidate checkout 固定为 LF。 | exact Windows job `99769281418` 已证明 separator 与 candidate 依赖问题关闭，但 nested `22` 项仍以单一 `GIT_WORKTREE_TREE_INVALID` 造成 `10 pass / 12 fail`；历史 validator 要求 `2016` 个 `100644` 与 `2` 个 `100755` worktree 文件精确呈现 `0644/0755`，Windows Node 无法原生表达。该 validator 随后还以 `git hash-object` 校验原始 worktree bytes；runner 原始 `core.autocrlf` 未打印，但本地可丢弃 Git probe 证明 CRLF checkout 会使 blob OID 漂移。 | 修改 `57fab04a`/current validator、跳过 Windows historical suite、全局包装 `fs`/Git hash、关闭 type/content/index/audit 门，或把未观测 `core.autocrlf` 写成既成根因。 | preload 仅在真实 win32 解析当前 cwd/HEAD 的严格 `ls-tree -z` mode map并包装 `fs.lstatSync`；仅请求路径等于其canonical realpath且位于cwd内的tracked、writable regular file投影低 `0777`，readonly/dir/symlink/untracked/off-root保持原值。受控 Git env 只覆盖 `core.autocrlf=false`；HEAD/index mode、realpath/type、blob hash、status/diff/audit roots仍由历史 validator 原样校验。 |
...
| 受控 mode/LF harness 首轮本地机制回归 | 官方Node22.18；严格 `ls-tree -z` parser拒绝非法mode/type/path/duplicate；forced `path.win32` mode投影覆盖tracked 0644/0755且readonly/dir/symlink/untracked/off-root不变；可丢弃Git probe证明`core.autocrlf=true`的CRLF worktree hash不等于HEAD、false时精确相等；当前外层`1/1`并复验nested historical`22/22` | 证明实现边界与负例在本地未被遮蔽；macOS forced probe不等于真实 Windows PASS，最终exact Windows unit/integration仍是合并门。 |
...
| 受控 worktree mode投影与LF配置在真实 Windows 的行为 | PROBE；mode充分根因来自exact日志+锁定历史代码，runner原始`core.autocrlf`仍未观测；本地只验证forced win32纯函数、Git config继承与blob机制 | 推送严格新head后核对nested 22/22、candidate CLI、worktree content/type/index负例与root清理；失败按新exact日志隔离 | 阻止合并；不得把mode投影扩大到untracked/readonly/type/content，亦不得修改historical/current validator或关闭Git hash。 |
