...
| Windows historical candidate 的实际 Git baseline 拓扑 | PROBE；job `99803981567` 只证明candidate tree count与exact五新增diff不同时满足，安全摘要没有实际count、missing/extra path、mode/status/index/config；四个真实Git反例允许额外errors，不能反证baseline clean | current outer wrapper先运行同语义bounded probe；若Windows仍失败，仅依据计数、规范相对路径、mode/status与local config选择最小修复 | 阻止合并；不得猜路径碰撞/filemode/ignorecase原因，不得接受额外Git errors或输出raw snapshot/sentinel。 |
