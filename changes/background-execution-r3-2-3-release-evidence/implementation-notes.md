...
| 当前外层 wrapper 在 nested suite 前复刻 historical candidate，并只在 baseline 漂移时输出有界、脱敏 Git 元数据。 | exact Windows job `99803981567` 已让 historical 主仓、raw-EOL、win32 path/mode/LF preload、authority 与 21/22 项通过；唯一失败是 duplicate-key candidate 在 raw parser 前收到 `GIT_HEAD_TREE_INVALID + GIT_CHANGED_PATHS_INVALID`。历史 CLI 安全摘要不包含实际 tree count、missing/extra paths、raw diff mode/status、index/status 或 local Git config，现有日志只能隔离到 candidate 构造/commit 拓扑，不能支持猜修复。 | 修改 historical/current validator、接受额外 Git errors、记录 raw snapshot/sentinel/绝对 runner 路径、拦截 Git/child_process，或把四个“包含目标code”的真实Git负例当 baseline clean 证明。 | 探针逐字复刻 exact base/branch/main ref/五文件/单父 commit；失败摘要只含计数、布尔值、Git mode/status、最多20个规范 repo-relative path 与必要OID，unsafe path只留hash；baseline精确后仍要求 CLI 唯一返回 `RAW_JSON_DUPLICATE_KEY`，finally精确清理candidate root。 |
| 当前外层 historical wrapper 使用固定短 root `r3-`，bounded candidate 与历史 helper 共用精确 prefix `v323-release-evidence-`。 | exact Windows job `99821320839` 中 bounded probe 已通过，随后 nested historical 仍为 `21/22`；失败 candidate 使用历史 prefix 后，两条在 exact base/head 均为 tracked blob 的最长路径达到 `262/261`，而先前 bounded probe 的较短 prefix 未覆盖该预算。以同一已观测 Windows temp root 计算，短 outer root 将精确同名 candidate 收缩到 `245/244`。 | 设置或依赖未观测的 `core.longpaths`、缩短历史 helper、自行改 Git 配置、修改 historical/current validator，或把推断的 actual tracked count 写成事实。 | 当前 test 锁定历史 helper 的 `os.tmpdir()+v323-release-evidence-` 构造恰好一次、两条最长路径在 base/head 的 blob 身份及旧/新长度；仅缩短本轮可丢弃 outer root，Git tree/diff/index/status/content/type/audit 门与 bounded 诊断隐私边界不变。 |
...
2026-09-01 的第五个 exact Windows historical 日志证明此前 adapter 已让主 historical repo 与 21/22 项通过，但也推翻了“candidate 继承成立即可等价推出 candidate baseline 精确”的判断。`1f1d5fda...` 的唯一 nested 失败发生在第一个 raw duplicate-key candidate：Git guard 先报 tree count/diff contract，第二个 number candidate 与 integration 均未启动。当前安全CLI只暴露错误code，不暴露会改变修复方向的路径/mode/status事实；实际方案因此不是放宽 validator，而是在 current outer test 中先复刻同一 candidate，输出严格限界的 Git metadata。该 probe 是为下一次真实Windows失败取证，不改变验收口径，也不把本地成功写成Windows PASS。

2026-09-01 的第六个 exact Windows historical 日志推翻了“bounded probe 与历史 helper 已具有相同文件系统路径预算”的假设。`a213495f...` 的 bounded baseline 先通过，nested 仍只在 duplicate-key candidate 失败；日志中的 current outer root 与锁定历史 prefix、两条 exact base/head tracked 长路径共同证明历史 candidate 达到 `262/261` 字符，而旧 probe 因 prefix 较短未覆盖这一边界。actual tracked count 与 `core.longpaths` 均未观测，故不作归因或配置补丁；实际方案只把 current outer prefix 缩短为 `r3-`，并让 bounded probe 使用历史精确 prefix，使同一已观测 temp root 下的预算为 `245/244`。历史字节、Git guard 和 production/资金合同不变。

本轮最初按现有“17文件矩阵`436/436`”标签复跑显式17个路径，实际TAP为`354/354`；随后把此前未列出的四个后台核心文件纳入同一命令，21个实际路径为`452/452`。源码静态`test(`数量不能代表包含nested subtests的TAP总数，因此“补四文件会精确恢复436”的中间假设无效。两轮均0 fail/skip/cancelled，完整unit亦一次全绿；旧`436/436`作为此前head的历史证据保留，不被本轮伪造复写，本轮最终矩阵明确记录实际21路径与`452/452`。
...
| Windows historical candidate baseline exact失败 | job `99803981567` checkout `1f1d5fdaaa20385315736dc7d7f5644785f1b2e4`；lint/smoke成功，unit `6592/6595`、1 fail/2 skip/0 cancelled，integration未启动；nested `57fab04a` 为21 pass/1 fail，唯一 raw duplicate-key candidate 先返回 `GIT_HEAD_TREE_INVALID + GIT_CHANGED_PATHS_INVALID` | raw-EOL、主历史仓 path/mode/LF、authority、tamper/privacy/action-scope与4个真实Git反例均已到达；失败类别收敛为candidate构造/commit拓扑，但CLI未输出实际count/path/mode/status，不能据此选择具体兼容修复。 |
| Windows historical candidate 路径预算 exact失败 | job `99821320839` checkout `a213495fbab074119e90c2cf02d401d15cc33256`；lint/smoke成功，unit `6592/6595`、1 fail/2 skip/0 cancelled，integration未启动；current bounded candidate baseline 已通过，nested `57fab04a` 为21 pass/1 fail，仍只在 duplicate-key candidate 返回 `GIT_HEAD_TREE_INVALID + GIT_CHANGED_PATHS_INVALID` | bounded probe 与历史 helper 的 candidate prefix 不同；历史精确 prefix 下两条 base/head tracked blob 的路径为 `262/261`，越过传统 Windows 259 字符预算。actual tracked count与`core.longpaths`未观测；证据只支持缩短 current test outer root，不支持 validator/Git 配置变更。 |
| candidate baseline bounded probe 本地机制回归 | 官方Node22.18；current outer wrapper在正常temp下外层`1/1`且nested historical `22/22`；探针复刻exact base/branch/main ref/五文件/单父candidate，核对2018 tree、五个100644 additions、index/status/worktree，并要求CLI唯一`RAW_JSON_DUPLICATE_KEY`；candidate root在finally无残留 | 本地证明成功路径与诊断边界可执行；失败摘要不含内容/sentinel/账号金额/绝对路径/环境值。真实Windows具体拓扑仍为PROBE。 |
| candidate baseline bounded probe 最终本地验收 | 官方Node22.18；正常temp与精确`RUNNER~1` alias下current wrapper均外层`1/1`、nested historical`22/22`；v3.2.2 evidence `30/30`；R3/E10-B/protocol/privacy及前序17文件矩阵`436/436`；完整unit `6592/6595`、0 fail/3 Windows-only skip/0 cancelled，日志`logs/unit-tests/unit-20260901-181354.log`；`check:packaged-inputs` PASS、lint exit0、changed JS node-check与diff-check通过 | bounded probe、candidate CLI唯一raw code、历史22项负例及前序业务合同同树通过；真实Windows candidate拓扑与首次可达integration仍为PROBE，本地结果不代偿exact Windows CI。 |
| short-root path-budget 最终本地验收 | 官方Node22.18；current wrapper正常temp与精确`RUNNER~1` alias均外层`1/1`、nested historical`22/22`；v3.2.2 evidence`30/30`；显式17路径矩阵`354/354`、补齐四个后台核心文件后的实际21路径矩阵`452/452`，均0 fail/skip/cancelled；完整unit一次通过`6592/6595`、0 fail/3 Windows-only skip/0 cancelled，日志`logs/unit-tests/unit-20260901-191114.log`；`check:packaged-inputs` PASS、lint exit0、changed JS node-check与diff-check通过 | 锁定历史helper同名prefix、base/head最长blob与`262/261 → 245/244`预算；historical tamper/privacy/action-scope/4个真实Git反例、E10-B Publisher/cleanup和protocol/Supervisor同树通过。真实Windows bounded baseline、nested22与integration仍为PROBE。 |
...
| 短 outer root 与历史精确 candidate prefix 的真实 Windows 路径预算 | PROBE；job `99821320839` 已证明 bounded baseline 通过而历史同义 candidate 失败；两条 exact base/head tracked blob 在旧 outer 下为`262/261`、短 root 下为`245/244`。actual tracked count与`core.longpaths`仍未观测 | 新 exact Windows CI 核对 bounded baseline、nested `22/22`、candidate唯一raw code、unit与integration；失败只依据有界facts继续隔离 | 阻止合并；不得设置`core.longpaths`、放宽Git guard、修改historical/current validator或输出敏感内容。 |
