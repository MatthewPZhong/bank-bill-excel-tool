...
| `t=d` 直接验证raw XML值，不做trim或其它预规范化 | SheetJS对带空白的日期会走不同本地日期解析并形成不同serial；trim会把旧业务回读拒绝的workbook静默修复为期望日期 | trim后再验证；复制SheetJS宽松日期parser并接受其所有宿主差异 | canonical ISO/Zulu保持旧业务等价；leading/trailing/tab/newline与empty/whitespace-only统一fail closed，日期digest不再掩盖raw漂移 |
| canonical re-encode只用于cell refs及dimension/merge端点，不收紧outer row数字词法 | 旧SheetJS保留`D02`物理key导致D2缺失，但outer `row r=02`只提供row metadata，canonical cells仍按D2可见且业务通过 | 所有row digits一律`[1-9]\d*`；只校验Number坐标 | `D02`、A01/I02及超长前导零拒绝；outer row仍数值化并与canonical cell row比较，保持旧raw兼容 |
...
| raw date/canonical ref红→绿差分 | 修复前strict suite 12/16 PASS，四个失败面精确为date whitespace、`D02` cell、dimension前导零、merge前导零；修复后16/16 PASS。canonical/Zulu date与旧业务结果deep-equal；leading/trailing/both/tab/newline、empty/whitespace-only fail closed；`D02`/`D0002`/128位前导零与dimension/merge端点拒绝；outer `r=02`旧raw/strict deep-equal | 证明异常date不再被trim，物理cell key和range端点必须canonical；outer row兼容边界有独立oracle而非机械收紧 |
| raw date/canonical ref focused/golden | E10 generation 17/17、background/Statement/Toolbox focused 484/484、独立NewAccount integration 36/36 PASS | 日期/账户/币种/余额0/记录数/文件名、shape resource、5digest、取消/cleanup与其他action路径不回归 |
| raw date/canonical ref 250k/60,416真实边界 | 250k六阶段取消11–16ms，result/generated=null、Main cleanup=1、staging/final=0且无resource/transport leak；正常250k Worker14,042ms、peak RSS1,449,623,552、ticks2,379；60,416长文本Worker6,407ms、peak RSS1,260,044,288、ticks1,077；两者四digest精确不变 | 词法gate未改变streaming/yield/cancel或shape reservation；macOS耗时/RSS只作方向性证据，Windows packaged保留人工门禁 |
| raw date/canonical ref full regression/static | full unit首轮6328/6333，除NSIS基线外出现一次既有archive symlink并行波动；archive单文件46/46且full unit重跑6329/6333，仅父链Windows NSIS `System::Store` 1 failure、3 skipped；integration51 scripts/2455/2455、270110ms；smoke、lint、changed node-check、diff-check PASS；runner timing文档已恢复 | 新用例纳入全量平台/XLSX/资金路径；archive波动经隔离重放反证，未改其代码；无本次代码失败或范围外churn |
| raw date/canonical ref最终 blindspot/reconciliation 与重要变量软review | 命中Important-skeleton Worker读写管线和risk-sensitive日期/账户/行列/dimension/merge/digest/staging血缘；异常raw date与非canonical物理key在进入businessEvidence前fail closed，正常金额0、币种、日期、账号、行数/文件名守恒；未改generic scanner、Writer、业务normalizer、Supervisor/Governor、Protocol/resource/live selector | production保持`false/legacy/0`，无E10-B Publisher/copy；NewAccount资金字段和Windows packaged仍需人工复核；依禁令未运行`check-vars/scan:vars/release-check` |
