...
- Final restack parent：E11-C `d53d31ebe6463489bf8b1b238ab7ef2a0a97a2b0`；合并来源 `38c319427577671e3e3708b0dd3d4df4a3aff621`。`background-execution/runtime.js` 与 toolbox policy inventory 采用并集解析，保留最终 v3.2.3 的 NewAccount/FundRecon/Duplicate/parser-first shutdown，同时加入 VCC policy/entry/validator。
...
| Final FilePlan 的 `targetParentIdentity` 作为 E12-A managed Publisher 的强制 Main authority 贯穿 canonical plan、Join artifact 与 durable Publisher；legacy wrapper 未提供该新字段时保持旧行为。 | 最终 v3.2.3 已把 direct-parent identity 纳入每个 output；若 E12-A 仍按旧字段集比较，合法 plan 会在 Worker 前全部失败；若丢弃该字段，目标父目录被替换时会退化为只校验 lexical path/snapshot。 | 从 FilePlan 删除新字段；让 wrapper 对所有 legacy caller 无条件要求新字段；在 VCC 内复制另一套 parent checker。 | managed path 设置 `requireTargetParentIdentity=true` 并传 exact identity；legacy live 路径仍未接 managed capability，避免 dormant PR 改变现有生产调用。 |
| 传输到 SafeError detail 的 staging SHA-256 采用 `sha256:xxxxxxxx:...` 分组编码，内部 identity authority 仍保留原始 64 hex。 | 未分组摘要随机出现连续 12 位数字时会触发完整账号脱敏，造成同一 fail-closed 错误的审计详情随机缺失。 | 允许测试接受 `[redacted]`；放宽完整账号脱敏；从错误中删除摘要。 | 不泄露路径/资金值，不削弱 privacy gate，并使同一摘要在 transport 后稳定可关联。 |
...
| E12-A 原字段白名单只包含 `targetSnapshot` | 在 final v3.2.3 parent 上同时严格接受并逐层传递 `targetParentIdentity`，managed Publisher 强制校验 direct parent | final FilePlan 合同已扩展；旧白名单导致 focused 首跑 7/64、57 个同源前置失败 | 不改变金额、币种、Workbook、revision/archive 或 live legacy 行为；只补齐最终 predecessor 合同 | 是（本记录与 preflight 已同步） |
...
| Final-restack focused closure | 首跑 7/64 暴露旧 FilePlan 字段集；补齐 direct-parent authority 后原 64/64 PASS。Writer/恢复组合曾因随机 identity SHA-256 含连续 12 位数字，被 `finance-safe-v1` 误判为完整账号；现将审计摘要固定编码为 `sha256:xxxxxxxx:...`，新增全数字摘要确定性用例后 E12-A 65/65 PASS，不放宽 fail-closed 或脱敏 gate。 | 最终 predecessor 的 `targetParentIdentity` 必须存在、set/order/identity exact；managed Publisher payload 强制绑定 direct-parent identity；staging replacement 的可审计摘要稳定且 finance-safe。 |
| Final-restack VCC/affected regression | 全部 `vcc-financial-op*.test.js` 549/549 PASS；此前 background/Publisher/Archive/E10-B/E11-C 影响面 664/664 PASS。 | 金额/币种/Pending/revision/archive、task lifecycle、单一 Publisher/cleanup owner、最终 v3.2.3 runtime inventory及脱敏审计证据无回归。 |
| Final-restack integration/smoke/static | VCC 集成 226/226、77/77、19/19、29/29；recovery 9/9、27/27；`npm run smoke` PASS；15 个变更 JS 的 `node --check`/ESLint 与 `git diff --check` PASS。 | 调整/跨月/破坏性状态/历史模板、恢复、全局 smoke 与静态质量门禁。`release-check`、`check-vars`、`scan:vars` 按用户要求未运行。 |
...
| Independent Review Round4 focused | E12-A 56/56 PASS；writer/recovery 19/19 PASS | committed 后 generation `EBUSY/EPERM`、task-dir `rmdir` 失败、bounded pending evidence、retry collision/owner 收口；before-worker/before-handoff replacement；cleanup scan/delete/rmdir 前 identity replacement；post-mkdir replacement+EIO/持续 EACCES 不重取 authority；post-create identity/collision owner 收口；Pending 完整 merge ranges/顺序归一；外部同名文件不触碰；Windows-compatible 正常路径零残留。 |
| Independent Review Round4 affected VCC | 扩展全 VCC 单元矩阵 742/742 PASS | amount/currency/style/Result+Pending page layout/merge/order/revision/archive、Writer/Join/Publisher 与历史回归未漂移。 |
...
| 并发/幂等 | `export-subjects` compound topology 恒为 1；subjectIndex exact set/order；FilePlan fresh target snapshot + direct-parent identity；一次 journal publication。 | 不引入 shard、第二 Writer 或自动二次发布；目标父目录 replacement 由既有 Publisher fail closed。 |
...
| 幂等/重复 | generation 仅写冻结 identity 下的 task-private exact paths；Join identity、FilePlan direct-parent identity 贯穿 wrapper 与既有 Publisher 二次校验；未清 task dir 的同 operation retry collision fail closed；Publisher 无自动重试且成功仅一次。 |
