...
| E12-A focused unit | 11/11 PASS | 两 action policy exact、production false、一个 Writer topology、全主体/exact-one golden、bounded DTO、task/run/archive A/B、size/hash/业务 TOCTOU、cancel/crash/Publisher 0/1、shutdown clean。 |
...
| 失败模式 | 模板/Writer/transport/cancel/hash/业务回读/Publisher 任一失败都删除已知 generation paths；不自动 retry Publisher。 | focused fault tests 覆盖 pre-cancel、between-subject cancel、crash partial、tamper、Publisher throw。 |
...
| 幂等/重复 | generation 仅写 task-private exact paths；碰撞 fail closed；Publisher 无自动重试且成功仅一次。 |
...
| 部分失败 | Join 前无正式目标写入；任一主体失败清理全集并 Publisher=0；正式发布仍由既有 durable journal 统一处理。 |
