...
| Worker 私有 resolver 按 resolved canonical path 判重 | 不同公开 resourceId 可解析到同一文件，重复读取会破坏单次 import 的 file identity | 只按公开 resourceId 判重；读后再去重 | `a.xlsx`/`./a.xlsx`/`sub/../a.xlsx` 允许 Worker-private identity streaming read，但在 candidate mutation、resource request、adopt 前 fail closed |
