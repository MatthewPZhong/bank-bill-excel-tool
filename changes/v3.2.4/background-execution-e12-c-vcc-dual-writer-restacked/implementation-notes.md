...
- Goal/spec：冻结 v3.2.4 Spec §7.2～§11、TechDoc §9～§14；final restack 精确 parent E12-B `e6cb3a14fb837314162ba187ce4e6a61194d6047`，合并来源 `dc2caebeda3d7b34c9d86e33c10e01bc61f73a5a`。
...
| final runtime 冲突采用 registry 并集：沿用统一 `entryBindingForPolicy`，仅给 VCC subjects 加 admitted-topology 标记，并同时保留 PreFund、Duplicate、VCC topology planner。 | 最终 v3.2.3/E11 链已增加 NewAccount、FundRecon、Duplicate 与 parser-first shutdown；旧 E12-C runtime 片段没有这些注册。 | 直接选择旧 E12-C runtime；复制第二套 entry registry。 | 不回退既有 action；VCC child count 仍只能来自 Supervisor admission。 |
...
| 证据 | 结果 | 覆盖的行为/风险 |
| --- | --- | --- |
| final parent/source 取证 | parent=`e6cb3a14...`，source=`dc2caebe...`；runtime 唯一冲突按 registry 并集解析，其余自动合并 | 父链/范围与既有 action 不回退。 |
| branch/worktree/log/reflog/path 搜索 | 无旧 E12-C 草稿 | 不依赖未审查历史实现。 |
| E12-C unit | 8/8 PASS | 1～64 planner、唯一覆盖/reducer 反例、真实 single/dual 语义等价、真实 CompoundLease dual、Publisher 0/1、child crash/cancel/duplicate terminal/非法 SafeError、首错等待 sibling。 |
| `vcc-financial-op*.test.js` matrix | 566/566 PASS | E12-A/B/C、VCC DB/query/金额币种/Pending/archive/lineage/发布全域回归。 |
| BackgroundExecution/adapter/toolbox matrix | 374/374 PASS | ResourceGovernor/Supervisor/adapter、policy、protocol、recovery 与 runtime registry。 |
...
| final-restack performance/RSS | 5-run synthetic：16 subjects median 742.67ms→341.96ms（+53.96%）；4 subjects 228.71ms→220.15ms（改善3.74%）；dual peak RSS 325.00 MiB / delta 198.98 MiB | 本机进程隔离速度满足大型≥15%且小型不回退门槛；RSS 只记录证据，不替代真实样本/Windows/人工门禁。 |
| final-restack cancellation/integration/smoke | bounded cancellation 连续 20/20 PASS；VCC/归档/恢复定向集成合计 387/387 PASS；`npm run smoke` PASS。 | terminate→exit→allSettled→Main cleanup 可达；资金归档与恢复链无回归。 |
