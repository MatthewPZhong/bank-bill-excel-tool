...
- Context：final restack 的精确 parent 为 E11-C `d53d31ebe6463489bf8b1b238ab7ef2a0a97a2b0`，合并来源为 `38c319427577671e3e3708b0dd3d4df4a3aff621`；现有 live `vccFinancialOp:export:result` 在 VCC Service `activeTask` 内由 Main 进程调用 `writeRunWorkbooks`，然后一次 `publishVccFinancialOpOutputs`。
...
| FilePlan 在 TaskLifecycle prepare 时为所有正式目标生成 artifactKey/aliasKey/targetSnapshot/targetParentIdentity。 | `src/main.js` `vccFinancialOp:export:result`；`archive-center/file-plan.js`；`task-lifecycle.js` | Main 分配 generation paths；Worker 不能重新决定文件名/目标，Join 要求 exact set/order/binding；managed Publisher 必须绑定 direct-parent identity。 |
| 现有 VCC Publisher wrapper 把 N 个 artifact 作为一个 journal publication 提交，并二次核对 size/hash/target snapshot；最终 Publisher 已支持强制 direct-parent identity。 | `vcc-financial-op-output-recovery.js::publishVccFinancialOpOutputs`；`toolbox-output-publication*` | managed path 传 `requireTargetParentIdentity=true`；Join 成功前不调用，Publisher 异常不自动二次发布；legacy caller 未提供新字段时保持旧行为。 |
...
| final v3.2.3 新增的 `targetParentIdentity` 是否可以从 E12-A canonical FilePlan 中丢弃。 | restack 合同盲区 | 高 | 容易 | focused 首跑 57 个用例在 Worker 前统一报 FilePlan set/order/snapshot 变化；最终 FilePlan 与 E11-C 都把该字段作为 Publisher authority。 | PROBE（已闭合） | 字段级 canonical diff、managed Publisher payload 断言、Publisher parent replacement 回归 | 不得丢弃；canonical/Join/wrapper exact 传递并强制 Publisher 校验。legacy live wrapper 保持兼容，production 仍 false。 |
