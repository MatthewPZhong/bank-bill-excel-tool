...
- Context：初始历史 restack base 为 `1abcae910715e3271c53ad6022d95070e8d502d3`；最终 v3.2.4 候选 predecessor 为 `b91743a5073a8f66a3df38f63fc0f58126297d11`，E11-C 传播来源为 `50408e612d6775e33eafebd89145de4807a20d3a`。E11-C 原实现来自旧 base `225ab05f77cd74d25b9aae05dda1ab490104d5c6` 上按序提交 `6e0ee98f`、`c32db41e`、`2e03bf2a`。最新 E11-A 已交付 standard/BOC Service，E11-P0/B 已交付 JPM ID-aware durable mutation、receipt-first Inspector、Recovery Hold、threshold/startup recovery 与 phase admission。冻结 v3.2.4 Spec/TechDoc 把 E11-C 限定为 ReconFix export，不包含 E12。
- Constraints：Worker 只使用 Service 返回的 exact `serviceGeneration + revision + resultHandle` 与 task-private staging；复用 legacy 命名、sheet、列、样式、watermark、row lineage；Main DTO 不携带 raw row/JPM candidate/大对象；Main Join 必须重验 generation/result/revision、scenario/linked evidence、FilePlan set/order、path containment/alias/symlink/direct-parent identity、size/hash 与业务 workbook；全部通过后单次调用既有 Publisher，并把 frozen parent identity 作为 required authority 交给 durable journal；任一失败 Publisher=0；Publisher uncertain/crash 只沿用现有 journal recovery；不改 E11-P0/B mutation/receipt/Inspector，不启用 managed production，不接 live IPC，不改依赖，不实现 E12。
...
| 最终 FilePlan 已提供 artifactKey、target snapshot、direct-parent identity、alias/symlink 与 freshness 边界 | `archive-center/file-plan.js` | Main Join 必须消费 normalized eager FilePlan，严格比对 parent identity，并交给 Publisher 持久复核；不能信 caller/Worker 自造路径身份 |
...
| 最终 v3.2.3 新增的 `targetParentIdentity` 如何进入 E11-C strict schema 与 Publisher | 文件所有权/TOCTOU 盲区 | 高 | 一般 | 首次最终链测试 3/15，12 项统一在旧 exact key set 被拒；FilePlan/Publisher 已有完整 parent identity authority | PROBE | 字段级 canonical diff；替换 direct parent 后调用 export；mock/真实 Publisher 检查 required identity | exact key set、canonical equality 与 durable target 三处同时消费；`requireTargetParentIdentity=true`；15/15 与 630/630 PASS |
