...
| 最终 v3.2.3 Worker adapter 下 close-ack 后主动关闭 `parentPort` | 最终 adapter 只对亲眼观察到 `executor:close-ack` 的 Service 提供 250 ms 自然退出窗口；Node 22 实测证明只回 ack 而不关闭 port 会回落到强制 terminate，旧 post-terminate unref 又可能令 cleanup Promise 未 settle 时 event loop 提前耗尽 | 放宽 adapter 到任意 Worker；恢复 post-terminate unref；把 pending cleanup 当成功 | ReconFix 仍先同步投递 exact close-ack，再在 `queueMicrotask` 关闭自身 port；无 ack/未退出仍走强制 terminate，ServiceHost timeout/leak/error 与业务状态机不变 |
...
| 最终 worker-exit propagation 的 Node 22 ReconFix 复验 | 最终 v3.2.3 head `afc15f77967e69c63f527e59f4bce07b66f0bdd3` 传播到 E11-A 后，官方 Node 22.18 adapter + ServiceHost + Supervisor + ReconFix 联合矩阵 158/158 PASS、0 cancelled；其中 `recon-id-fix-service.test.js` 14/14，真实 close/crash、active/adoption/terminal-in-flight shutdown 均完成 | close-ack 在 `parentPort.close()` 前可观测，正常关闭以真实 exit settle；没有通过放宽 timeout、资源回收、金额/币种或 production policy 掩盖 Node 22 cleanup 问题 |
