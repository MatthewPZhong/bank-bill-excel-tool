...
> 状态：**已实施（v3.0.1，已发版）** ｜ 来源分支：`v3.0.0` ｜ 目标版本：**3.0.1**（2026-06-09 与链接表批量导入等一起收口进 v3.0.1 迭代，见 `docs/iterations/v3.0.1/PRD-v3.0.1.md` 需求4）
> ✅ 回写 2026-06-15：原「待落源码」状态行已 stale。实证落地 `src/renderer-dialogs.js:6333`（`if (!admDerive.total) return null;`）+ CHANGELOG v3.0.1 需求4。
