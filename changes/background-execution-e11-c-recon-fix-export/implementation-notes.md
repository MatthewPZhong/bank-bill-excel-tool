...
- 最终 v3.2.4 候选 exact predecessor：`b91743a5073a8f66a3df38f63fc0f58126297d11`（最终 v3.2.3 closeout 上的 E11-B restack）；E11-C 传播来源为 `50408e612d6775e33eafebd89145de4807a20d3a`。
- 历史 restack exact base：`1abcae910715e3271c53ad6022d95070e8d502d3`（E11-B committed-result-lost review-fix head）。
...
| 最终 v3.2.3 FilePlan 的 `targetParentIdentity` 在 E11-C canonical gate 与 durable Publisher 中端到端强制 | 最终基线为每个 output 新增 direct-parent dev/inode/realpath authority；旧 E11-C 字段白名单会把全部合法新计划误判为非法，若只放宽字段又会丢失父目录替换防护 | 删除严格字段白名单；只在 Main 比对但不传 Publisher；退回 lexical alias/snapshot | Main 重算并逐字段比对 frozen parent identity；Publisher target 接收 `expectedTargetParentIdentity` 且 `requireTargetParentIdentity=true`，父目录被替换时在生成/发布前 fail closed |
...
无冻结业务合同偏离。实施中补齐两项前序缺口：JPM public bounded result 增加冻结合同要求的 generation/revision；JPM private current result 的 linked hash 改为 receipt 证明的 post-image。两项均经 E11-B 全回归，未改变 mutation/receipt/Inspector/Hold 合同。Round 2 只交付共享 primitive、runtime owner API 与 fail-closed managed export 合同；按裁决不修改 `main.js` live handler，具体 scenario/BOC/JPM writer wiring 留 E12。此次 restack 的 phase preparation、generation cleanup 与 `targetParentIdentity` 传播均是状态/资源 ownership 兼容修复，不改变 export definitive outcome、业务字段、金额/币种、artifact 集合、重试语义或冻结 Spec/TechDoc，因此无需反向修改业务文档。
...
| 2026-08-30 最终链 E11-C 专项 | 15/15 PASS | 新鲜覆盖 direct-parent replacement fail closed、parent identity 传入强制 Publisher、三类输出集合、tamper/stale/alias/cancel/recovery |
| 2026-08-30 最终链 E11-A/P0/B/C + recovery/toolbox | 180/180 PASS | Service/critical receipt/Inspector/Hold/export settlement/FilePlan/Publisher 组合状态机 |
| 2026-08-30 最终链平台与 ReconFix 广域矩阵 | 630/630 PASS | Background Execution 全套、ReconFix engine/IO/C4、Toolbox、Duplicate startup 与最终 v3.2.3 基线兼容 |
| 2026-08-30 相关 integration | 27/27 + 9/9 + 19/19 + 40/40 + 73/73 PASS | RecoveryControl、recovery canary、linked streaming、gateway upsert、linked delete/rebuild |
| 2026-08-30 应用与静态门禁 | `npm run smoke` PASS；12 个变更 JS `node --check`、ESLint、`git diff --check` PASS | legacy ReconFix/IPC/engine/Excel 与语法/style/whitespace；按用户要求未运行 `release-check`、`check-vars`、`scan:vars` |
...
- 最终 v3.2.3 基线上首次 E11-C 专项为 3/15 PASS，12 项统一失败于 `RECON_FIX_EXPORT_FILE_PLAN_INVALID`；字段级 probe 证明 `normalizeFilePlanV1` 已新增 `targetParentIdentity`，而旧 E11-C exact key set 仍是旧结构。修复为 strict parent identity 比对并传入 required Publisher 后 15/15 PASS，未删除或弱化任何 alias/snapshot 校验。
...
- 权限/所有权：Service 是 full result 单一 owner；Worker path 限于真实 task-private root 的直属固定文件；Main 重新 canonicalize FilePlan 并重验 role/sourceOperation/target snapshot/direct-parent identity、realpath、symlink、alias、key/set/order；Publisher 再以 required parent identity 持久复核，不能只靠 lexical path。
...
- Archive FilePlan `targetParentIdentity`（Important-skeleton，使用前序实现）：E11-C 现在严格消费并传给 Publisher；不修改 capture/normalize 算法，但使 ReconFix 输出正式继承 direct-parent replacement 防护。
