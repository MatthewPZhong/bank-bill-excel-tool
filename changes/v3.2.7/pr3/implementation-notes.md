...
## 2026-09-07 合计流水改为入减出

- 用户已确认：允许合计金额列按新公式变号；反推余额改为终止余额减新合计，差额、结论及差异筛选不变。第 14 列表头同步减法。原预检见 flow-net-preflight.md。
- Decisions：保留导入规则与旧输入；独立计算规则进入新指纹，manifest/结果分片标注版本，导出按封存合同区分新旧表头。旧行、旧说明、原件引用不回写；无数据库迁移。新旧结果内均维持幂等。
- 风险优先计划：先保证金额等价和新旧版本分界，再贯通 worker、Main 核验、导出 expected/actual，最后验证真实临时文件回放与重跑兼容。
- 分工：主代理负责计算/版本与真实链；独立代理负责导出分流/错配拒绝；另一代理负责独立新 oracle 和精度/冲正测试，旧 oracle 保留。
- Evidence：计算与独立 oracle 28 PASS、导出规则兼容 6 PASS、现有导出 8 PASS、真实 Main 计算和导出 26 PASS，共 68 PASS / 0 FAIL / 0 SKIP。涵盖负数冲正、零、大数/高精度、取消、归档发布及提交前后进程退出恢复。
- Evidence：另以 `1479fc93` 原代码和当前代码分别启动独立子进程，在同一新建临时 Host 回放 17 个原批准样例。生成 17 行全量、12 行差异及六份 XLSX；除第 13 列及第 14 列表头外其余业务输出保持。历史所有工作表逐格一致，旧封存文件 SHA256 不变；原输入无需重导，新计算生成版本 2，重复计算复用同一版本。可重放脚本及证据见 flow-net-validation.md。
- Deviations：无金额合同偏离；内部历史 SQLite 列键保留，语义以封存版本为准。仅新增计算/结果版本，不升级导入合同或数据库结构。
- 交付：随 PR237 更新，保留此前页面和导出调整。Remaining：本次未重新执行完整 release-check，未运行真实业务数据、Windows、Excel/WPS 或目标规模验收。用户确认的是金额口径，不能标为人工业务验收 PASS。
