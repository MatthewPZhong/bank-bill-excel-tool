...
const root='/private/tmp/toolbox-rows-rereview-_cdsv1kq';
...
generateValidateAndPublishRows,publishToolboxPublicationAsync:options=>publishToolboxPublicationAsync({...options,targets:options.targets.map((x,i)=>({...x,expectedTargetParentIdentity:frozenPlan.outputs[i].targetParentIdentity}))}),
...
const plan=prepared.filePlan;frozenPlan=plan;assertFilePlanFresh(plan);
