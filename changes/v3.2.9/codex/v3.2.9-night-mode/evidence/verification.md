...
## 本地 release 三模块组合复验（2026-09-12）

源分支 74 个文件已核对并提交为 `654ba97a24223d671d61b0737974d1b6942ceb16`，在两模块 release `e1c31139dcb7eeca3edbc12dd293e4419b56950f` 上继续集成。当前结果以 [release 记录](../../../release.md)和 [组合候选摘要](../integration-candidate-20260912.json)为准。

组合后的 GUI 15 组/30 主题/2,580 断言通过；另补设置交错保存 4/4 场景、73 断言和原 6 组矩阵，Biz OP 双主题交互 20/20、状态/布局 14/14，均通过。所有源 GUI 图片及 JSON 保留为历史记录，本轮证据位于独立本地日志目录。

完整 `release-check` 于 14:55:48 +08:00 返回 exit 1：lint、smoke 和单测 7,381 PASS / 0 FAIL / 3 SKIP；57 个集成脚本中 56 个通过，工具箱多工作表拆分 RSS 门禁失败。当前候选不能记录为完整门禁通过。该扫描链与合并前字节一致；原门槛独立复跑的结果另记，不能改写首次整轮失败。真实 Main 冷启动仍沿用源分支 `BLOCKED` 记录，Windows/物理显示器等验收未完成。

以下内容保留源分支实现、收尾和排查当时的证据，原完整门禁统计及 GUI 摘要不代表三模块集成后的完整门禁结果。
