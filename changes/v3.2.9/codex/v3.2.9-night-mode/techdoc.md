...
2026-09-19 Windows 复审确认：`en-US` 原生 `h:mm a` 时间框在 104px 宽度下裁切 AM/PM，并侵入时钟按钮；只将内存宽度改为 132px 后完整，恢复 104px 后再次失败，输入值始终未变。因此正式与离线预览均改为 132px，保留原生小时制、8px 内边距和 44px 高度。`verify-dark-mode-live.js` 读取 UA shadow DOM 字段及文字范围，严格检查生产宽度、窄窗口和恢复后的可见性；104px 历史对照仅用于复现。截图及逐字段证据见本版 [发布前自审](../../release-review-2026-09-19.md)，实际平台复测状态以该记录为准。
