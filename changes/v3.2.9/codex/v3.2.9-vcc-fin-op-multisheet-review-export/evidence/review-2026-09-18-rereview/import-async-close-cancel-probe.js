...
let capture = null;
const richModule = require(path.join(reviewRoot, 'src/backend/xlsx-rich-reader'));
const originalOpen = richModule.openRichWorkbook;
richModule.openRichWorkbook = async (...args) => {
 const reader = await originalOpen(...args);
 return {...reader, async close() {
   capture?.event('close-start');
   await reader.close();
   capture?.event('close-complete');
 }};
};
...
test('real async reader close receives cancellation before business BEGIN',async t=>{
 const f=fixture(t);const file=write(f.dir,[{name:'系统',type:T.SYSTEM_OP,rows:system('甲')}]);
 const request=await prepare(f,[file]);let cancelled=false;const events=[];
 const event=name=>events.push({event:name,cancelled,snapshots:f.db.prepare('SELECT COUNT(*) n FROM vcc_fin_op_system_snapshots').get().n});
 capture={event}; const exec=f.db.exec.bind(f.db);
 f.db.exec=(sql)=>{if(sql==='BEGIN IMMEDIATE')event('BEGIN IMMEDIATE'); return exec(sql)};
 request.shouldCancel=()=>cancelled;
 request.onProgress=p=>{if(p.phase==='reading'&&p.sourceType===T.SYSTEM_OP){event('last-reading'); setImmediate(()=>{cancelled=true;event('cancel-delivered')})}};
 let result,error;try{result=await importFiles(request)}catch(e){error={code:e.code,message:e.message}}finally{capture=null}
 event('complete');console.log(JSON.stringify({events,result,error}));
 assert.ok(events.findIndex(e=>e.event==='cancel-delivered')>events.findIndex(e=>e.event==='close-start'));
 assert.ok(events.findIndex(e=>e.event==='cancel-delivered')<events.findIndex(e=>e.event==='close-complete'));
 assert.equal(f.db.prepare('SELECT COUNT(*) n FROM vcc_fin_op_system_snapshots').get().n,0,
  'real event loop delivered cancellation during async reader.close before transaction');
});
