...
Windows NTFS 的 dev/ino 可能超过 Number 安全整数范围。删除、只读副本、迁移和来源快照的身份读取必须使用单次 `{ bigint: true }` stat/lstat/fstat，持久化为无损十进制字符串；不得先转 Number 再生成字符串。兼容视图仅将 size/mode/nlink 等原元数据转回 Number，并从同一次返回的纳秒时间重建小数毫秒，保持已有严格时间比较。旧安全整数身份继续兼容；已舍入 Number、缺少可靠身份或旧不完整证据继续拒绝授权，不从当前文件补造历史身份，不放宽 owner、摘要、父链和替换检查。
...
目标已缺失而需重建时，重新确认缺失并采用排他发布，不先删除路径或以 rename 覆盖现场文件。同卷使用本次已同步落盘的 staging inode 排他创建目标；文件系统不支持 hardlink 时通过 `wx` 独占创建目标句柄，并通过同一原句柄写入、同步和设置模式。关闭原写句柄前，将只读见证句柄与原句柄身份、目标路径及原根／父链绑定；关闭后从见证句柄采集最终身份，再与目标路径核对，供首次 journal 登记使用，不能关闭后从现存路径认领对象。

Windows 可将本次 link/unlink 或 chmod 的 ctime 更新延迟到写句柄关闭。仅在本次 link 成功、链接数精确增加且 staging unlink 后精确恢复，或实际执行 chmod 时，允许该关闭边界的 ctime 变化；mode 变化仅限实际 chmod 且必须等于目标模式。dev/ino、size、mtime、birthtime 和 nlink 继续严格核对；未 link/chmod 的 `wx` 目标仍严格比较 ctime。见证句柄与路径的最终核对、首次登记及后续恢复均使用完整身份。检查、复制或首次登记前出现的替代对象保留，迁移不切换设置。storage-materializer 的可选发布回调仅由迁移使用，默认调用维持既有发布行为。
