...
本文件是待执行清单，**没有任何人工或 Windows 性能项目被预先判为通过**。候选包和 PR 尚未生成，链接待补。自动检查、性能采样、安装包操作及 Excel/WPS 人工核对分别记录；源码脚本通过或安装包成功生成均不代替人工验收。本文件不修改各模块的需求、资源门槛或发布合同。
