...
| legacy parent 与 `__FILENAME_MAPPING__` 会让同一 session/batch 中不同 source 命中不同 template | Reviewer Round1 对照 `src/main.js` 的 parent provisional entries、filename mapping 与 `matchedTemplateId` 路径 | session owner identity 必须与每 source template evidence 分离；不能用单一 template snapshot 解释整批 |
| 相同来源可能经 hardlink、symlink/目录别名、大小写、同内容副本或跨 session 再次出现 | Reviewer Round1 资金红线；旧实现只按 `path.resolve`，且重复文件会造成 row count 双计 | E09-A 无重复交互能力，必须在 candidate/resource adoption 前全局 fail closed，保留旧 revision/session |
| 五个 dormant Statement action 共用同一 Service entry | canonical policy fixture 与 `runtime-bindings.js` | E09-A entry 必须在解析 payload、resolve/read source、申请 resource 前 exact 限定 `statement:import`；其余 action 返回 bounded unsupported error，不升级成 Host protocol failure |
...
| E09-A 是否需要接管全部 legacy parent/filename/big-account import 分支 | 范围/主流程 | 高 | 一般 | frozen sequence 将 token/interaction 放 E09-B；Reviewer Round1 证明 parent/filename 本身可在无交互时合法多模板 | PROBE → CLOSED | parent+child 与 filename mapping 多模板真实 workbook golden | non-interactive parent/filename import 用独立 session owner + bounded template catalog + per-source ref；需要大账号的候选仍 blocked，不发明交互 |
...
| 同一来源的去重身份能否只靠 normalized path | 主键/幂等/资金 | 高 | 一般 | hardlink、symlink、Windows case-fold 与同内容副本均可绕过单一 lexical path | PROBE → CLOSED | batch/跨session真实 hardlink、file/dir symlink、same-content、same-basename、case-variant 与 replacement 反例 | Worker 私有 identity 组合 realpath hash、dev/ino、case-fold basename hash、streaming content digest；任一维度重复均在 resource request 前拒绝 |
| 共用 Service entry 收到 future action 时何时拒绝 | 入口/状态机 | 高 | 容易 | 若先解析 payload，可返回错误 action 的 contract error；若先 resolve/read，可产生越界 I/O | PROBE → CLOSED | 已有 session 下四 action × import/status/extra/deleted-source payload，捕获 exact event order | `start.actionKey !== 'statement:import'` 在 payload parser 前拒绝；只发 `job:start → job:error(STATEMENT_ACTION_UNSUPPORTED)` |
...
3. 已用真实 `createExecutionSupervisor`、`createServiceHost`、`createResourceGovernor` 与 native Worker entry 执行两次 import、reject/adopt failure、cancel/crash/status。
4. 已捕获所有 Job/Service Control envelope 与 public result，递归证明无 `detailRows`、prepared rows、private context；status 不公开路径。
5. 已以真实 workbook 关闭 parent+child 与 filename mapping 的 per-source template/matchedTemplateId/order/金额币种 golden。
6. 已以真实 filesystem 关闭 batch/跨session hardlink、file/dir symlink、case-fold basename、same-content、same-basename、replacement 与独立合法来源反例；全部 duplicate 在 resource request/adopt 前失败且旧 state 不变。
7. 已证明四个 future action 在解析 malformed/extra payload 与 resolve 已删除 source 前 exact unsupported；无 resource request/adopt 或 Service fatal。
8. 计划复跑 E09-P0 legacy golden，证明四金额、币种与 current/all characterization 未漂移。
...
- 同名（含 Windows 大小写等价）source 在 E09-A 一律按 legacy duplicate 语义 fail closed；真正的覆盖/取消交互留给 E09-B。不同 basename、不同 inode、不同 content 的合法 source 继续允许追加。
- streaming digest 在 identity adoption 前与 mapper 读取后各执行一次，以换取“去重证据 + 读取期间未替换”的闭环；单批最多 1024 source 且受 1 MiB request、256 MiB retained-state ceiling约束，真实大文件吞吐仍保留为人工/发布 PROBE。

## 风险优先计划

| 顺序 | 步骤 | 消除的未知/保护的不变量 | 成功证据 | 失败影响 | 回滚/收缩 |
| --- | --- | --- | --- | --- | --- |
| 1 | 冻结 import request/result/status 与 session owner/template catalog/source ref validator | Main/协议无业务行和private context；每 source evidence 可重算且匹配自己的 config | strict正反 tests、parent/filename per-source golden、byte/privacy tests | 公共边界不可信则停止 runtime | 只保留 preflight，不接 entry |
...
## Reviewer Round1 Triage

- 接受并关闭 P1：原单一 `templateEvidence` 与 `sessionKey === templateId` 不能表达 parent/child 和 `__FILENAME_MAPPING__` 的 per-source template lineage；改为 exact `sessionOwner + templateCatalog + sources[].templateRef`。
- 接受并关闭 P1 资金红线：原 canonical lexical path 不能阻断 hardlink、symlink、case-fold、same-content 与跨 session 重复；改为 Worker-private layered identity，且 E09-A 在任何 resource request/adopt 前 fail closed。
- 接受并关闭 P2：共用 Statement Service entry 原先会先解析 input；改为 `actionKey` 早门禁，四个 future action 无论 payload shape 均 bounded unsupported。
- 没有新增用户选择型 BLOCK。重复来源的覆盖/取消选择仍明确留在 E09-B；本轮只实现 E09-A fail-closed 路径。
