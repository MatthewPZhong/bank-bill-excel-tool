...
| 3 | private terminal outcome coordinator + Worker reservation。 | 先占Service、后lease/parse；两侧clean-exit success才Service；任何failure marker/teardown都必须在全部Parser exit后才允许parent释放；shutdown-only不被普通cancel旁路。 | completion乱序、busy冲突、normal/EACCES outcome、两类真实Worker terminal barrier、app shutdown、manifest后crash零adopt/零DB。 | reservation旁路、未记账Worker或部分采用。 | paired gate关闭，direct single保留。 |
