...
| 3 | private terminal outcome coordinator + Worker reservation。 | 先占Service、后lease/parse；两侧clean-exit success才Service；shutdown-only不被普通cancel旁路。 | completion乱序、busy冲突、success/failure outcome、app shutdown、manifest后crash零adopt/零DB。 | reservation旁路或部分采用。 | paired gate关闭，direct single保留。 |
