...
| 证据 | 结果 | 覆盖的行为/风险 |
| --- | --- | --- |
| E08-A focused unit | `13/13 PASS` | additive旧库、dataset/金额币种血缘、import事务与Inspector、run replay、四态、old/absent/concurrent mirror、staging边界、全程operation lock、Worker critical/receipt/取消/跨operation拒绝 |
| 相关BankBU/receipt unit | `71/71 PASS`（含上述13条focused） | month/run repository、side store、legacy dual-source、E06 receipt事务与identity回归 |
| 真实临时SQLite/XLSX集成 | `16/16 PASS` | 两reader、side commit、真实子进程import critical前kill、run side commit后kill、重启Inspector、CAS complete-mirror、single/aggregate三sheet、included/skipped与stale mirror |
...
| 行数守恒 | import实际insert count写dataset；run totals来自同一side dataset；parity/smoke覆盖四种基数及N:M行处置 |
...
| 输出可审计性 | single/aggregate保留dual-source、月份升序、included/skipped、异常sheet与Main runId；stale managed mirror不得对新dataset重算 |
