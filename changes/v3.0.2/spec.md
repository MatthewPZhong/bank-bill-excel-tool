...
> 状态：**已实施（v3.0.2 已提 PR #69，release-check 全绿）**（4 项关键决策已确认，见下表）｜ 来源分支：`v3.0.1` → `v3.0.2` 开发分支 ｜ 目标版本：**3.0.2**
...
> - 产品需求：`docs/iterations/v3.0.2/PRD-v3.0.2.md`（21 条 AC）
...
- **需求3**（15 条）：改名 + idEnabled 开关默认勾选；取消勾选灰显 + Reference 取原值不报必填；fieldValue 开启校验；🔴（用户修订）字段取值**限定「网关1v1渠道」** —— 1v1 赋值正确，勾 1v多/多v1 时 UI 禁用+灰显+提示+自动取消、校验拦截、引擎入口 gate 强制 `fieldValue.enabled=false`（`apply1vN`/`applyNv1` 不赋值，双重防御）；🔴（用户修订）两功能区垂直布局（标题行含开关在上、内容在下）；🔴 不污染原始行；🔴 seq 字符串经 `Number()` 归一命中；目标列超 14 列不体现不报错；两开关独立 + 分组过滤；旧场景兼容；`release-check` 全绿。
...
| 日期 | 变更内容 |
|------|---------|
| 2026-06-10 | 初稿：依据已批准实施计划收口 3 项需求为 v3.0.2 变更入口；4 项关键决策已确认；指向 PRD（21 AC）+ TechDoc |
| 2026-06-10 | reverse-sync（PR #69 review P3）：状态改「已实施」；需求3 用户修订「限定网关1v1渠道 + 垂直布局」，AC 18→21、需求3 12→15；§三验收标准「1v1/1v多/多v1 赋值正确」改为「限定 1v1，1v多/多v1 不生效」 |
