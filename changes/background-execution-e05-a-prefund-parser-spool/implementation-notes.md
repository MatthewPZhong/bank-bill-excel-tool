...
| Worker error 始终只有 `name/code/message`。 | 仅当受信 cleanup error 且明确存在 residual scope 时，追加 `cleanupRequired=true`、固定 `cleanupScope` 和 bounded `causeCode`；普通业务错误仍保持原三字段。 | 原投影会丢失“当前 file spool 仍需 task owner cleanup”和原 parser cause，Windows locked-file 也无法安全闭环。 | 条件错误 shape 增量扩展；不含 residual path、source path 或 filename，不决定 launcher/product outcome。Reviewer 已要求并接受该修复。 |
...
| P1-4 Worker cleanup error 丢失残留处置与原 cause。 | writer 的 cleanup error 保留 `cause` 与 `details.residualPaths`，但原 `safeError` 只投影 `name/code/message`。真实 declared-count mismatch + `foreign.keep` 会留下 fileDir，却无法让未来 task owner 判定 cleanup。 | `safeError` 只对白名单 cleanup code 且非空 residual scope 添加固定、无路径信号；`causeCode` 仅接受 1..128 位大写 code，否则稳定为 `null`。 | 真实 Worker cleanup-incomplete 回包包含 expected cause/scope、JSON 不含 temp/source/file/foreign 名称且残留存在；fixture 随后显式清理。普通 declared-count mismatch 仍只有原三字段且不误标。 |
...
| Parser/Core golden + existing parser suite | `64/64 PASS` after P1 fixes, including 100k-row gzip streaming, long IDs/source sequence, exact decimal text, date, fingerprint, content hash, strict/skip issues, OUTBOUND complete-pair fallback, header-only compatibility/fault cases and real Worker cleanup signalling. | Parser and funds-field semantics remain on the current implementation. |
...
| 命令 | 结果 |
| --- | --- |
| `node --test tests/unit/main-process/pre-fund-reconciliation/mpt-import-e05-a.test.js tests/unit/main-process/pre-fund-reconciliation/mpt-parser.test.js` | `64/64 PASS` |
...
| Worker cleanup observability and privacy | Only trusted cleanup codes with non-empty residual scope carry a fixed cleanup signal. Real Worker tests prove original cause survives, ordinary parse errors are unmarked, and no residual/source path or filename crosses the thread boundary. | Closed for E05-A; launcher reaction remains intentionally unwired. |
