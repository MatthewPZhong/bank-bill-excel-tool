...
> ⚠️ 实施硬前提：O-2 值口径逐格 diff 验证通过后方可落流式落库。
...
| # | 风险 | 说明 | 缓解 |
|---|------|------|------|
| R-1 | **值口径一致性**（最高） | 流式 `parseCellBody` 解析的单元格值必须与 SheetJS 现状逐格一致：日期（序列号 vs 字符串）、数字精度/科学计数/前导零、空格、合并单元格。实测样本看似一致，但需系统比对 | 用一个 SheetJS 能读的中等链接表，**双跑逐格 diff**，纳入单测 |
...
| O-2 | 值口径一致性验证策略 | **中等文件 SheetJS↔流式 逐格 diff 验证**，入单测。🔴 **实施硬前提**——diff 未过不得落流式落库。 |
...
| O-5 | detector 多 sheet vs 流式引擎单 sheet | **确认链接表单 sheet 约定**；若非单 sheet 则**补多 sheet 支持**（`readXlsxStreamed` 现硬编码只读 sheet1.xml）。 |
...
3. [ ] R-1/O-2 值口径双跑 diff 验证（🔴 实施的硬前提，diff 未过不落流式落库）
...
## 附：实测脚本（一次性，已删）
用 `readXlsxStreamed(file, onRow, {colCount:44})` 读用户文件：657,758 行 / 12.4s / RSS 385MB / 堆 19MB。表头 `账户主体,账户BU,BizId,BillDate,ValueDate,Channel,地区,MerchantId,Currency,Credit Amount,Debit Amount,ReconciliationId...`。
