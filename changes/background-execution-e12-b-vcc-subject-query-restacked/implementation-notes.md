...
- Final restack parent：E12-A `2aec4a18df8467db1a7e88ce9b319e79f413952b`；合并来源 `9c66a7f52b262c5746456667af34abb3c6c1f588`，自动合并仅引入 E12-B 的 11 个预期文件，无文本冲突。
...
| E12-A affected regression | 65/65 PASS | single Writer、FilePlan/Main Join、raw OOXML、staging identity、finance-safe digest、cancel、Publisher 0/1、committed cleanup/retry。 |
| `vcc-financial-op*.test.js` matrix | 558/558 PASS | schema/migration、金额币种/adjustment/revision/archive、writer、storage/recovery与legacy UI/contract。 |
...
| directed read/RSS benchmark | 非目标 adjustments 1→20,000 时目标readCounts不变；VM steps 19→19、fullscan 0→0；median 0.226→0.212ms；RSS delta 2.16→1.84MiB；maxRSS 103.47→100.27MiB；三类query均SEARCH subject index。 | 反证hidden full-run adjustment scan；确认query/RSS scale按目标主体。 |
| final-restack directed integration | VCC adjustment/archive 226/226、destructive 77/77、effective 19/19、historical export 29/29、recovery canary 9/9、control 27/27，合计 387/387 PASS；来源叠栈曾全量 51/51 scripts、2455/2455 assertions PASS。 | 最终父链上的跨模块迁移、归档、历史模板、恢复与输出链；来源全量证据只作为补充，不替代本次定向回放。 |
| smoke/static | 最终父链 `npm run smoke` PASS；changed JS `node --check`、ESLint 与 `git diff --check` PASS。 | 仓库smoke、源码语法/风格与diff卫生；`release-check`、`check-vars`、`scan:vars` 按用户要求未运行。 |
