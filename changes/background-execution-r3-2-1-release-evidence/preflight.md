...
| 6 | PR #183 一次性 repair synchronize final gate。 | 原 attempt #3取消且旧 head 未包含 #182 isolation；waiver 不得扩散到后续 push。 | PR #183 / same-repo / R4→R3 / synchronize / attempt 1 / commits 4；exact checkout 后验证旧 head ancestor及新增提交数 2。 | 任一 tuple 或 lineage 漂移必须在 release-check 前失败；远端失败/取消不允许重跑。 | 只撤回本次 waiver；保持 hard gate open、native production disabled、main/tag未授权。 |
