...
(async () => {
  for (const item of [
    ...
    { name: 'subject-CNH', subject: 'CNH', expected: 'success' },
    { name: 'formatted-system-subject', subject: '000123', expected: 'success', beforeImport(filePath) {
    ...
    try {
      ...
      assert.equal(response.subjectCount, 1);
      assert.equal(response.sheetCount, 8);
      assert.equal(response.sourceRowCount, 7);
