...
PR #229 方向修复后的完整 `npm run release-check` **退出码 0 / PASS**（2026-09-05，macOS + Node 24.13.0），提交中的验证结论及本地日志位置见 [验证记录](verification.md)。

| 检查 | 最终结果 |
| --- | --- |
| lint | PASS |
| smoke | PASS，含 C2、仓储迁移、场景派发及文件读写链 |
| unit | 442 个文件，6936 PASS / 3 SKIP / 0 FAIL，共 6939 项；3 项为既有 Windows 专用探针，本机未运行 |
...
| scan:vars | PASS；版本调整前、3.2.6 收尾及方向修复后均已执行，刷新两份统计报告 |
