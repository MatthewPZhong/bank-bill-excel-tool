...
- 2026-08-30 最终版本链重叠基线：v3.2.3 元数据候选 `ab9bb149b397ba4f0ea9f272c9177eb06a473feb` → E11-A `7d9c73ecee94b5a1ac9dd831a53f22f30c77fb23` → E11-P0 `7d9de840b2a5ae2feadf9434070962419972e9ef`；本地 E11-B 合并来源为 `1afaf5db8f7a9a406ef037a685586456d106b12c`。这些 hash 是隔离 worktree 的审查候选，远端推送前仍须按 PR exact head 复核。
...
- 最终 v3.2.3 重叠时，prepared transition reader 已统一返回 `{ transition, event }`；prepared Hold 校验改从 persisted `event.safePayload` 取 exact payload。首轮因仍读取旧顶层字段而使 9 个 crash-resume case 同根失败，修正 API 适配后 E11-B 47/47 与平台组合回归全部通过；未改变 Hold/Task/receipt 决策。
- E11-B 的 atomic `INSPECTOR_UNAVAILABLE` threshold bundle 与 v3.2.3 通用 prepared Hold 恢复合并：anchor、Task、Hold 仍以 exact owner 收口；通用 `DURABILITY_BARRIER_UNAVAILABLE` 等 prepared Hold 保留原 reason；action-specific prepared `RESULT_LOST` 继续由 definitive inspection plan 原样恢复，禁止在 Inspector 前降级为通用 unknown。
...
- 2026-08-30 最终 v3.2.3 基线重叠验证：E11-B 47/47 PASS；E11-B/E11-P0/E11-A/RecoveryContract/RecoveryControl/PreFund E05-B 聚焦矩阵 193/193 PASS；Background Execution 全套 + ReconFix E11-A/P0/B、engine/IO/C4、Toolbox、Duplicate startup 组合回归 608/608 PASS。
- 同一精确候选的定向 integration：RecoveryControl 27/27、recovery canary 9/9、linked streaming 19/19、gateway upsert 40/40、linked delete/rebuild 73/73 PASS；`npm run smoke` PASS。
- 冲突解决后的全部变更 JS `node --check`、定向 ESLint 与 `git diff --check` PASS；明确未运行用户禁止的 `release-check`、`check-vars`、`scan:vars`。
