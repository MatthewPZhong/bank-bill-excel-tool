...
| **T4** | 文本：B1 月度Pending 去空格 / B2 / B3 / getCategoryLabel(builtin-fixed) | 前端 | — | ⬜ 待办 |
| **T5** | A1 镜像对调（bankStatementModulePanel 加 layout-mirrored + 验证 rtl 交互） | 前端 | — | ⬜ 待办 |
| **T6** | A2 ReconID 去渠道下拉 + D-2/D-4 builtin-fixed 列表渲染（置顶/优先级0/仅管理/启用/通用渠道可见） | 前端 | T2 | ⬜ 待办 |
| **T7** | D-3 写死场景「管理」弹窗（抽公共多选下拉 + 多选适用渠道 + 保存） | 前端 | T2 | ⬜ 待办 |
| **T8** | C 复制场景（C1-C4 header 按钮 + 复制弹窗 + 灌 config） | 前端 | T2 | ⬜ 待办 |
| **T9** | D-1 移除 extract-recon-id 新建选项 | 前端 | — | ⬜ 待办 |
| **T10** | 收尾：preview 回归 + 新弹窗 preview 入口 + 文档三件套 + 版本 bump + scan:vars/check-vars + release-check | 收尾 | T1-T9 | ⬜ 待办 |
