...
| `npm run release-check`（最终且仅一次） | PASS：lint；smoke；unit 5918/5919（0 fail、1 skip）；integration 51/51 scripts、2455/2455 assertions | 全仓回归、Main/C1 兼容、C2 canary 9/9、recovery-control integration 27/27 |
...
| 未知 | 处理 | 负责人/下一步 | 合并影响 |
| --- | --- | --- | --- |
| Windows packaged directory-fsync capability | PROBE | Dev：实现显式 probe；非 Windows 注入测试；真实 packaged 证据保持 PENDING | target-post-image 资金 action 不得 production enable |
...
- **PROBE**：Windows packaged directory fsync 能力尚无真实 evidence；明确 unsupported 返回 `durability-unavailable`，其他错误抛出，target-post-image 资金 action 保持 production=false。
