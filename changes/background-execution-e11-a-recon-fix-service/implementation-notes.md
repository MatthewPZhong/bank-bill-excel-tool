...
| `node --test tests/unit/main-process/recon-id-fix-service.test.js` | 9/9 PASS | canonical policy、standard/BOC golden、unsafe numeric 长单号/大金额不改型且 hash 类型可区分、busy/stale/revision、scenario/BOC evidence 失效、JPM 阻断、有界 DTO、真实 Worker close/crash/generation/lease 回收、active import/run shutdown cancelled outcome、唯一 `cancel:ack` + `job:error`、无 late message/leak |
...
- 取消/竞态：active import/run 均通过 Worker 安全点 cooperative terminal；已发出的 resource exchange 先等 grant/reject/adopt-ack 收口，再发唯一 cancellation terminal，不生成协议外撤销。
