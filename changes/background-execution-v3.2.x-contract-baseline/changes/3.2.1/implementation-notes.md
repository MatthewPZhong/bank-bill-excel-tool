...
- Done when: format/content and result/error semantics remain equivalent; Publisher is called once on success and zero times on generation/artifact validation failure; shutdown is re-entrant and releases the single runtime; event-loop evidence shows generation does not run on Main.
...
- Lifecycle/idempotency: one lazy runtime owns one Supervisor/Governor, production-disabled handlers do not construct it, concurrent shutdown shares one promise, quit cancellation reports no leaked transport, and failed restart explicitly resumes the owner.
