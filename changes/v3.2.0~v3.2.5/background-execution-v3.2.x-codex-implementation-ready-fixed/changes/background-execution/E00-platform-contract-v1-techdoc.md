...
```javascript
{
  root: { workerThreadSlots: 1, memoryBytes: 128 << 20 },
  childrenMax: 4,
  child: { cpuSlots: 1, workerThreadSlots: 1, ioHeavySlots: 1,
           memoryBytes: 256 << 20 }
}
```
...
type TaskRunTransitionV1 =
  | { entityKind: 'task-run'; command: 'mark-interrupted'; taskRunId: string; expectedState: 'prepared' | 'running' }
  | { entityKind: 'task-run'; command: 'begin-recovery'; taskRunId: string; expectedState: 'interrupted'; recoveryAttemptId: string }
  | { entityKind: 'task-run'; command: 'complete-recovery-success'; taskRunId: string; expectedState: 'running'; recoveryAttemptId: string }
  | { entityKind: 'task-run'; command: 'complete-recovery-failure'; taskRunId: string; expectedState: 'running'; recoveryAttemptId: string }
  | { entityKind: 'task-run'; command: 'interrupt-recovery'; taskRunId: string; expectedState: 'running'; recoveryAttemptId: string };
...
type CriticalIntentTransitionV1 =
  ...
  | { entityKind: 'critical-intent'; command: 'mark-acked'; intentId: string; expectedState: 'prepared'; patch?: object }
  | { entityKind: 'critical-intent'; command: 'mark-committed'; intentId: string; expectedState: 'acked'; receiptRef: object }
  | { entityKind: 'critical-intent'; command: 'mark-recovered'; intentId: string; expectedState: 'prepared' | 'acked'; inspection: object }
  | { entityKind: 'critical-intent'; command: 'close'; intentId: string; expectedState: 'committed' | 'recovered'; result: object };

type RecoveryHoldTransitionV1 =
  | { entityKind: 'recovery-hold'; command: 'create-or-get'; input: RecoveryHoldCreateInput }
  | { entityKind: 'recovery-hold'; command: 'resolve'; holdId: string; expectedState: 'active'; resolution: Resolution; evidence: object };
```
...
`TaskRunTransitionV1` 故意只包含中断与恢复相关边：`prepared/running → interrupted`、`interrupted → running(recovery)`、`running(recovery) → succeeded/failed/interrupted`。常规 `prepared → running` 及非恢复执行的 `running → succeeded/failed/cancelled` 保持既有 TaskLifecycle/ArchiveRepository 所有权，不得为了满足 recovery event 合同改由本 Repository 重写。
...
- Registry 在 startup freeze，缺少静态引用时 action 不可生产启用。
...
→ committed:
     ...
     complete → close source / Task recovery success
     incomplete → hold / Task interrupted
→ not-committed:
     close intent/source / Task recovery failure or cancellation by context
→ partially-committed:
...
```text
open Main-owned control DB
migrate platform schema
register inspectors and SettlementRecoveryProviders
...
then initialize business services
```
