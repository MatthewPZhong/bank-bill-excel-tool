...
最新机器门禁：`21/21 PASS`；覆盖 51 个 action policy、9 个协议消息、完整 Service resource lifecycle sequence、5 类 RecoverySource、58 行 action 表及其与 Registry 的 287 个静态字段对照。`codex-input-contract-drift` 核对 Protocol 摘要、seq scope、Renderer 状态和四份文档的恢复审计原子性；新增 `recovery-control-transaction-contract-drift` 结构化校验事务入口、作用域 writer、四类 observation event、TaskRun/Batch/Intent/Hold command、command→event 映射、审计血缘、Critical Intent 邻接与 10 个 mutation negative self-tests。报告哈希覆盖 58 个实际规范/fixture/validator/evidence 输入，详见 `validation-report.json`。
