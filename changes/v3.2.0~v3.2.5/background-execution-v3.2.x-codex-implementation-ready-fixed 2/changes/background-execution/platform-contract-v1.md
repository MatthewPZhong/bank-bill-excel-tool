...
每个范围严格递增。重复、回退、非法跳变或旧 generation 消息均视为协议错误或 stale event。

### 4.8 Context

- `operation` context MUST 使用现有 exact-5 validator；
- `file-batch` context MUST 使用现有 exact-7 validator；
- `none` 仅限平台 canary/内部 maintenance；
- `operationKey`、jobId、unitId、grant、reservation、critical intent 不得夹带进 context；
- Service Control Envelope 不携带业务 context，业务关联只使用 `jobRef`。

### 4.9 Payload 与隐私

每个 policy MUST 声明 command/event 最大字节数、错误条目上限、artifact 数量和 progress rate。
