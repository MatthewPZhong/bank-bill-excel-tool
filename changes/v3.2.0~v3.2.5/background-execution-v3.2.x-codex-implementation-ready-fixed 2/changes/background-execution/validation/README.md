...
3. `full-policy-registry-schema`：51 个 action 的完整 Registry fixture 通过 Schema；
...
7. `recovery-source-valid-fixtures`：五类 RecoverySourceV1 正例通过 Schema 与 bounded evidence 门禁；
8. `recovery-source-invalid-fixtures-rejected`：legacy 字段、manual source 等 RecoverySource 反例必须拒绝；
9. `protocol-resource-lifecycle-continuity`：`request → grant/reject → adopted → adopt-ack → release → release-ack` 的身份、seq 与状态连续；
10. `cross-document-recovery-contract`：Intent/receipt/Inspector/Provider/Startup/TaskRun/Batch/Statement 等恢复合同跨文档一致；
11. `codex-input-contract-drift`：Codex Protocol 摘要、seq scope、Renderer 状态、Critical Intent 映射，以及四份规范文档中的恢复审计原子性一致；
12. `recovery-control-transaction-contract-drift`：结构化解析 E00 的 Repository/Transaction interface、observation event union、四类 transition command union、command→event map、审计血缘和 Critical Intent 前置状态；交叉核对四份事务合同、Lifecycle 事件表，并运行 10 个 mutation negative self-tests；
13. `negative-fixtures-rejected`：Policy、Protocol 与 sequence 的全部负例必须 fail closed；
14. `version-action-table-canonical-values`：v3.2.0～v3.2.5 action 表只使用 canonical enum；
15. `version-action-table-registry-alignment`：action 表的 disposition/mode/lifetime/adapter/commit 与 Registry 逐字段一致；
16. `document-contract-paths`：本地 Spec/TechDoc/Schema 链接都能解析；
17. `service-main-governor-boundary`：Worker 文档不得直接调用 Main-owned ResourceGovernor；
18. `required-baseline-files`：必需 Spec、TechDoc、Schema、fixture、runner 和 Codex 输入完整；
19. `package-hygiene`：发布包不得包含 `__pycache__` 或 `.pyc`；
20. `normative-text-invariants`：Service、resource handshake、Critical Intent、生命周期等最高层术语存在且一致；
21. `validation-input-hash-coverage`：校验器实际读取范围内的 Markdown/JSON/Python/Shell/requirements 输入全部进入报告 SHA-256 证据链。
