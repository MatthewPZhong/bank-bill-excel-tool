...
每个作用域严格递增。重复、回退、旧 generation、错 direction、终态后消息和重放消息均 fail closed；late terminal 只记诊断，不再次 settle。`controlId`、`requestId`、`grantId`、`reservationId` 是关联身份，不进入 seq scope。
