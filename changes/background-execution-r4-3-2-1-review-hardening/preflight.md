...
- Constraints：保持金额、币种、source identity、sequence、receipt、Recovery Hold 与 production policy 不变；5 个 native action 继续 `production.enabled=false`；不运行 `release-check`、`check-vars` 或 `scan:vars`；初始本地修复不得 push、rerun、merge、改 #182 base/head 或启用 production。2026-08-26 release owner 后续明确授权把 R4 作为新 final PR 推送并获得一次新的 automatic CI；仍不授权 merge/main/tag/production。
...
| R4 是否获准成为新的最后一个 PR 并取得一次新的 automatic release-check。 | 已决授权 | 高 | 一般 | release owner 明确回复“推”；远端只读核对显示 #182 仍为旧 exact HEAD 且旧 CI 运行中。 | RESOLVED | 将授权迁移为 `R4 -> R3` 的 same-repo `pull_request/opened`、`run_attempt == 1`；旧 R3 进入拒绝集合。 | 允许 push/开新 final PR并产生一次新 CI；旧 #182 结果不得替代 R4，仍禁止 rerun/merge/production。 |
...
| 4 | 取得明确授权后迁移 exact final tuple，推送 R4 并只在 PR opened 首轮触发新 CI。 | 保留 #182 exact evidence；新 head独立验证；旧结果不替代。 | R4 PR exact base/head/state 与 automatic run。 | 错误 tuple 会在 checkout 前稳定失败。 | 不 rerun、不改 #182、不 merge。 |
