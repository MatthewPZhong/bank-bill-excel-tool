...
| final gate 非精确 invocation 在 smoke job checkout 前显式 `exit 1`；授权后 exact tuple 迁移为 `R4 -> R3`，旧 R3 与 R4 的所有非精确 invocation 都在拒绝集合。 | 仅skip release-check仍可能让同名workflow/job变绿；新修复必须由自己的 exact head 验证。 | 只依赖人工纪律或branch protection context；让后续build继续；沿用旧 R3 PASS 代替 R4。 | 仅 same-repo R4 PR `opened`、正确 base、attempt 1运行release-check；wrong base/synchronize/reopened/rerun/fork/dispatch及旧 R3稳定失败。 |
| 修复置于新的 R4 branch；release owner 明确回复“推”后允许推送并开新的 final PR，但不修改 #182。 | #182 exact HEAD与旧 automatic CI仍在运行；新代码不能借用旧结果。 | 直接push R3 branch触发 synchronize；取消/重跑旧 CI；等待旧结果后再复用。 | 新 R4 automatic attempt 独立记录为 attempt #3；仍不授权 merge/main/tag/production。 |
...
| 初始 R4 只允许本地修复。 | release owner 后续明确授权推送 R4 final PR与一次新的 automatic CI，exact tuple改为 `codex/v3.2.1-r4-review-hardening -> codex/v3.2.1-r3-release-evidence`。 | 用户新授权；#182 旧 CI仍运行且不能验证R4。 | 产生新的 automatic attempt #3；仍禁止rerun和merge。 | release-evidence snapshot、workflow、validator与truth table同步。 |
...
| `node --test tests/unit/windows-build-contract.test.js` | PASS，5；SKIP 2 个仅 Windows packaged 专用用例 | R4 exact authorized tuple、wrong base/synchronize/reopened/rerun/fork/dispatch 与旧 R3拒绝真值表。 |
...
| CI 失败模式 | 旧 R3 与 R4 非精确 invocation 都在 checkout 前失败，`build` 因 `needs: smoke-test` 不会继续形成绿色替代证据；仅新 R4 exact opened tuple放行。 | workflow依赖图、Windows contract truth table、validator。 |
...
| 未知 | 处理 | 负责人/下一步 | 合并影响 |
| --- | --- | --- | --- |
| R4 automatic attempt #3 结果。 | PENDING_REMOTE_REQUIRED_CI | PR opened 后只读等待首轮 CI；不得 rerun、synchronize 或用旧 #182 结果替代。 | PASS 前 hard gate保持open，不得merge或启用production。 |
