...
| 字段 | 值 |
|---|---|
| 文档版本 | v0.19（2026-05-20 — PR #50 round 3 reviewer NewF1：reader sax 层 header row 用 dynamic array + cell write 条件放宽 让 validator 真正检测「列多」case，补 Round 2 CodexP2 的不完整修复）；v0.18 = round 2 多 finding；v0.17 = round 1 F1+F2+F3；v0.16-v0.1 起草 |
...
| **O**（fix6 + v0.18 PR #50 NewF1 修正）| 流水「通道清算金额 + 通道清算币种」单元格同步为空（30,057 行非清算流水，业务上 ACQ 部门下不走清算）| reader 入库 `settle_amount=''`/`settle_amount_abs=''`/`settle_currency=''`/`settle_currency_norm=''`，不整批拒绝；**run JOIN 保留参与对账**（spec §5.2 SQL 不过滤空值）→ 对应单据 `settle_currency_norm` 非空时 COALESCE 比较 `''` ≠ `非空` → 写入 diff_rows，diff_type='currency_mismatch'，差异表 28/29 列显示流水侧空字符串（便于财务看「单据有币种但流水无清算」状态）|
...
| v0.18 | 2026-05-20 | **PR #50 round 2 reviewer findings 修复**（NewF1/F2/F3 + CodexP1/P2）：① **NewF1 [P1]** spec Case O 描述「run JOIN 跳过空金额行」**与实际 code 行为不符**（code 保留参与对账，正确）。用户业务意图 = 流水侧 30,057 行空金额/币种行**保留参与 JOIN**，对单据有币种时作为 mismatch 写入差异表，便于财务排查；实测 466w 流水 0 单边空、461w 单据 100% 都有币种。reverse sync 改 spec Case O 描述（「跳过」→「保留」）+ USER_GUIDE §1.8.6 改「跳过」→「保留进差异表」；**code 不动**；smoke Case O 新增 7 个 assert 验证 run 行为（流水空 vs 单据 EUR → diff_rows 含 1 行 + diff_type='currency_mismatch'）。② **CodexP1 [P1]** writer Pass 2 写入时实时检测 sheet 行数，超 MAX 自动开 sub-sheet 加后缀 `(2)(3)`（防御单日 > 1,048,575 行 edge case；planSegments 保持原逻辑同日不切，Pass 2 内部细切）。③ **NewF2 [P2]** session.runCheck 写盘失败 catch 块**真正落库** `runs.status='success-no-files'`（之前注释说做但代码没做）；run-repository 加 `updateRunStatus`；cleanupOrphanData 跳过 `success-no-files` 状态（识别为可恢复 run，不清数据）。④ **NewF3 [P2]** reader streamImportOneFile 接 `streamSheetRows` 的 resolve 返回值 `streamStopValue`，header 失败时优先 `throw streamStopValue` 携带 detailLines（之前抛「xlsx 无表头」吞掉真实列差异明细，多文件批量导入误报）。⑤ **CodexP2 [P2]** reader header 解析去 `.slice(0, expectedHeaders.length)` truncate，让 validator 看到实际列数（之前列多被静默忽略）。smoke 161 → 168（Case O +7）|
| v0.19 | 2026-05-20 | **PR #50 round 3 reviewer NewF1 修复**：reviewer 指出 round 2 的 CodexP2 修复未真正生效 — reader.js `streamSheetRows()` 行初始化 `new Array(expectedHeaders.length).fill('')` + cell write 条件 `currentCellCol < currentRowValues.length` **在 sax 解析层就丢弃了多余列**，validator 看到的 array 永远等长。修复：① row tag 处理时区分 header row vs data row：`r === 1` 用 dynamic array `[]`（让 sparse 扩容），其他用固定 `expectedHeaders.length` fill ''（防数据 row 越界）；② cell closetag write 条件改为 `currentRowR === 1 \|\| currentCellCol < currentRowValues.length`（header row 允许任意列号写入）。smoke 加 Case ExtraColumn 4 assert：构造 49 列 xlsx（多 1 列 'multi 多余列'）→ session.importFlowFiles 抛 ImportValidationError + message 含「模板 48 列，文件 49 列」+ detailLines 含「多余列」文本。smoke 168 → 172 |
