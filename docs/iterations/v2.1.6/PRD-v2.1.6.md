...
| 字段 | 值 |
|---|---|
| 文档版本 | v0.12（2026-05-20 — PR #50 round 1/2/3 reviewer findings 修复集中追溯，spec v0.17/v0.18/v0.19 同步）；v0.10 = fix15 月份选择弹窗标题三分支；v0.9 = fix14 UI 镜像布局；v0.8 = fix11/12/13 联合调整；v0.7 = fix5-fix10 集中追溯；v0.6 = fix4；v0.5 = fix2；v0.4 = fix1；v0.3-v0.1 起草 |
| 目标版本 | `v2.1.6`（patch） |
...
| v0.10 | 2026-05-20 | **fix15 月份选择弹窗标题三分支**：用户反馈「导入流水表/单据表点击后弹窗左上角文本改为『请选择导入文件的月份』」，renderer-dialogs.js `createAcquiringBillCurrencyMonthPickerDialog` 标题逻辑从 2 分支扩到 3 分支（'导入' → '请选择导入文件的月份' / '导出' → '选择导出差异的月份' / 其他 → '选择对账月份'）|
| v0.11 | 2026-05-20 | **PR #50 round 1 reviewer findings 修复**（spec v0.17）：F1 [P1] smoke cleanup helper 解 Windows CI EBUSY；F2 [P1] 账单日期 YYYY/MM/DD 入库归一化为 YYYY-MM-DD + writer sanitize 防御；F3 [P2] USER_GUIDE.md §1.8.2-§1.8.9 同步 fix5/11/13/14 最终口径 |
| v0.12 | 2026-05-20 | **PR #50 round 2 + round 3 reviewer findings 修复**（spec v0.18 / v0.19）：① round 2 NewF1 [P1] spec Case O 描述错（code 保留参与对账正确）— 实测 466w 流水 0 单边空 + 461w 单据 100% 都有币种印证业务意图，修 spec/USER_GUIDE 不修 code；② CodexP1 [P1] writer Pass 2 sub-sheet 实时切分防单日 > MAX edge case；③ NewF2 [P2] writeRunOutputs 失败 → run.status='success-no-files' + cleanupOrphanData 跳过此状态；④ NewF3 [P2] reader header 错 detailLines 透传不被「无表头」吞；⑤ CodexP2 [P2] 去 .slice() truncate（R2 不完整，R3 完整修复）；⑥ round 3 NewF1 [P2] reader sax 层 header row 用 dynamic array 让 validator 真正检测列多 |
