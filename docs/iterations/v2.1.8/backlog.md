...
| **N4** | 收单差异表输出字段瘦身（29→12 列按模版）+ DB raw_json 也瘦身（破坏性 migration）| 输出契约变更 + 破坏性 schema | 🔴 HIGH | ~2-3 天 | 2026-05-26 用户立项 |

**资金红线提醒（CLAUDE.md 规则 7）**：F5 + A3 + A4 + N4 四条都涉及资金 / 对账 / 状态机变更，spec 阶段必须有专门 reviewer。**N4 是本批最重风险**：对外契约破坏性变更 + DB 永久删字段（不可逆），实施前必须确认数据备份策略 + migration 中断恢复方案。G1 不动业务代码，但 unit case 一旦落地会**钉死现有金额/币种/对账契约**，PR review 需关注 case 期望值的准确性。N2/N3 触及对账配置/对账结果导出，属于对账契约边缘，spec 阶段需 review。
