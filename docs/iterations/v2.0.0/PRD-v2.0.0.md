...
**关键列**：`pending资金类型`（注意首字母小写 p；导出差异分 sheet 的依据，**必须存在**，否则导入校验即失败）

**已知可能值**（OT-9 确认）：`提现` / `退票` / `充值`。**导入阶段做枚举校验**：任一行 `pending资金类型` 值不在这 3 种里，整批拒绝，状态框报错 `Pending 资金类型值不合法：发现 "{实际值}"，仅允许 {提现/退票/充值}。`
...
| OT-9 | `pending资金类型` 枚举 | **{提现/退票/充值} 三种枚举校验**；非枚举值导入期整批拒绝 | §4.1 / §5.4 |
