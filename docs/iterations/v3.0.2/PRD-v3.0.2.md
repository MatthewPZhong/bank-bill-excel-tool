...
| AC3-5 | 字段取值规则 1v多 匹配 → 每笔取对应 `matches[i]`（逐笔不同 channelRow）赋值正确 |
| AC3-6 | 字段取值规则 多v1 匹配 → 每笔主行取共同 `rightRow` 赋值正确 |
...
| 字段取值 1v多 / 多v1 | 配对应规则 → 跑对账 | — | 逐笔 matches[i] / 共同 rightRow 赋值正确（AC3-5/6） |
