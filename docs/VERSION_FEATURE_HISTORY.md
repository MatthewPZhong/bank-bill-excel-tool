...
v3.1.8 将 VCC 财务OP校验的输入、调整、归档生命周期和 Excel 输出收口为可以追溯、遇到异常会明确停止的完整流程。六项人工门禁已经用户明确确认通过，当前已批准进入正式发布流程，但 tag 和 GitHub Release 尚未创建。
...
- 该通过结论来自人工明确签字，不由自动测试代替。当前状态为“已批准发布、待 annotated tag + Windows Release workflow + 公开资产复核”，仍不得把本节写成已经发布。
