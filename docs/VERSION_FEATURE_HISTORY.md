...
工具箱按行模式按有效数据行跨 Sheet 连续拆分，表头不计入 N，每批最多 1000 份；字段拆分和原多分组方式保留。VCC 按 Sheet 识别正式表头、说明页排除及通道主体，沿用按原表类型分组的事务和幂等规则；系统 OP 的九币种快照须在同一 Sheet 完整成立。待确认表导出全部主体及差异来源，不改变归档、余额和正式结果导出权限。
