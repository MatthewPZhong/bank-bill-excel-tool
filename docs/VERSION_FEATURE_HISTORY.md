...
v3.1.8 发布候选将 VCC 财务OP校验的输入、调整、归档生命周期和 Excel 输出收口为可以追溯、遇到异常会明确停止的完整流程。
...
- Windows installer/portable 中两份模板的实际存在性和可读性，Windows Excel/WPS 字体、填充、调整行、打印区和 M/N 展示，受保护后台任务关窗收口，以及真实月份逐主体逐币种资金复核仍待人工完成。自动测试成功不代表 v3.1.8 已发布或资金验收已通过。
