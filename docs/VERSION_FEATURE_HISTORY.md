...
## v3.2.7 开发状态：新区间业务 OP（已获显式启用授权）

PR1a—PR6 按顺序实现共享关闭观察、目录恢复、公共同步候选写入、区间计算、七类输出、页面/两种删除、一次性清旧与旧入口退役。原件/版本/结果/说明独立保留，历史输出可以追溯冻结输入。

2026-09-06 用户明确要求“全部开启”，当前已配置开启十二项业务 OP 后台动作，应用版本保持 3.2.6。用户授权与测试结果分别记录：Windows 目录耐久、2200 万输入/最大结果并集与人工资金、Excel/WPS 验收仍未完成；不将授权转换为测试通过。实际启用仍须经过既有资源预检、旧任务收口、五阶段清旧和恢复就绪。详细启用记录见 `changes/v3.2.7/enablement`；原实现证据见 `changes/v3.2.7/pr1b` 至 `pr6`。
