...
- 主库在侧库写入前预登记基准 checkpoint 和 operation token；主库与平盘侧库再通过实例身份、generation、事务 token、父 token 和 operation token 历史校验同批数据。侧库缺失、空库、旧主库单独恢复、旧库回滚、同代或更高代次分叉时阻断使用；旧 initialized 版本可验证迁移。备份恢复必须同时包含 `tool-data.sqlite` 与 `run-data/`。
- 运行来源由原始银行行重新推导，已确认消费冲突数由逐行血缘重算；scope、snapshot 和 summary 即使共同改写也必须与明细一致。
- 静态模板与运行时 writer 共用标识符判定；结果、链接和原始表在列级固定 ID、账号、卡号、CustomerRef、客户编号、客户号和 `accountReference` 等文本格式，零数据工作簿也不会把长 ID 或前导零账号转成数字。
...
- 平盘成功输入、每次成功结果导出和结果回导均形成独立即时存档批次；业务返回成功前完成存档或形成可重试失败记录，存档状态与业务表库状态互不影响。
