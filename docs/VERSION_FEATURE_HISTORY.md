...
本轮包含六项产品功能：业务 OP 错误报告自动保存、按模块设置存档期限、存档永久删除、定时深色模式、工具箱按行拆分、VCC 多 Sheet 导入及待确认表导出；同时整合仓库规则、Skill 和版本资料目录。

业务 OP 保留原诊断字段和核对口径，报告自动写入现有错误目录，业务结果与保存结果分开反馈。存档期限沿用默认 60 天及永久等选项，14 个模块可独立覆盖或继承；仅新建批次采用新值。永久删除继续保护锁定、活动任务、业务引用和恢复中的批次，仅清理有授权归属的存档/受管副本，并以实际文件清理结果反馈完成状态。

“设置 → 外观设置”提供默认关闭的定时深色模式，默认本地时间 18:30–06:00，支持跨午夜；自定义图片保留，默认/纯色背景在深色时使用深色底色。原生时间输入和连续保存的回包不会覆盖更新的草稿。

工具箱按行模式按有效数据行跨 Sheet 连续拆分，表头不计入 N，每批最多 1000 份；字段拆分和原多分组方式保留。总配额不足时立即反馈，暂时占用继续限时等待；失败显示申请、总额、占用和可用额度，源件与旧目标保持原有保护。VCC 按 Sheet 识别正式表头、说明页排除及通道主体，沿用按原表类型分组的事务和幂等规则；系统 OP 的九币种快照须在同一 Sheet 完整成立。待确认表导出全部主体及差异来源，不改变归档、余额和正式结果导出权限。

VCC 存储升级到 v3，保留历史原表与来源身份边界，旧程序不得写入升级后的数据库；回退使用升级前完整备份或既定维护流程。所有六项 Spec/TechDoc 及审查、门禁与发布状态见 `changes/v3.2.9/release.md`，平台/人工检查不以模拟或历史门禁代替。

## v3.2.8（2026-09-07）

修复 Windows 更新至新版业务 OP 后在目录落盘预检被阻断、无法打开主窗口的问题。目录同步使用 Windows 可写句柄，候选分片封存和旧文件回收同样补齐真实同步；导出发布与存档中心同步修正同类调用。原件字节和只读保护、迁移阶段及失败保护保持原合同。

导出候选的文件身份改用无损整数字符串，解决 Windows 文件编号较大时凭据保存失败的问题；内部凭据版本显式区分，新代码兼容旧安全数值凭据，账单与结果内容保持不变。

启动资源预检继续申请原有 1 GiB 后台预算；不足时显示实际可用预算和预算上限，任务和迁移进度保留，不通过降低预算绕过保护。

新增 Windows 正常源码应用和实际打包应用验收，覆盖首次激活、ACTIVE 重启、真实 IPC 导入/列表/核对/七类导出/删除/中断恢复，并阻止业务 OP 因目录能力不足跳过成功路径。自动化 Windows 结果、目标设备人工验收和实际在线升级分别记录，详见 `changes/windows-bizop-durability/`。

本补丁不修改业务公式、币种和账号语义、数据格式或历史输出。已经完成新版升级的用户无需重新导入；尚未完成的首次升级仍沿用 v3.2.7 明确的检查与清理范围。

## v3.2.7（2026-09-07）

发布前修复启动与退出保护：业务 OP 首次升级及未完成升级的恢复均检查资源，资源不足及时失败并保留原任务与数据；有效验收证据更新不改写首次授权和升级收据，非法日历日期继续拒绝。其他业务 OP Main 阶段无法满足固定预算时立即拒绝，暂时竞争最多等待 5 秒；启动错误保留内部原因。退出清理修正 Promise 链，完整等待后台与存档收尾，失败仍可重试。

PR1a—PR6 按顺序实现共享关闭观察、目录恢复、公共同步候选写入、区间计算、七类输出、页面/两种删除、一次性清旧与旧入口退役。原件/版本/结果/说明独立保留，历史输出可以追溯冻结输入。

2026-09-06 用户明确要求“全部开启”，当前已配置开启十二项业务 OP 后台动作，版本元数据收口为 3.2.7。用户授权与测试结果分别记录：Windows 目录耐久、2200 万输入/最大结果并集与人工资金、Excel/WPS 验收仍为 NOT_RUN；不将授权转换为测试通过。实际启用仍须经过既有资源预检、旧任务收口、五阶段清旧和恢复就绪。首次从旧业务 OP 模块升级会清理旧明细、旧结果及专属月度侧库，业务数据需从原件重新导入；历史存档、原件、用户锁、其他模块与外部文件保留。详细启用记录见 `changes/v3.2.7/enablement`；原实现证据见 `changes/v3.2.7/pr1b` 至 `pr6`。

2026-09-07 修正 PR5 的页面布局偏差：按 VCC 财务 OP 恢复两行四按钮主面板，输入“导出数据”放回数据管理底部，数据分类改回左侧导航。输入/结果列序、操作月份、删除确认及后台执行合同保持不变。

同轮界面细节收口：统一欢迎语，取消导入回到导入按钮原位；结果导出月份与表名横排，标题与分割线对齐。管理页移除刷新/下一页，采用“删除 / 导出 / 返回”与按需页码；两种 OP 的结果表操作标题及文字按钮左对齐。删除仍先选取，再确认影响。

差异导出按用户要求移除“核对说明”工作表，19 列保留，完整说明继续从结果原表取得；差异为空时仅有表头页。输入导出的账期/目标也横排缩窄，导出原表时不显示管理页进度框，选取删除阶段移除整版本注释。

开始运行的起止日期改用有数据月历：每次打开默认定位最近成功导入的当前 OP 数据账期月份，只允许选择当前有 OP 数据的日期，可切到前后有数据月份。标题对齐分割线，日期框缩为原宽 2/3，检查按钮放到左下角并移除原区间说明；原流水完整性和版本预检保留。

导出数据继续精简：去掉当前可用版本说明，日期框缩至调整前的 4/5，目标下拉框缩至 1/2；导出数据和导出校验结果表的提交按钮统一显示“导出”。确认删除影响标题与下方分割线左侧对齐。

最终弹窗统一参照 VCC 的导出、删除、月份选择及导入账期页面：标题与正文字体、控件和按钮采用紧凑尺寸，缩小整体宽度、字段间距及上下留白。开始运行的检查按钮与左日期框对齐，关闭按钮与右日期框对齐；导出数据的返回按钮与目标下拉框右侧对齐，结果导出的关闭按钮与右侧选择框对齐。长清单仅在内容区滚动，底部按钮可见；预检表格保留，额外成功说明移除。

结果原表导出期间，数据管理底部的删除、导出、返回按钮保持原位置，左侧取消按钮与删除按钮同尺寸。结果原表、OP 校验原表及流水校验原表仅在后台确认成功后弹出导出提醒；取消、失败与尚未完成均不显示成功。

合计流水发生额改为“流水入账合计 − 流水出账合计”，倒推余额相应改为“终止期末 − 合计流水发生额”，第 14 列表头为“终止期末－合计流水”。倒推余额、核对差额、金额结论、原因和差异行集合保持原值；冲正符号、高精度十进制及 `0.01` 容差保持不变。

新计算使用独立规则版本，已经导入新版区间模块的 OP/流水数据无需重新导入。首次按新公式重跑同一区间产生新结果版本，同一新规则下再次运行复用该结果；历史封存结果、说明和原件不回写，导出按对应版本使用原表头和数值。68 项相关自动测试、17 组旧代码到新代码的真实临时文件回放通过：其余 18 个业务列及差异集合保持，历史导出逐格一致，封存文件校验值不变。证据见 `changes/v3.2.7/pr3/flow-net-validation.md`，不代替未执行的资金与环境人工验收。
...
## v3.2.6（2026-09-05，正式发布）

### 新增

- “银行对账单赋值自身”的对账字段增加“等于 / 包含”单选操作符；“包含”按左侧包含右侧，空值不命中、区分大小写、多条件 AND。操作符贯通配置保存、复制、确认详情和场景导入导出。
- 每条对账字段按自身选择的左右账单类型取值，支持同一类型对的正反向条件混用；反向条件不再因复用第一条方向而误赋值、误锁定或漏匹配。
- 自己输入 MerchantId 的模板在【提取大账号顺序】时增加全批维护检查，保留账户映射得到的原始账号及全部文件/分段来源；未维护账号经用户确认后结束整批导入。

### 变更与兼容

- C2 多候选按全部命中关系检查双方唯一性，交叉歧义不误赋值、不锁定；反向 AND 换序不改变修改和锁定对象，同场景中的独立合法配对继续执行。旧反向类型且左右异字段的等于条件也按所选类型修正来源，可能改变命中结果，须人工复核。
- 提取与完成互斥，取消失败可重试；已绑定、未选择、保留的空交易分段同样检查。当前未完成批次取消后不生成账单、不保存本批顺序，历史结果继续有效。
- 未维护提醒优先定位本分段表头，首段、中间段和末尾空段均可准确定位；缺少表头证据时才回退到预览行号。
- 完全未识别仍可返回手动选择，已维护多币种继续原选择流程；检查只由提取按钮触发。
- 旧场景缺省为“等于”，无数据库迁移。降版前将“包含”场景停用或改回“等于”。后台执行平台生产策略保持原状。
- package 元数据更新为 `3.2.6`；自动化证据见 `changes/3.2.6/implementation-notes.md`，脱敏真实账单与资金归属人工复核待完成。
