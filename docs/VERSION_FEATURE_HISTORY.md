...
- **小助手功能收纳弹窗**（PRD T3）：主页面右下角 🔄 按钮 → 弹窗双区域（闲置 / 启用）+ ➡️/⬅️ 移动 + 启用区行末 ⋮⋮ 拖拽手柄即时落库
  - 持久化：SQLite `app_settings.enabled_modules`（JSON 数组）；首次启动 seed 默认 3 个启用模块（网银账单生成 / 银行对账单处理 / 对账单ReconID修复）
  - 启用区至少保留 1 个（O3）+ 当前激活模块被移出时自动切到启用区第 1 个（O4）
  - 闲置区始终按模块名 name.length 升序展示（O1）+ 即时落库无"保存/取消"（O6）
