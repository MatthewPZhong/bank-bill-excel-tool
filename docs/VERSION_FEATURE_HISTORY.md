...
- 主库与平盘侧库通过实例身份、generation、事务 token 和父 token 历史链校验同批数据；侧库缺失、空库、合法旧库回滚、同代分叉或更高代次分叉时阻断使用，只有可证明祖先关系的异常退出窗口才自动追平。备份恢复必须同时包含 `tool-data.sqlite` 与 `run-data/`；运行范围、快照和汇总还会与实际明细交叉校验。
- 静态模板与运行时 writer 共用标识符判定；结果、链接和原始表在列级固定 ID、账号、卡号、CustomerRef、客户编号等文本格式，零数据工作簿也不会把长 ID 或前导零账号转成数字。
...
- 平盘成功输入、每次成功结果导出和结果回导均形成独立即时存档批次；存档状态与业务表库状态互不影响。
