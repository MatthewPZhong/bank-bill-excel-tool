...
## v3.2.7 开发状态：新区间业务 OP（未启用）

PR1a—PR6 按顺序实现共享关闭观察、目录恢复、公共同步候选写入、区间计算、七类输出、页面/两种删除、一次性清旧与旧入口退役。原件/版本/结果/说明独立保留，历史输出可以追溯冻结输入。

当前默认仍使用旧生产模块，版本保持 3.2.6。发布开关不会由版本号自动打开，迁移尚未对用户数据执行；Windows 目录耐久、2200 万输入/最大结果并集与人工资金、Excel/WPS 门禁仍未通过。详细证据及实施范围见 `changes/v3.2.7/pr1b` 至 `pr6`。
