...
4. 资金对账数据处理（`v2.0.0-beta.3` 新增 / `v2.1.13` 新增「自带写死场景」+「复制场景」、功能类别改名、主面板镜像 / `v2.1.14` 由「银行对账单处理」**改名**，面板改 control-row 三行布局、新增「链接表管理」入口与「导入不平表」按钮；下文 §1.4 沿用旧称「银行对账单处理」指同一模块）
...
8. 收单单据币种校验（`v2.1.6` 新增 / `v2.1.12` 导入解析提速 + 大数据量对账多 worker 并行提速，行为不变）
