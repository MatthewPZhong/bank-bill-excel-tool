...
> 发布说明：v3.1.6 新增「VCC财务OP校验」，并修复平盘普通链接原始表在预检后可能被存档清单门禁误阻断的问题。VCC 是资金校验功能，首次正式使用前必须完成历史业务键扫描和真实数据人工复核；平盘 Electron 真实文件重试尚未执行，技术发布后仍须补做，不得记为已验证。
...
> **v3.0.22 存档中心**：点击主页面左下角 ⚙️，再选择左侧“存档中心”，可以按日期、模块和批次查看 12 个模块成功处理时使用的输入文件和结果文件。通常同一业务批次只保存第一次正常结果；平盘模块的成功输入、每次结果导出和结果回导各自形成独立即时批次。一般文件复制失败会保留可重试记录，不会回滚已完成业务；相同内容只占一份磁盘空间。详见 §1.11.9。
...
2. 勾选后下方展开**三个输入框**，三项**都得填**：
   ...
   - **大账号**（如 `202782001`；`v3.0.24` 起可填 `202782001、202782002`）——使用中文顿号 `、` 分隔，既用来筛订单侧的「收款账户（卡号）」，也用来筛银行侧的大账号。不能使用逗号，不能有空段或重复账号。
...
- **R5s2**：不论选择调拨对账单还是网关对账单作为来源，银行真实方向都必须与 `FundTransfer-in/out` 一致。错误方向行不会增加多候选数、不会占用银行行、不会回填 ReconciliationId，也不会因此拦住后续 Payment 线下调拨。
...
- 点 `[完成]` 后只选择一次输出目录。若多个目标文件已存在，软件会一次列出并询问是否「覆盖全部」；取消后不会创建或覆盖任何文件。
...
点击主页面左下角 ⚙️ 打开“设置”，再点击左侧“存档中心”。每次重新打开设置时仍默认显示“自动更新”，不会改变原在线升级操作。

#### 会保存什么

- 支持存档的 12 个既有主模块本次成功处理所使用的输入文件，以及符合各模块存档规则的正常结果文件。VCC财务OP校验的原表、导入审计、期初和结果归档保存在其「数据管理」中，不进入本节存档中心。
- 通常有“开始运行”按钮的模块只在运行成功后建批；只导入未运行、运行失败或取消不会建立运行批次。
- 通常同一批次只保存第一次成功结果。之后再次另存、重复导出、Pending/银行 BU 聚合导出或业务 OP 日期区间导出不会追加到旧批次。
- “平盘对账数据处理”是明确例外：银行输入和成功识别的链接原始表在导入成功后立即各自存档；每次成功结果导出和结果回导也分别建立独立批次，不依赖是否在本次启动周期运行。
- “资金对账数据处理”的链接表、以及“前置资金对账”的临时 MPT，在导入成功时作为所属模块的独立输入批次保存，不会在之后每次运行时重复保存。
- VCC业务OP计算没有结果文件，保存计算成功后只存本次导入的流水文件。

错误报告、失败明细、模板、场景、规则和映射配置包、工具箱合并/拆分文件、日志、数据库、缓存及更新包都不会进入存档。

#### 查找和使用文件

1. 用“日期 / 模块 / 批次号”筛选批次；日期默认是今天。
2. 在左侧选择批次，右侧会显示输入/输出方向、原文件名、大小和存档状态。
3. 点击文件后的打开按钮会创建并打开只读副本，不会直接修改存档原件；“另存为”可复制到自己选择的位置。
4. 批次显示“不完整”或“失败”时，一般可点击重试。平盘模块会优先使用导入时保存的不可变暂存副本；其它模块通常再次读取当时的原文件路径。源文件和暂存副本都不可用时仍会失败。
5. 如果提示文件在业务完成后发生变化，软件会拒绝把变化后的内容存为原批次，且不会显示无效重试按钮。请回到原模块重新执行一次业务，生成新的存档批次。

#### 保留、锁定和删除

- 默认保留 60 天，可在存档设置里改为 30、60、90、180、365 天或永久。修改后点击弹窗底部“确认”保存并关闭；保存失败时弹窗会保留并显示错误。返回箭头或右上角关闭会放弃未保存修改。新设置只影响之后创建的批次；自动清理在下次启动软件时执行。
- 锁定批次不会被自动清理，也不能直接删除。先解锁，再点击删除并确认，才会永久删除该批存档。
- 删除存档不会删除原始输入文件、业务模块正常生成的结果文件或已经另存的副本。
- 相同内容会通过 SHA-256 自动去重：列表里仍能在每个批次看到各自文件，但磁盘只保存一份；只有最后一个引用被删除时才释放物理文件。

> v3.0.25 起不再提供网银模板“不存档”。升级启动后旧模板排除配置全部失效；之后生成的网银账单和月度余额都按正常规则存档。

#### 当前版本边界

- 输入文件与运行结果的对应关系只在本次打开软件期间维护。重启前只导入未运行的文件不会在重启后补档；升级前或以前已经落库的历史文件也不会自动补录。
- 存档中心是文件柜，只提供查找、打开副本和另存，不会从存档恢复数据库、重新导入或重新运行业务。
- 一般模块的存档仍按各自业务流程执行；平盘模块会在业务返回成功前完成本次存档，或写入可跨重启恢复的失败记录。若正式存档数据库暂时不可用，会先把文件清单原子写入持久 outbox，并在下次启动时转成正式批次。
- 平盘结果文件已经写出、但后续运行状态更新失败时，软件也会保留该文件的持久存档任务；不要因为页面提示失败就立刻重复导出，应先核对已生成文件和存档中心。
- 如果正式存档和持久 outbox 都无法写入，软件会提示“业务处理已完成，但存档重试记录建立失败”。此时业务数据或输出文件可能已经提交，**不要直接重复导入、运行、确认或导出**；先重启软件并核对平盘表库、待确认结果和存档中心。
- 正常关闭软件或重启升级时，会等待正在进行的存档和业务操作完成；大文件较多时退出可能稍慢。
...
## 1.14 VCC财务OP校验（v3.1.6 新增）

### 1.14.1 这个模块做什么

本模块把当月 VCC 财务核算单据和移除归档 Pending 转成按主体、币种汇总的发生额，再用“上月已归档余额 + 当月发生额”计算本月财务 OP，并和系统财务 OP 比较。确认归档后的计算余额会成为下月期初。
...
五个原表的空模板随软件放在 `assets/VCC财务OP校验/`，其中 Pending 模板为正式 48 列，系统财务 OP 模板为正式 16 列。请保留完整表头及顺序；系统财务 OP 缺列、多列或调换顺序时，选文件识别和正式导入二次校验都会提示实际列数与差异列。不要使用旧的 23 列 Pending 计算样表或 `YYMMOP + 系统行` 横表代替正式原表模板。

只有通道文件会显示公司主体输入框，必须填写真实主体，软件不会按文件名猜测。系统财务 OP 主体直接取正式模板的“主体”列，一个文件可以包含多个主体；目标月份的“业务部门”必须为 VCC。每个主体必须提供 AUD、CAD、CNY/CNH、EUR、GBP、HKD、JPY、SGD、USD 九币种且每个币种只有一行；CNY 在比较时映射为 CNH。比较余额固定取“财务余额”的 Excel 显示值，不取 OP期末余额或财务主体余额。

导入提交后，软件会先为本批次全部已识别原表类型建立导入记录。导入进行中可取消；即使在第一类原表读取前取消，或前一类异常导致后续类型尚未开始，记录也会明确显示失败且不参与计算。校验失败、工作进程异常退出或同键冲突时，未完成数据不会进入有效数据集，导入记录和回滚原因仍会保留。

导入完成后不会再弹出导入记录框。主页面状态框会显示本次账期、实际新增行数和幂等跳过行数；存在失败记录时，还会显示待处理异常记录数并提示进入「数据管理 → 校验原表」。完整记录和逐行明细仍保存在数据管理中。

单次可选择多个分片，合计支持 500 万行。真实 501 万行样本回放峰值内存约 342 MiB，但含逐行审计的 SQLite 文件约 8.62 GiB；不同数据文本长度会改变实际占用。正式大批量导入前建议至少预留 15 GiB 可用磁盘，并避免在机械硬盘或空间紧张的同步目录中运行。
...
- 同键不同原始内容：本类原表整次导入失败，不按先后顺序保留新值。
- 空业务键、非法日期、未知方向、未知币种或金额不合规：整类原表失败并回滚。
...
点击“查看导入明细”，可在导入汇总、幂等跳过、幂等冲突和其他异常中查询具体原始行。跳过和冲突会同时展示本次数据与已保留/对比数据的文件、sheet、行号和原字段，也可按原表表头导出审计文件。

失败记录不会参与计算。确认失败数据确实不应纳入时，填写处理说明并勾选“保留当前有效数据集，本次失败导入不参与计算”，才可标记已处理。这个动作不会修复或导入失败行；需要采用新数据时，应先查明原因后重新导入正确文件。

某类有效原表被删除后，该账期、该原表类型中原本成功的导入记录会在列表显示“已删除”。点击详情仍可看到原导入状态、删除时间、统计数和逐行审计；失败记录从未形成被删除的有效数据，因此继续显示原失败状态。删除后重新导入会新增一条正常记录，旧记录不会被改写或复活。

### 1.14.4 发生额口径

固定统计币种为 AUD、CAD、CNH、EUR、GBP、HKD、JPY、SGD、USD。
...
“是否错币”只用于明细说明：Pending 币种与流水币种相等时为 FALSE，不相等时为 TRUE；它不改变金额汇总。`pending资金类型` 和“备注”都不能替代对账类型、币种、金额或业务键。

所有金额最多 2 位小数、15 位有效数字。软件使用精确十进制相加，不会自动四舍五入或截断；系统财务 OP 的“财务余额”使用 Excel 看到的显示值，避免底层浮点尾差影响结果。

### 1.14.5 首月、运行和归档

点“开始运行”后，在“选择运行账期”中选择已导入账期。弹窗不显示“月份账期”文本，下拉框缩为原整行可用宽度的 25%，并与标题左侧对齐；下拉框下方的操作区横线下移 15px。运行前必须满足：充值清退、费用换汇、通道和系统财务 OP 已导入；所涉及的每个主体都有对应系统快照；该月没有正在进行的导入批次，也没有未处理失败记录。

![VCC财务OP选择运行账期](previews/vcc-financial-op-run-month.png)

有上月归档余额时，软件自动作为本月期初。首次使用或某主体没有上月归档时，会打开一次性期初初始化框：九个币种都必须填写，零余额填 `0`，同时填写余额来源和核对结论并勾选确认。保存后不能修改，软件也不会默认填 0 或用系统财务 OP 代替。

![VCC财务OP期初初始化](previews/vcc-financial-op-opening.png)

计算完成后，确认框按主体展示“当月计算财务OP、系统财务OP、差异”；差异 = 系统财务 OP - 当月计算财务 OP。逐项核对后勾选确认并点击“确认归档”。

![VCC财务OP结果确认](previews/vcc-financial-op-result.png)

归档后该账期永久冻结：不能新增或改写原表、系统快照、期初或结果，不能撤销归档或原地重算。完全相同的历史文件重放只会留下幂等跳过记录。若下月已经人工初始化某主体期初，软件也会禁止再归档上月，避免出现两个期初来源。

### 1.14.6 导出和数据管理

只有已确认归档的结果可以导出；重新打开应用后，主页面会自动恢复最近已归档结果的导出入口。每个主体生成一个 `YYYY-MM_主体_VCC财务OP校验结果表.xlsx`，文件只包含：

1. `财务OP校验结果表`：期初、当月各类发生额、当月计算财务 OP、系统财务 OP 和差异。
2. `移除归档Pending发生额计算表`：A:J 展示 Pending 汇总明细，J:K 展示每币种差额。

金额按千分位显示，工作表使用 A4 横向并按一页宽度打印。数据管理左侧“结果表”可查看计算/归档结果和人工期初；“校验表”可查看各类有效数据状态及生成时间；“校验原表”用于查询每次导入及异常审计。校验表生成时间表示当前有效数据最近一次实际新增完成时间；重复文件全部幂等跳过、失败导入或确认归档时不会变化。系统财务OP以“账期 + 主体”的九币种集合作为一个幂等快照，导入状态和详情会同时显示币种数据行数与主体快照数；例如单主体完整导入显示“9 行币种数据（1 个主体快照）”，不表示导入了九个独立快照。三个页面标题与“月份账期”保持同一水平线，并与下方表格首列表头左边界对齐；左侧导航文字与页面标题“数据管理”的左边界对齐。右下角依次为“删除 / 导出 / 返回”。

点击“导出”后，左侧选择月份账期，右侧“目标表”分为“校验原表”和“校验表”两组。可导出五类当前有效原表，或充值清退、费用换汇、通道、移除归档 Pending 和系统财务 OP 五类校验表；已归档但仍有效的数据也可只读导出。原表导出保留正式模板的完整表头、原列顺序、长编号和前导零；校验表附带实际参与计算的发生额、统计币种、Pending/流水发生额及是否错币。超过 1,048,575 条数据时会自动续 sheet，生成失败不会覆盖保存位置已有文件。

![VCC财务OP导出数据](previews/vcc-financial-op-export.png)

这里导出的是数据库中当前有效的业务数据，不是原上传文件本体：原文件样式、公式、隐藏行列和其它非业务 sheet 不会被还原。导入详情中的“导出当前分类”仍只导出幂等/异常审计；主页面“导出校验结果表”仍只导出已归档结果，三者用途不同。

删除页左侧选择“月份账期”，右侧从“目标表”选择 VCC充值清退明细、VCC费用及换汇明细、VCC通道明细、VCC_移除归档Pending账单或系统财务OP。月份账期下拉框恢复原宽后取 25%，并与标题“删除数据”左边界对齐；目标表保持原宽的 40%，紧邻月份账期右侧 10px。页面会先显示当前有效数据条数和将失效的未归档计算结果数；确认无误后点“删除”。

![VCC财务OP删除数据](previews/vcc-financial-op-delete.png)

删除会同时移除所选原表在该账期的当前有效数据和对应校验表数据，并删除该账期尚未归档的计算结果；之后需重新导入并运行。对应成功类导入记录的列表状态同时变为“已删除”，但原导入结果、幂等跳过/冲突、其他异常审计和人工初始化期初都会保留，审计中引用的旧数据仍可查看和导出。失败记录保持原失败状态。已归档账期、存在活动导入或正在执行任务时禁止删除。有效数据、删除审计和“已删除”状态在一个事务中完成，失败不会留下只完成一部分的状态。
...
这是资金校验功能。当前可访问的 PPHK 样本已扫描 5,047,229 行，其中三类明细为 2026-06、Pending 为 2026-05；四类业务键没有空值或重复。这不能替代完整历史验证。正式上线前必须由业务人员：

1. 扫描全部历史月份的订单号、渠道订单号和 PendingBizId，确认不存在合法跨期复用。
2. 用真实样本逐项核对主体、账期、币种、方向、金额、Pending J:K 和系统差异。
3. 连续验证至少两个月，确认上月归档余额准确进入下月期初。
4. 记录人工核对结论后再用于正式归档。
