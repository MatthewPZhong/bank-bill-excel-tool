...
> 发布说明：v3.1.9 当前为本地发布候选，尚未合并或正式发布。PR1—PR6 的本地实现和分阶段自动验证已经完成；独立代码评审、用户人工验收和最终发布总门禁仍待完成。Windows installer/portable、Excel/WPS、约 16 GB、约 700 万行、目标生产库和资金人工门禁当前均未记为通过。
...
> **v3.1.9 全局批次与存档中心发布候选**：实际处理、写入、导出和删除任务统一使用跨模块连续的 `YYYY-MM-DD-NNN` 批次号；失败和取消也保留号码，关联任务可跨重启查看。VCC财务OP校验和工具箱已真实接入存档，运行文件按年/月/日/批次目录展示，并支持存储位置迁移、修复、保留期清理和新的统计/列表/设置界面。VCC 大库信息改为后台读取，标准 v3.1.7 旧归档可以受控查看、导出和解归档。当前仍是本地候选，未完成人工门禁见 §1.11.9。
...
- 实际导入、运行、生成、导出、确认、调整、归档、解归档和删除等任务会在开始处理前取得批次号。查询列表、打开页面、预览、选择文件和取消保存不会增加运行次数。
- 任务失败或取消后，批次仍显示对应状态，号码不会回收；即使删除当前最新批次，设置页的“最新批次”也不会倒退，下一个任务不会复用旧号码。
- 没有运行文件但会改变业务状态的任务仍会保留批次记录，例如 VCC 调整、归档、解归档和删除；这类批次显示“本次任务无运行文件”，不会创建空文件夹。
- 同一业务流程中的导入、运行、导出等批次会显示“关联任务”。至少有两个仍存在的关联批次时才显示；同日号码会压缩展示，跨日则按日期分组。点击号码可切换详情，重启后仍能继续查看；重新开始一轮业务不会串入旧关联。

#### VCC财务OP校验和工具箱

- VCC财务OP校验不再只把原表和结果留在自己的数据管理中。真实导入、运行、结果调整、归档、解归档、删除、结果导出、数据导出和导入审计导出都会形成任务批次；有真实文件的任务会把输入或最终输出登记到对应批次。
- 工具箱“合并表格”会把选中的全部输入和最终合并结果放进同一批次。“拆分表格”读取字段和值只属于预览，不计次；用户确认保存后，真实输入和一个或多个最终拆分结果进入同一批次。取消选择或取消保存不产生空批次。
- VCC业务OP计算没有结果文件时，成功任务仍保留批次和本次实际输入。其它模块继续按各自页面上的真实操作形成批次，不会把日志、应用数据库、缓存、模板、规则配置或更新包当成普通运行文件。

#### 查找、统计和使用文件

1. 存档页顶部显示“文件总大小”；设置页同时显示存储位置、文件总大小、运行次数和最新批次。文件总大小按用户能在各批次中看到的正常运行文件求和；同一文件被两个批次引用时会在两边分别计入。
...
- 同一存储卷支持时，批次目录使用只读 hardlink；不支持、跨卷、网络位置或权限受限时使用流式 copy。两种方式都核对文件大小和摘要，应用不会原地修改目录中的存档文件。
- copy 入口被外部修改时，可从仍可信的存档内容 repair；hardlink 与存档真相共同受损、或摘要与记录不一致时会停止并要求从可信来源重试，不会把被修改内容当成新真相。
...
1. 在存档设置中点击存储位置旁的【变更】，选择目标文件夹；选中的文件夹本身就是新存储根目录。
...
5. 切换完成后，即使旧根清理暂时失败，新位置仍是唯一使用位置；页面会显示清理待完成状态，重启后继续。迁移不会把旧 hardlink 当普通文件重复复制，而是在目标位置从可信存档重新建立 hardlink/copy 目录。

#### VCC 大库与旧归档使用提示

- VCC 数据管理的月份、归档、解归档和删除预览在后台读取。打开页面会先显示框架和加载状态；读取失败可重试，切换目标不会为每次点击重复读取整库。
...
- 当前为本地发布候选，尚未合并或正式发布。自动验证不能替代 Windows installer/portable 实际运行、目标生产库 legacy/trigger 只读检查、约 16 GB 冷热性能、约 700 万行多 Sheet 工具箱、跨盘符/UNC/网络路径和 hardlink/copy/readonly 行为。
