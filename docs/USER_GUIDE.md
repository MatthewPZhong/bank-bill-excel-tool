...
> v3.2.7：启用业务 OP 区间核对，支持批量导入、所需输入检查、历史结果版本及全量/差异导出。运行、导出与删除弹窗按 VCC 样式缩小，导出按钮保持稳定并在成功后提醒；合计流水按入账减出账计算，核对差额和结论保持一致。首次从旧模块升级与新版数据的公式升级适用不同的导入说明，详见下文。
...
>
> 如启动提示业务 OP 资源预算不足，请关闭占用内存的其他程序后重新启动小助手，让应用重新读取资源预算。尚未完成的任务和恢复记录会保留；升级不会因等待无法满足的资源而一直停住。运行期间若提示等待后台资源超时，请在其他任务结束后重试。补充有效的验收记录不会要求重新升级或清理已导入的新版数据。
>
...
>
> **首次从旧业务 OP 模块升级**会清理旧明细、旧结果和专属月度侧库，需从原件重新导入业务数据；历史存档、原件、用户锁、其他模块与外部文件保留。这项首次升级清理不适用于已在新版区间模块完成导入后单独升级计算公式的情况。内存或磁盘不足、旧任务未结束时会保留保护；中途失败后继续同一次升级。激活后降回旧程序不会恢复旧业务数据，须按明确备份和人工流程处理；不要手工删除恢复记录来跳过保护。Windows 目录耐久、2200 万输入/最大结果并集、真实资金与 Excel/WPS 人工验收仍为 NOT_RUN，需另行完成。
