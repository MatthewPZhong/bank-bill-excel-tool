...
> v3.2.8：修复 Windows 更新后因业务 OP 目录同步失败无法打开软件的问题。已完成新版升级的输入和历史结果无需重新导入；尚未完成的首次升级会继续原有检查和升级步骤。原件、历史存档以及业务 OP 的计算和输出口径保持不变。
...
>
> 初始状态显示“欢迎使用小助手”。导入期间，【导入文件】原位变为【取消导入】，请求取消后等待任务实际结束再恢复。结果导出的操作月份和结果表下拉框横排显示。数据管理底部为【删除／导出／返回】：先点【删除】进入选取，勾选后再次点【删除】查看影响确认；【取消选取】退出选择。超过一页时使用左下方页码选择，换页会清空勾选；列表数据已变化时会回到第一页重新选取。结果表行内的【导出原表】以文字按钮显示。
>
> 差异文件只包含差异数据页，保留 19 列，不附“核对说明”工作表；没有差异时只有表头。需要完整说明时，在数据管理中点击同一版本的【导出原表】。导出期间管理页不显示进度框，仍可请求取消。“导出数据”窗口的账期和目标横排显示；删除选取阶段的注释已去掉，具体影响在确认窗口查看。

> 导出结果原表期间，数据管理底部【删除／导出／返回】保持原位置，左侧显示与【删除】同尺寸的【取消】。结果原表、OP 校验原表及流水校验原表导出成功后弹出提醒；取消或失败时不显示成功提醒。导出数据窗口使用【返回】，其右侧对齐目标下拉框；导出校验结果表的【关闭】右侧对齐结果表选择框。开始运行窗口的【检查所需数据】左侧对齐起始日期框，【关闭】右侧对齐终止日期框；预检成功后展示所需表格，不再显示额外的“所需输入齐全”说明。开始运行、导出数据、结果导出、日期选择、删除确认和成功提醒统一采用 VCC 的紧凑字体与小按钮，缩小标题、表单和底部留白；两侧边距一致。删除影响明细与运行输入清单较长时在内容区滚动查看，底部按钮保持可见。
>
