...
> 版本说明：本指南随 v3.1.12 应用构建并在打 tag 前定稿；当前公开稳定版、下载资产和发布时间以项目的 GitHub Releases 页面为准。v3.1.12 包含网银源文件确认、启动治理、VCC CNY/CNH 新导入口径以及存档中心布局、迁移和后台维护；发布负责人已于 2026-08-20 确认验收完成并授权合并。
...
> **v3.1.12 网银源文件、启动、VCC 币种与存档中心**：网银账单在真正生成前只核对一次用户确认期间的源文件身份，文件变化会要求重新选择；软件完成数据库与必要恢复后才显示主窗口，并移除每次启动的全库 `ANALYZE`。VCC 财务 OP 的新导入接受标准大写 CNH，并在计算、幂等和输出中统一为 CNY，历史数据不改。存档设置把【变更】放到“存档位置”右侧、完整路径移到下一行，变更位置会完整迁移已有存档；每次启动首次进入存档中心后再后台分页维护。发布负责人已于 2026-08-20 明确确认本版验收完成并授权合并；公开版本与资产状态以 GitHub Releases 为准。
