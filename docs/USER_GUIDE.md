...
> **⚠️ 流水重导清旧对账结果约束**（v2.1.3 round 3 P1 资金红线修订）：重新导入同日流水后，**该日期所有 BU 的旧对账结果（`biz_op_recon_runs` + `biz_op_recon_diff_rows`）会被自动清空**，请重新点「开始运行」生成新差异，再「导出差异」。**与业务OP 重导仅清单 BU 不同**：流水按 date 跨 BU 共用，重导即所有 BU 的对账失效；业务OP 按 (date, BU) 分片，重导仅清该单 BU 的对账。**资金红线**：流水换了对账没重跑 → 导出旧差异 = 资金事故。
