...
> 业务 OP 错误报告（v3.2.9）：本次操作产生可用诊断时，软件自动将 XLSX 报告保存到 `Documents/网银账单生成小助手/error-reports/{日期}/`，无需另存。页面会分别显示业务结果与报告保存状态；保存失败会保留原错误并提示原因，不能把报告失败理解为原业务已成功。应用被强制结束时，尚未开始的自动报告不保证在重启后补导。
