...
- **恢复后提示“平盘对账侧库缺失/数据代次不一致”**：说明只恢复了主数据库、只恢复了旧 side DB，或主数据库与 `run-data/` 不属于同一备份批次。软件会比较实例身份、数据代次和 checkpoint 父链并停止读取，旧库或更高代次分叉都不能冒充当前数据；只有能证明来自同一事务历史的异常退出窗口才会自动追平。退出软件并恢复完整数据目录。
