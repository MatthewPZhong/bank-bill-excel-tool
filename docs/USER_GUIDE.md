...
> 发布说明：v3.1.10 当前为发布候选。功能 PR #147 已合入 `main@f75af1ed4eb2cd7cead8ffd6562174b2fc24ee6e`，但尚未创建正式 tag 或 Release。现有 VCC 核心约 27.42 GB，新结构预计约 4.3–4.6 GB；真实副本至少下降 75%、Windows installer/portable 的 SQLite/WAL 切换，以及主体×九币种资金人工复核均未签字，因此当前不得正式发布。
...
> **v3.1.10 VCC 存储瘦身发布候选**：有效数据只保留计算、校验、幂等和最小血缘字段；原始输入由存档 artifact 保存，只有真正异常进入紧凑审计。数据管理改为六列【导出明细】，校验原表按当前有效行从已验证原件重建；历史血缘缺口可明确标记后部分导出，实体损坏则整次停止。【优化存储】通过维护模式 copy-on-write 重建数据库。PR #147 已合入，但真实约 27.42 GB→4.3–4.6 GB/至少下降 75%、Windows installer/portable SQLite/WAL 和主体×九币种人工复核仍是正式发布阻断项。
...
单次可选择多个分片，合计支持 500 万行。旧合同下，真实 501 万行样本因永久逐行审计曾让 SQLite 增长到约 8.62 GiB；v3.1.10 新导入改为精简有效行、紧凑异常和存档原件，不再重复保存全部原始 JSON。升级不会在普通启动时自动缩小历史数据库，必须在数据管理中显式执行【优化存储】；开始前仍要按页面空间预检结果保留足够磁盘，不要在空间紧张或不稳定的同步目录中运行。
...
v3.1.10 不再提供“查看导入明细”和对应分页列表。只有导入记录存在真正异常或文件级失败时，操作栏才显示【导出明细】；导出的 `.xlsx` 固定六列为“幂等键 / 文件名 / 原表行号 / 分类 / 异常字段 / 说明”。纯成功和纯幂等跳过只保留汇总计数，不显示按钮；失败且尚未处理的记录继续显示【标记已处理】。
...
数据管理左侧“结果表”只查看计算/归档结果，不展示独立的首月期初初始化审计卡片；“校验表”可查看各类有效数据状态及生成时间；“校验原表”用于查询每次导入、输入文件存档状态和真正异常。校验表生成时间表示当前有效数据最近一次实际新增完成时间；幂等跳过、失败导入或确认归档时不会变化。系统财务OP以“账期 + 主体”的九币种集合作为一个幂等快照，导入状态会显示币种数据行数与主体快照数。数据管理底部的“解归档 / 优化存储”固定在左侧，“删除 / 导出 / 返回”保持在右侧及原顺序。
...
点击左下角【优化存储】可把历史 VCC 大表转换为 v3.1.10 精简合同。软件先检查是否仍有进行中导入和 staging 数据；确认后进入维护模式，等待活动任务结束并禁止新任务，执行 WAL 截断、完整性检查、磁盘空间预检、copy-on-write 候选重建和计数/哈希/九币种守恒验证。成功后原子切换并重启；是否在首次只读校验后删除旧数据库备份，由确认框中的复选项决定。空间不足、checkpoint、复制、守恒或首次只读校验失败时不会把候选当成新真相，旧库保持或按 journal 自动恢复。
...
v3.1.10 发布候选的功能 PR #147 已合入 `main`，Windows CI、完整自动回归和最终 reviewer 均已通过；但以下三项仍是独立发布红线，不能由自动测试代签：
...
上述三项未签字前，v3.1.10 仍是发布候选，不得创建正式 tag/Release 或宣称已经正式发布。下面的 2026-08-09 六项结论只属于已发布 v3.1.8，不自动覆盖本次存储合同和物理迁移。
