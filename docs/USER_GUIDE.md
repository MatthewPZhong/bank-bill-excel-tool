...
> 发布说明：v3.1.7 调整 Payment 线下调拨与 R5s2-recon 的执行顺序和共享消费，新增付款账户与订单周边界核对。技术 Release 正在准备；该功能会写入银行 ReconciliationId，正式业务启用和公告前仍必须人工复核固定样本中的 220 条 Payment 配对和 2 条 R5 后续回填。
