...
5. 对账单 ReconID 修复（`v2.1.0-beta.1` 新增单据对账单子模式 / `v2.1.0-beta.3` 扩展网关对账单子模式 / `v2.1.5` 模块名加空格）
6. 月度银行对账单BU回填校验（`v2.1.2` 新增）
7. 业务OP数据核对（`v2.1.3` 新增 / `v2.1.12-beta.1` 导入支持百万行级大文件 + 子进程 worker 化，行为不变）
8. 收单单据币种校验（`v2.1.6` 新增 / `v2.1.12-beta.1` 导入解析提速 + 大数据量对账多 worker 并行提速，行为不变）
...
> `v2.1.12-beta.1`：业务OP 账单 / 流水对账单**导入支持百万行级大文件**。旧的全量加载方式（SheetJS）在百万行 xlsx 会超过底层单字符串 512MB 硬上限而读不出，本版导入改为**流式读取 + 子进程 worker 化**（边读边分批入库、内存恒定、不阻塞主界面），双重校验/整批拒绝/失败报告/重导清旧对账结果等**所有资金红线行为完全不变**。
...
- 单 xlsx 单 sheet **不限行数**（v2.1.6 fix2 起 reader 改为 yauzl + sax 流式，支持 POI 流式导出 30w+ 行 inlineStr + ZIP data descriptor 格式）；`v2.1.12-beta.1` 导入解析由 sax 库换手写字节扫描，**实测端到端提速 5.6x（50万行 122s→22s）**、内存更低，入库金额/币种/原始数据与旧版**逐行 SHA1 完全一致**（行为不变）；按月组织，多个 xlsx 视为一类
...
| 开始运行 | 点击后弹「选择对账月份」弹窗 → 选月份 → SQL JOIN 对账 + **同步产出**单文件多 sheet 差异 xlsx（`v2.1.12-beta.1` 大数据量对账多 worker 并行提速，50万行实测 2.3–2.7x，差异结果与单 worker 完全一致、行为不变；`<100万行` 自动走单 worker，可经 settings `acquiring_bill_worker_count` 调整/回退）|
