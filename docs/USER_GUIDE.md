...
> **v2.1.4 起**：左上角模块切换菜单默认只展示 3 个高频模块（网银账单生成 / 资金对账数据处理 / 对账单修复），其他模块包括新增的「前置资金对账」默认收起，需通过主页面右下角 🔄「小助手功能收纳」启用。详见 §1.10。
...
3.0.14 只开放「缺网关账单」：拿本次导入的标准银行对账单，和临时 MPT 网关账单 + 现有网关链接表做严格 1:1 匹配，找出银行有、网关没有的右单边账。具体操作见 §1.12。
...
`MPT_CHANNEL_OTHERS`、临时银行对账单和「缺渠道账单」暂不在 3.0.14 处理，计划在 3.0.15 支持。
...
4. **查看统计**：状态框显示平账、不平、银行空 ID/零金额/空渠道、网关空 ID/无效/重复折叠/未使用候选等数量。
5. **导出**：点「导出文件」并选一个目录。软件按银行 `Channel` 拆分，每个渠道生成一个 Excel。

### 1.12.4 平账规则（🔴 涉及资金）

银行行和网关候选只有以下四项**全部相同**才平账：

1. 银行 `ReconciliationId` = 网关 `reconciliationId`，且 ID 非空。
2. 银行 `Channel` = 网关 `channel`。
3. 银行 `abs(Credit Amount - Debit Amount)` = 网关 `amount`。
4. 银行 `Currency` = 网关 `currency`。

ID、渠道、币种只去掉首尾空格，仍区分大小写；金额用精确十进制比较，`1`、`1.0`、`1.00` 视为一样。不做子串、模糊、日期窗口或其他兜底。

同时满足四项时，临时 MPT 候选优先于现有网关链接表；每条网关候选只能被一条银行行消费。对账 ID 相同，但渠道、金额或币种不同时，不平账，也不会消费该网关候选。
...
- MPT 文件按 UTF-8、`|$|`分隔符、33 字段、文件名/首行/明细账期、金额和声明行数强校验；一份文件任一行错误，该文件整批不入库，其他已成功文件不回滚。
...
两类数据分开显示数据日期范围和更新日期，也可选定其中一类按账单日期闭区间删除；删入金不会误删出金，反之亦然。临时 MPT 数据会跨应用重启保留，直到用户手工删除。

### 1.12.7 导出文件

文件名：`资金对账不平_{渠道名}_XXXX年XX月XX日.xlsx`。固定 5 个 sheet：
...
某个渠道全部平账时仍会生成文件：「不平结果」和「渠道账单」只有表头，平账数据写在「平账结果」。数据源在运行后发生变化、或运行结果文件丢失/损坏时，「导出文件」会被禁用，需重新运行。

> 🔴 首次上线请用真实小样本人工核对：四字段精确匹配、临时来源优先、重复折叠、CREDIT/DEBIT 金额与姓名/卡号方向、右单边去向、行数守恒和按渠道不串数据。
