...
> 版本说明：v3.1.12 已于 2026-08-20 正式发布，并成为项目的 GitHub latest stable Release；Setup、portable、blockmap 和 `latest.yml` 四项资产可从 [GitHub Releases](https://github.com/MatthewPZhong/bank-bill-excel-tool/releases/tag/v3.1.12) 获取。发布负责人已确认验收完成并授权合并。本版包含网银源文件确认、启动治理、VCC CNY/CNH 新导入口径以及存档中心布局、迁移和后台维护。Windows `production/latest` 在线升级 canary 仍需另行人工验证，正式发布不等于该 canary 已通过。
...
> **v3.1.12 网银源文件、启动、VCC 币种与存档中心**：网银账单在真正生成前只核对一次用户确认期间的源文件身份，文件变化会要求重新选择；软件完成数据库与必要恢复后才显示主窗口，并移除每次启动的全库 `ANALYZE`。VCC 财务 OP 的新导入接受标准大写 CNH，并在计算、幂等和输出中统一为 CNY，历史数据不改。存档设置把【变更】放到“存档位置”右侧、完整路径移到下一行，变更位置会完整迁移已有存档；每次启动首次进入存档中心后再后台分页维护。本版已于 2026-08-20 正式发布为 latest stable Release，四项 Windows 资产已公开。
...
v3.1.12 起，存档设置第一行左侧显示“存档位置”、右侧显示【变更】，第二行显示完整只读路径。路径过长时会自动换行，并可选择复制，不再用省略号截断。
...
每次启动后第一次进入存档中心时，软件会先显示批次列表，再在后台分页执行到期清理、目录化修复、已证明归属的孤儿保洁和历史文件健康筛查。维护期间仍可查看列表；完成后列表自动刷新。若当前选中的批次已按保留期删除，页面会清空该选择并提示；维护失败不会伪装成完成，离开后再次进入可重试。如果本次启动始终不进入存档中心，就不会运行这些日常维护。
...
1. 在存档设置第一行右侧点击【变更】，选择目标文件夹；当前完整路径显示在下一行，选中的文件夹本身就是新存储根目录。
