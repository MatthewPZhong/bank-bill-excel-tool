...
> v3.2.9：业务 OP 错误报告自动保存；存档中心支持按模块配置期限及有确认保护的永久删除；新增定时深色模式、工具箱按行拆分、VCC 多 Sheet 导入和待确认表导出。VCC 存储升级到 v3，升级后请勿用旧版本写入数据库；回退须使用升级前完整备份。

> v3.2.8 历史版本说明：修复 Windows 更新后因业务 OP 目录同步失败无法打开软件的问题。已完成新版升级的输入和历史结果无需重新导入；尚未完成的首次升级会继续原有检查和升级步骤。原件、历史存档以及业务 OP 的计算和输出口径保持不变。

> v3.2.7 历史版本说明：启用业务 OP 区间核对，支持批量导入、所需输入检查、历史结果版本及全量/差异导出。运行、导出与删除弹窗按 VCC 样式缩小，导出按钮保持稳定并在成功后提醒；合计流水按入账减出账计算，核对差额和结论保持一致。首次从旧模块升级与新版数据的公式升级适用不同的导入说明，详见下文。

> v3.2.6 历史版本说明：银行对账单赋值自身支持“等于 / 包含”；提取大账号顺序时，明确识别出未维护账号会提示来源，确认后结束本次整批导入。首次使用请用脱敏样本核对匹配和账号归属。
...
> 业务 OP 错误报告（v3.2.9）：本次导入失败且有通过归属校验的可用结构化诊断时，软件自动将 XLSX 报告保存到 `Documents/网银账单生成小助手/error-reports/{日期}/`，无需另存。页面会分别显示业务结果与报告保存状态；保存失败会保留原错误并提示原因，不能把报告失败理解为原业务已成功。应用被强制结束时，尚未开始的自动报告不保证在重启后补导。

> 业务 OP 新版启用：v3.2.7 已按明确授权配置开启。启动检查和升级完成后，可一次导入多个文件、选择起止日期核对、查看缺失输入、保留历史版本并导出全量或差异结果。删除前会展示关联结果和原件影响，选择保留结果或同时删除关联结果。
>
> 如启动提示业务 OP 资源预算不足，提示中会显示所需的 1024 MiB 后台预算、当前可用预算和预算上限。请关闭不需要的程序后重新启动小助手，让应用重新读取资源预算。这里显示的是后台任务预算，不是电脑的总内存。尚未完成的任务和恢复记录会保留；升级不会因等待无法满足的资源而一直停住。运行期间若提示等待后台资源超时，请在其他任务结束后重试。补充有效的验收记录不会要求重新升级或清理已导入的新版数据。
>
> 新生成的业务 OP 结果中，【合计流水发生额】按【流水入账合计 − 流水出账合计】计算，下一列改为【终止期末－合计流水】。倒推余额、核对差额、金额结论、原因及差异筛选口径不变，冲正金额保留原正负号。**已经导入新版区间模块的数据无需重导**，重新运行相同区间会生成新口径的结果版本，再次运行则复用该版本。历史结果保留原金额、原公式说明和原表头，导出历史版本不会将旧值套用新公式。
>
> 新版页面与 VCC 财务 OP 布局一致：上排左侧为【导入文件／开始运行】，右侧为【导出校验结果表】；下排左侧显示状态，右侧为【数据管理】。导出 OP 或流水的原表、校验表，请进入【数据管理 → 导出】，打开“导出数据”窗口。数据管理左侧切换【结果表／校验表／校验原表】，右侧按操作月份查看列表。
>
> 初始状态显示“欢迎使用小助手”。v3.2.9 起，业务 OP 导入和其他已提交任务不再提供取消入口；运行期间等待任务实际结束后再继续。提交前的文件选择、日期选择、删除确认仍可返回或取消。结果导出的操作月份和结果表下拉框横排显示。数据管理底部为【删除／导出／返回】：先点【删除】进入选取，勾选后再次点【删除】查看影响确认；【取消选取】退出选择。超过一页时使用左下方页码选择，换页会清空勾选；列表数据已变化时会回到第一页重新选取。结果表行内的【导出原表】以文字按钮显示。
>
> 差异文件只包含差异数据页，保留 19 列，不附“核对说明”工作表；没有差异时只有表头。需要完整说明时，在数据管理中点击同一版本的【导出原表】。导出期间管理页不显示进度框，v3.2.9 起不再提供已提交任务的取消按钮。“导出数据”窗口的账期和目标横排显示；删除选取阶段的注释已去掉，具体影响在确认窗口查看。

> 导出结果原表期间，数据管理底部【删除／导出／返回】保持原位置；v3.2.9 起左侧不再显示已提交任务的【取消】按钮。结果原表、OP 校验原表及流水校验原表导出成功后弹出提醒；取消或失败时不显示成功提醒。导出数据窗口使用【返回】，其右侧对齐目标下拉框；导出校验结果表的【关闭】右侧对齐结果表选择框。开始运行窗口的【检查所需数据】左侧对齐起始日期框，【关闭】右侧对齐终止日期框；预检成功后展示所需表格，不再显示额外的“所需输入齐全”说明。开始运行、导出数据、结果导出、日期选择、删除确认和成功提醒统一采用 VCC 的紧凑字体与小按钮，缩小标题、表单和底部留白；两侧边距一致。删除影响明细与运行输入清单较长时在内容区滚动查看，底部按钮保持可见。
>
> 【开始运行】中点击起始或终止日期框，会打开最近一次成功导入的 OP 数据所在月份；如果刚补导了较早月份，就会先显示该月份。只有当前有 OP 数据的日期可选，灰色日期不能选择；左右箭头可切到前后有数据的月份。没有 OP 数据时会提示先导入。日期框支持鼠标，也可聚焦后按 Enter、空格或向下键打开，再用 Tab 和 Enter 选择。选好起止日期后，点击左下角【检查所需数据】，通过后再确认运行；检查仍会核对区间所需流水是否齐全。运行窗口的两个日期框紧凑并排，标题与下方分割线左对齐。
>
> “导出数据”窗口移除当前可用版本说明，账期与导出目标使用紧凑横排布局。该窗口和“导出校验结果表”的提交按钮均显示【导出】，点击后选择保存位置。“确认删除影响”标题与下方分割线左对齐。
>
> **首次从旧业务 OP 模块升级**会清理旧明细、旧结果和专属月度侧库，需从原件重新导入业务数据；历史存档、原件、用户锁、其他模块与外部文件保留。这项首次升级清理不适用于已在新版区间模块完成导入后单独升级计算公式的情况。内存或磁盘不足、旧任务未结束时会保留保护；中途失败后继续同一次升级。激活后降回旧程序不会恢复旧业务数据，须按明确备份和人工流程处理；不要手工删除恢复记录来跳过保护。v3.2.7 的冻结验收记录保持原状；本补丁的 Windows 启动与功能验证另行记录。2200 万输入/最大结果并集、真实资金与 Excel/WPS 人工验收仍需另行完成。
...
1. 点「拆分表格」那行的 `[导入文件]`，**单选一个** Excel / CSV → 软件读出这张表的表头、以及每一列里有哪些不重复的值 → 弹出「**选择拆分字段**」小窗：
   ...
   - **字段模式至少要勾一个值**才能完成——一个值都不勾不让导出（否则会筛出 0 行、导出一张空表）。某一列要是整列都空着、没有任何值，对应的值清单就是空的、选不了。

2. 在字段模式下，不勾选「需要拆分成多个文件」时，选完字段和值直接点 `[完成]`。软件会**只保留「该字段的值正好是你勾中的那些值」的行**，并弹出另存为对话框。
...
在“选择拆分字段”中勾选【按行拆分】，输入每份文件最多包含的有效数据行数 N（正整数）。表头不计数，多个 Sheet 的有效行按来源顺序连续计数，最后一份可以不足 N 行。行数默认空白；勾选后字段、值和原“需要拆分成多个文件”禁用，无需选择字段和值。

点【完成】后选择一次输出目录，文件按“原文件名_按行拆分_序号.xlsx”命名，每批最多 1000 份。若文件数超过上限，增大 N 后重试；本界面不显示常驻预计文件数量。取消勾选会恢复原字段和值，同一弹窗内保留行数草稿，关闭或重新导入后不保留。遇到同名文件仍需确认覆盖，所有结果沿用下文的格式保真、校验及整批发布保护。

若提示“后台资源配额不足”，拆分文件尚未开始生成，详情会列出所需、总额、占用和可用配额。若总配额低于所需值，先关闭暂不使用的程序，释放内存后重新启动小助手再试；这里的配额不是电脑的总内存。若提示“等待后台资源超时”，请等待其他后台任务结束后重试，持续出现时保留详情用于排查。
...
- 默认保留 60 天，可选择 30、60、90、180、365 天或永久。v3.2.9 起，13 个业务模块和工具箱共 14 项可分别设置期限或选择“跟随默认”；明确选择永久不会跟随默认值变化。下拉框变更立即自动保存，快速连续修改按顺序保存最终选择；失败回到最近一次成功值并提示。
- 新期限只影响之后创建的批次，历史批次、已预约任务和恢复任务保留原来的到期日，不会因为修改配置立即清理。到期当天仍保留；锁定、业务引用和恢复保护继续生效。
- 保存进行中，【返回】和右上角关闭会暂时禁用；保存完成后恢复。离开或销毁页面后，较早请求不会再改写已经关闭的页面。
- 永久删除先检查批次是否仍被任务、业务数据、锁定或恢复流程使用，检查通过后再显示确认框。确认后只删除该批次信息、存档中心保存的原始文件及能确认归属的受管副本；其他批次仍使用的共享内容会保留，外部导入原件、正常外部输出和用户另存副本不在清理范围内。
- 若提示“批次记录已删除，但文件清理尚未完成”，请在【待完成删除】中重试；这表示物理文件尚未全部清理，不能当作删除完成。重复确认、重启恢复和到期维护都不能绕过原有保护。
...
打开【设置 → 外观设置】，启用定时深色模式并填写开始/结束时间，默认关闭、默认时段 18:30–06:00。有效修改自动保存，支持跨午夜，开始时刻计入深色、结束时刻恢复浅色；相同或非法时刻会提示且不保存。关闭开关立即恢复浅色并保留已保存时段。

主题按本机当地时间判断，与操作系统深浅偏好无关；每分钟重新判断，唤醒或聚焦时立即校正。默认/纯色背景在深色时使用深色底色，返回浅色后恢复原颜色；自定义背景图片保留。保存失败会显示原因并保留原配置，连续输入时无需等待每个字符保存完毕。
...
| VCC通道明细 | 渠道订单号 | 导入预检中逐文件、逐 Sheet 填写 |
| VCC_移除归档Pending账单 | PendingBizId | 原表“主体” |
| 系统财务OP | 账期 × 主体快照 | 原表“主体” |

软件按完整正式表头、列数和列顺序识别原表，不依赖文件名或 Sheet 名。v3.2.9 支持一个工作簿内同类型或混合类型的多张业务 Sheet，包括隐藏 Sheet。预检逐页显示类型、表头位置和原因；空白页跳过，非空说明页须明确排除，缺列、错列、旧模板或一页多块业务表不能通过排除绕过。充值/费用的 `BillDate`、通道的“账单日期”和 Pending 的“平账账期”必须与所选月份一致；系统财务 OP 按“账单日期”只读取所选月份。
...
通道主体按“文件 + Sheet”填写；同一文件的不同通道页可以指定不同主体，提交前每页都须明确，软件不会按文件名猜测。系统财务 OP 主体直接取正式模板的“主体”列，一个文件可以包含多个主体；目标月份的“业务部门”必须为 VCC。每个主体的有效快照必须在同一 Sheet 内覆盖 AUD、CAD、CNY、EUR、GBP、HKD、JPY、SGD、USD 九币种且每个币种只有一行，不能跨页拼凑。v3.1.12 起，新导入允许用标准大写 CNH 表示人民币，但进入业务快照后统一作为 CNY；同一主体同月同时提供 CNY 和 CNH 会形成重复人民币币种并拒绝该主体，其他完整主体继续。比较余额固定取“财务余额”，不取 OP期末余额或财务主体余额。数值或公式缓存数值优先使用 Excel 原始数值，显示值只用于文本兼容和审计；因此 PPHK / JPY 样例必须保留为 `135886024.59`，不得变为 `135886024.6`。

导入提交后，软件会先为本批次全部已识别原表类型建立导入记录。导入进行中可取消；即使在第一类原表读取前取消，或前一类异常导致后续类型尚未开始，记录也会明确显示失败且不参与计算。四类明细按行处理异常；系统财务OP按完整主体九币种快照处理异常。正常行/完整主体快照继续进入有效数据集，被过滤的数据及原因保留审计。损坏文件、模板结构错误或已归档账期会在提交前拒绝；运行按原表类型分组，组级失败回滚当前组。取消或 Worker 异常时停止尚未提交和未执行的组，已提交的其他组保留并明确显示，不能把已提交数据当成已回滚。
...
有活动 VCC 任务、未处理导入异常、归档结果与余额/数据集证据不一致，或预览后状态发生变化时，解归档会在提交前再次阻断，不修改任何余额、结果或数据集状态。解归档成功后，基础结果、原有人工调整、导入事实和审计都保留，该月结果恢复为“待确认”，原表和校验表的数据集状态恢复为“未处理”，并立即从已归档导出月份中移除。用户可对尚未调整过的结果行与币种新增调整后重新归档，或删除全部未归档结果后重新运行。已保存调整的一次性约束不会因解归档重置。

### 1.14.7 结果导出和数据管理

v3.2.9 起，结果确认页左下角可【导出待确认表】：使用原生另存为选择位置和名称，默认 `YYYY-MM_VCC财务OP校验待确认表.xlsx`，一次包含页面全部主体。首张 Sheet 按确认页完整顺序包含明细、调整和汇总，后续 Sheet 给出差异关联的原表行；该操作不等于确认归档。数据管理中的未归档结果显示“待确认”。

主页正式校验结果表仍只允许导出已确认归档的结果。点击主页“导出校验结果表”后，先在“请选择要导出的月份”中选择年份和月份；列表来自全部状态一致的已归档结果，默认最新月，也可选择更早归档月。解归档的月份不再出现在该列表中；没有已归档结果时，主页导出按钮禁用并显示空状态。每个主体生成一个 `YYYY-MM_主体_VCC财务OP校验结果表.xlsx`，文件只包含：
...
v3.1.11 不再提供【优化存储】按钮或普通用户迁移通道。v3.1.11 的全新安装和旧空 v1 静默升级终态为 v2；v3.2.9 会继续完成 v3 迁移及校验后才开放业务写入，已有 v2 保留业务数据升级。非空 v1 不会被自动迁移或删除，仍需明确授权的维护流程。初始化或后续启动可能创建一条空的模块状态记录，这不属于校验原表、校验表、结果表或历史审计数据。受控维护仍沿用 WAL 收敛、完整性与空间检查、copy-on-write 候选、逐表守恒、原子切换和 journal 恢复，但只能用于明确授权的一次性维护，不能理解为软件会替所有用户清空历史数据。
