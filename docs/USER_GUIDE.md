...
12. 平盘对账数据处理（`v3.0.24` 新增；`v3.1.0` 开放第一期“平盘资金性质校验”；`v3.1.4` 支持调拨/测试付款异常行自动过滤和导出）
...
> **v3.1.4 平盘异常行过滤**：中台调拨单和测试付款单中的两类明确无效证据行会自动过滤，正常行继续落库；导入完成可立即导出异常报告，之后也可在存档中心下载。资金校验结果确认页始终显示“过滤数据导出”，无过滤数据时按钮置灰。付款成功调拨缺金额/币种仍会整文件拒绝。
