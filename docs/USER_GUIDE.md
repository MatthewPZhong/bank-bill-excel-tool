...
4. 银行对账单处理（`v2.0.0-beta.3` 新增 / `v2.1.13` 新增「自带写死场景」+「复制场景」、功能类别改名、主面板镜像）
...
> **v2.1.4 起**：左上角模块切换菜单默认只展示 3 个高频模块（网银账单生成 / 银行对账单处理 / 对账单 ReconID 修复，v2.1.5 起 ReconID 前后加空格），其他模块需通过主页面右下角 🔄「小助手功能收纳」按钮启用。详见 §1.8。
