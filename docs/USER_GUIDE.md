...
> 版本说明：本指南随 v3.1.12 应用构建并在打 tag 前定稿；当前公开稳定版、下载资产和发布时间以项目的 GitHub Releases 页面为准。v3.1.12 只纠正 v3.1.11 安装包内过时的发布状态文字，不改变业务功能、数据或输出。
...
> **v3.1.12 安装包内用户指南纠正**：v3.1.11 已发布资产中的本指南仍写着“未发布候选”，仓库在发布后补写的正式状态不会进入既有 Setup 或 portable。按照不可变发布规则，旧资产不替换；本版通过更高补丁版本交付已在 tag 前定稿的指南。除版本和随包文档外，业务行为与 v3.1.11 相同。
