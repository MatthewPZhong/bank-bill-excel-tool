...
12. 平盘对账数据处理（`v3.0.24` 新增；本版仅提供前端入口，业务功能尚未开放）

> **v3.0.25 设置与存档调整**：设置页底部统一为“确认”；存档默认保留 60 天，可选 30、60、90、180、365 天或永久。网银模板“不存档”已移除，旧排除配置升级后全部失效，后续网银账单和月度余额均按正常规则存档。

> **v3.0.24 平盘前端与 Payment 多账号**：「平盘对账数据处理」默认收纳在闲置功能中，启用后可查看三个功能选项和五个占位入口；本版不会处理或导出数据。Payment 线下调拨订单回填的“大账号”可使用中文顿号 `、` 配置多个账号，三轮匹配均要求银行账号与订单账号相同。

> **v3.0.22 存档中心**：点击主页面左下角 ⚙️，再选择左侧“存档中心”，可以按日期、模块和批次查看 11 个模块本次成功处理时使用的输入文件及第一次正常结果。存档失败不会让原业务失败；相同内容只占一份磁盘空间。详见 §1.11.9。
...
“开始运行 / 对账表管理 / 链接表管理 / 对账配置管理 / 导出文件”五个入口目前只显示后续版本开放提示，不会导入、运行、保存配置或生成文件。关闭软件后，功能下拉恢复为第一项。
...
`v3.0.8` 在**主界面左下角**新增了一个🧰按钮。`v3.0.18` 起左下角按钮顺序为 🎨 / 📕 / ⚙️ / 🔄 / 🧰。点🧰打开「**工具箱**」——一个**和对账流程完全无关**的轻量小工具，专门干两件日常零碎活：把几个表头一样的表拼成一张、从一张表里按某一列的某些值挑出需要的行。**它跟对账、模板、币种换算都没关系，就是纯粹地搬运表格里的行，原样拷贝、不改任何内容。**
...
- 11 个主模块本次成功处理所使用的输入文件，以及该批次第一次生成的正常结果文件。
- 有“开始运行”按钮的模块只在运行成功后建批；只导入未运行、运行失败或取消不会建立运行批次。
- 同一批次只保存第一次成功结果。之后再次另存、重复导出、Pending/银行 BU 聚合导出或业务 OP 日期区间导出不会追加到旧批次。
...
4. 批次显示“不完整”或“失败”时，一般可点击重试。重试会再次读取当时的原文件路径；原文件已移动或删除时仍会失败，当前版本不能重新指定源文件。
...
- 存档复制在业务成功返回后后台执行。若存档目录不可写、磁盘空间不足、原文件丢失或排队期间文件被替换，原运行和正常结果仍然成功，软件会提示“业务已成功，但存档失败”。正常关闭软件或重启升级时，会先等已排队的存档完成；大文件较多时退出可能稍慢。
...
1. `不平结果`：只有缺网关的银行右单边行，数据来源为「导入银行对账单」，差错类型为「右单边账」。
...
某个渠道全部平账时仍会生成文件：「不平结果」和「渠道账单」只有表头，平账数据写在「平账结果」。数据源在运行后发生变化、或运行结果文件丢失/损坏时，「导出文件」会被禁用，需重新运行。旧 4-sheet 文件和本版 5/6-sheet 文件都可继续用于「对账单 ReconID 修复」的网关对账单流程；第 6 个重复审计页会先校验表头，但其中数据不会改变修复结果。
