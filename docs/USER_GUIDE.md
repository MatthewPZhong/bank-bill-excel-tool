...
> **v3.1.9 全局批次与存档中心发布候选**：实际处理、写入、导出和删除任务统一使用跨模块连续的 `YYYY-MM-DD-NNN` 批次号；失败和取消也保留号码，关联任务可跨重启查看。VCC财务OP校验和工具箱已真实接入存档，运行文件按年/月/日/批次目录展示，并支持存储位置迁移、修复、保留期清理和新的统计/列表/设置界面。VCC 大库信息改为后台读取，标准 v3.1.7 旧归档可以受控查看、导出和解归档；VCC 人民币代码统一为 CNY，异常明细/异常主体快照会被过滤而正常数据继续落库。当前仍是本地候选，未完成人工门禁见 §1.11.9。
...
- 当前为本地发布候选，尚未合并或正式发布。自动验证不能替代 Windows installer/portable 实际运行、目标生产库 legacy/trigger 只读检查、约 16 GB 冷热性能、约 700 万行多 Sheet 工具箱、跨盘符/UNC/网络路径和 copy/readonly/容量行为。
...
只有通道文件会显示公司主体输入框，必须填写真实主体，软件不会按文件名猜测。系统财务 OP 主体直接取正式模板的“主体”列，一个文件可以包含多个主体；目标月份的“业务部门”必须为 VCC。每个主体必须提供 AUD、CAD、CNY、EUR、GBP、HKD、JPY、SGD、USD 九币种且每个币种只有一行；人民币只使用 CNY，不再把 CNY 转成 CNH，CNH 会按异常币种过滤。比较余额固定取“财务余额”，不取 OP期末余额或财务主体余额。数值或公式缓存数值优先使用 Excel 原始数值，显示值只用于文本兼容和审计；因此 PPHK / JPY 样例必须保留为 `135886024.59`，不得变为 `135886024.6`。
...
导入完成后不会再弹出导入记录框。主页面状态框会显示本次账期、实际新增、幂等跳过和异常过滤数量；只要存在过滤或失败记录，就会以警告状态提示进入「数据管理 → 校验原表」。完整记录和逐行明细仍保存在数据管理中。
...
成功（含跳过/异常过滤）的记录只让已落库正常数据参与计算，过滤数据不参与计算，也不需要额外解除运行门禁。整条失败记录不会参与计算；确认失败数据确实不应纳入时，填写处理说明并勾选“保留当前有效数据集，本次失败导入不参与计算”，才可标记已处理。这个动作不会修复或导入失败行；需要采用新数据时，应先查明原因后重新导入正确文件。
