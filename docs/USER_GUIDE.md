...
12. 平盘对账数据处理（`v3.0.24` 新增；`v3.1.0` 开放第一期“平盘资金性质校验”）
...
> **v3.0.22 存档中心**：点击主页面左下角 ⚙️，再选择左侧“存档中心”，可以按日期、模块和批次查看 12 个模块成功处理时使用的输入文件和结果文件。通常同一业务批次只保存第一次正常结果；平盘模块的成功输入、每次结果导出和结果回导各自形成独立即时批次。存档失败不会让原业务失败；相同内容只占一份磁盘空间。详见 §1.11.9。
...
旧草稿失效和新草稿写入是一个整体操作；新结果生成失败时，旧草稿仍可继续使用。若软件检测到平盘侧库关键数据损坏，会直接停止运行并提示重新导入，不会把损坏数据当成普通未命中。
...
- 日期同日优先、前后 1 天容差、大账号、币种、方向、候选顺序、严格 1:1 和同值消费规则都不变。
...
- **勾上（默认）**：用上面 ① 整理出来的**「调拨对账单」**，去和银行对账单里资金性质是「调入 / 调出」（`FundTransfer-in` / `FundTransfer-out`）的那些行对，对上了就把调拨对账单里的流水号回填进银行行的「对账ID」（并标黄）。对账依据 = **日期（先找同一天，找不到再放宽到前后各 1 天）+ 大账号 + 含手续费金额 + 币种**。
- **取消勾选**：用「网关对账单」作为对手方，日期、账号、含手续费金额、币种和 1:1 规则与默认来源保持一致。
...
- 12 个主模块本次成功处理所使用的输入文件，以及符合各模块存档规则的正常结果文件。
