...
7. 业务OP数据核对（`v2.1.3` 新增）
8. 收单单据币种校验（`v2.1.6` 新增）
...
- 单 xlsx 单 sheet **不限行数**（v2.1.6 fix2 起 reader 改为 yauzl + sax 流式，支持 POI 流式导出 30w+ 行 inlineStr + ZIP data descriptor 格式）；按月组织，多个 xlsx 视为一类
...
| 开始运行 | 点击后弹「选择对账月份」弹窗 → 选月份 → SQL JOIN 对账 + **同步产出**单文件多 sheet 差异 xlsx |
