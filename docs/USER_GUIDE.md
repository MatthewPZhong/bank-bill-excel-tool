...
4. 状态栏显示统计：`{date} BU={X} 对账完成：测算金额差异 N 笔 / T-1 有 T-2 无 X 笔 / T-2 有 T-1 无 Y 笔 / 多 OP 账户 Z 个 / T-2 异常 W 个`
   ...
   - **T-2 异常 W 个**（v2.1.3 round 1 I3 新增） = 该 BU 该日 **T-2** 业务OP 表中期末余额非数值（如 `#N/A`、空字符串、含字母）的账户号去重计数。这些账户号在算法 4.2.a 步骤会被跳过（无法计算 T-1 期末），后续会走"T-1 有 T-2 无"分支。> 0 时建议核查 T-2 源文件期末余额列；DevTools console 有 `[biz-op-recon] T-2 NaN end_balance: 账户=...` 的 warn 日志辅助 debug。详见 PRD §3.5.5。
