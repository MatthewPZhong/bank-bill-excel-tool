...
> 版本说明：v3.1.13 为当前迭代版本，新增工具箱状态框与运行期间关闭保护，把多文件拆分的同名覆盖确认按钮改为【返回 / 覆盖全部】，调整存档中心前端，修复平盘对账数据处理和对账单修复成功初始化时没有保持“欢迎使用小助手”的问题，并移除所有状态框的星星 SVG；尚未标记为正式发布。当前 GitHub latest stable Release 仍为 [v3.1.12](https://github.com/MatthewPZhong/bank-bill-excel-tool/releases/tag/v3.1.12)。
