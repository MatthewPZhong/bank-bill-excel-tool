...
> 业务 OP 新版启用：已按明确授权配置开启。启动检查和升级完成后，可一次导入多个文件、选择起止日期核对、查看缺失输入、保留历史版本并导出全量或差异结果。删除前会展示关联结果和原件影响，选择保留结果或同时删除关联结果。应用版本号暂仍为 v3.2.6。
>
> 新生成的业务 OP 结果中，【合计流水发生额】按【流水入账合计 − 流水出账合计】计算，下一列改为【终止期末－合计流水】。核对差额、金额结论和差异筛选口径不变，冲正金额保留原正负号。已导入数据无需重导，重新运行相同区间会生成新口径的结果版本，再次运行则复用该版本。历史结果保留原金额和原表头，导出历史版本不会将旧值套用新公式。
>
...
>
> 首次启用会清理旧业务 OP 明细、旧结果和专属月度侧库；历史存档、原件、用户锁、其他模块与外部文件保留。新版业务数据需从原件重新导入。内存或磁盘不足、旧任务未结束时会保留保护；中途失败后继续同一次升级。激活后降回旧程序不会恢复旧业务数据，须按明确备份和人工流程处理；不要手工删除恢复记录来跳过保护。Windows 目标耐久、目标规模及人工资金、Excel/WPS 验收仍需另行完成。
