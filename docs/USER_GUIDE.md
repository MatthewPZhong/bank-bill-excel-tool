...
> 导出结果原表期间，数据管理底部【删除／导出／返回】保持原位置，左侧显示与【删除】同尺寸的【取消】。结果原表、OP 校验原表及流水校验原表导出成功后弹出提醒；取消或失败时不显示成功提醒。导出数据窗口使用【返回】，其右侧对齐目标下拉框；导出校验结果表的【关闭】右侧对齐结果表选择框。开始运行窗口的【检查所需数据】左侧对齐起始日期框，【关闭】右侧对齐终止日期框；预检成功后展示所需表格，不再显示额外的“所需输入齐全”说明。开始运行和导出数据弹窗同时收紧右边界，保留字段宽度，左右留白一致。
>
> 【开始运行】中点击起始或终止日期框，会打开最近一次成功导入的 OP 数据所在月份；如果刚补导了较早月份，就会先显示该月份。只有当前有 OP 数据的日期可选，灰色日期不能选择；左右箭头可切到前后有数据的月份。没有 OP 数据时会提示先导入。日期框支持鼠标，也可聚焦后按 Enter、空格或向下键打开，再用 Tab 和 Enter 选择。选好起止日期后，点击左下角【检查所需数据】，通过后再确认运行；检查仍会核对区间所需流水是否齐全。运行窗口日期框缩为原宽的三分之二，标题与下方分割线左对齐。
>
> “导出数据”窗口移除当前可用版本说明，日期框进一步缩到调整前的五分之四，导出目标下拉框缩到一半。该窗口和“导出校验结果表”的提交按钮均显示【导出】，点击后选择保存位置。“确认删除影响”标题与下方分割线左对齐。
>
