...
4. **查看统计**：状态框显示平账、不平、银行空 ID/零金额/空渠道、网关空 ID/无效/重复折叠/未使用候选等数量。重复折叠不只显示数量，导出时还可查看双方原始记录。
...
> 🔴 首次上线请用真实小样本人工核对：四字段精确匹配、临时来源优先、重复折叠、保留/被折叠双方原始 JSON 可还原、CREDIT/DEBIT 金额与姓名/卡号方向、右单边去向、重复专属渠道仍导出、行数守恒和按渠道不串数据。
