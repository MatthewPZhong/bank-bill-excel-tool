...
- **`npm run release-check` 全绿（PASS/FAIL 源）**：
  - 单测 **unit 1735 / 1735 PASS**（含 PR#62 review 修复新增 4 用例）
  - 集成 **integration 952 / 952 PASS**
  - smoke 全过（0 regression）
- 新增单测：R1（`r1-recon-id-match.test.js`）/ R4（`r4-fund-nature-check.test.js`，含二次改值链 + no-op 守卫 + priority 顺序）/ R5场景2（`r5-fund-transfer-backfill.test.js`，含同日/±1day/绝对值归分/1v1/覆盖 warn/tie-break）/ R5场景3（`r5-platform-inbound-cleanup.test.js`）/ 日期工具 / 编排器契约 / `readLinkedTableRows` 还原 / 剔除 writer / migration 幂等 / FundType 枚举与改拼。
...
> commit `fa3d625` 后针对自动 review findings 的修复；回归 unit **1735/1735** + integration 952/952 + smoke 全绿。

- **F1 🔴P1**（`r5-fund-transfer-backfill.js`）：空 `reconciliationid` 网关行不再占用银行候选——gwPool 增加非空 reconid 过滤，空值不进池、不消费 `usedBankRowId`。+test。
- **F2 P2**（`r5-fund-transfer-backfill.js`）：Phase2 多候选按 `|Δday|` 升序稳定排序后再取，`dateToleranceDays>1` 时优先近日。+test（tolerance=2）。
- **F4 🔴P1**（`reconciliation-orchestrator.js`）：`buildOutputRows` 分区改为 `isHit = modCols || r2Hit`——R2 first-match-wins 锁定但未改值的命中行保留进 `modifiedRows`（带 R2 命中元数据 + 空 `_modifiedColumns` Set），不再被误判为 unmatched 而从主输出/命中场景行报表消失。+test。
- **F3 P2**（`r4-fund-nature-check.js`）：文档对齐真实行为——R4 不做运行时 FundType 枚举校验（`setFundType` 来自受控 seed config、builtin-fixed config 不可经 UI 编辑、枚举已含全部目标值）；去除「枚举外值 warn」声称。

## 📝 自查修复（team-lead self-review）

> 编排器 / 导出衔接精读后再修 2 处 Important（回归 unit 1735/1735 + integration 952/952 + smoke 全绿）：

- **A 🟠**（`src/main.js` 导出端）：N5「命中场景行」报表只放 R2 命中行——`writeScenarioHitRows` 改传 `modifiedRows.filter(r => r._hitScenarioId != null)`；R4/R5-only 改写行（无 `_hitScenarioId`）不再以空白「命中场景」列污染审计报表（它们进主输出 sheet1 标黄即可）。
- **B 🟠**（`reconciliation-orchestrator.js` stats）：顶层 stats 用最终分区计数覆盖 R2 作用域的 `r2.stats`（`totalRows` / `hitRowCount` / `unmatchedRowCount` / `warningCount`），使状态框「已处理 N 行命中」（renderer `pr.hitRowCount`）反映全部 5 轮（原少计 R4/R5 改写行、多计 unmatched）。
- 自查未发现 Critical；seed config↔引擎↔编排器字段逐字一致已核（funcCategory / subCategory / gwTradeType / setFundType / directions / excludeFundType）。Minor（R4 同 reconid 多网关行叠加、网关空时静默 no-op）记入下版待测。
