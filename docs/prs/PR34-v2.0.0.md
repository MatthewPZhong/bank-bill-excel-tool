...
- **URL**：https://github.com/MatthewPZhong/bank-bill-excel-tool/pull/34
- **分支**：`v2.0.0-beta.4` → `main`
- **版本**：`2.0.0`（GA 正式版，本 PR 一次性 bump `2.0.0-beta.3` → `2.0.0`）
- **依赖**：v2.0.0-beta 系列全部已 merge（PR #29/#30/#31/#32a/#33）
- **commits**：`9389a7d`（feat F1+F2）+ `42f5b3e`（增量 #3 #4）+ `9dff45f`（草稿）+ `9afed62`（round 1）+ `d57324b`（round 2）+ `2797e59`（round 3）+ `ec1a808`（round 4）+ `9952255`（self-review）
- **状态**：MERGED 2026-04-30（merge commit `9cebc99`）
- **integrated**：true（已回写 PRD-v2.0.0-beta.3 §十六 实施记录阶段 9）
