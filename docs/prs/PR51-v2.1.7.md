---
pr_number: 51
pr_url: https://github.com/MatthewPZhong/bank-bill-excel-tool/pull/51
version: v2.1.7
branch: v2.1.7
target_branch: main
status: open
integrated: true  # PRD §二十三 实施记录已含完整 commit 清单（防 archive 重复，按 memory workflow_pr_integrate_prd）
draft_by: PM (T14-F)
draft_at: 2026-05-21
opened_at: 2026-05-21
opened_by: team-lead (T14-K)
total_commits: 49
total_rounds: 7
---
...
v2.1.7 共 50 commit，7 轮迭代收敛，覆盖 6 项主功能 + 多轮用户反馈修复：
...
| 节点 | 影响范围 | 回归覆盖 |
|---|---|---|
| **F7-A1 PRAGMA** | 全局 SQLite | smoke 22 suite + DB 备份恢复演练（含 *.sqlite-wal + *.sqlite-shm） |
| **R3 状态框「：」换行** | 全模块状态栏文案 + CSS pre-wrap | smoke 22 suite + 6+ 模块状态栏手测 |
| **B5 wiring 加固** | 3 处直写 statusBox → 统一走 `updateStatusBox` | smoke 22 suite + acquiring + bankStatement + reconIdFix 手测 |
...
- 50 commit 完整表（按时间逆序 + commit hash + 对应 PRD/spec §引用）：详见 PRD §23.1
...
用户已手测全部通过；GitHub Actions smoke-test SUCCESS（PR 多轮 push 后稳定通过）：

- [x] `npm run smoke`（**22 suite 全过** — 含 scenario-engines 43 + scenario-dispatcher 21 + exceljs-writer 7 + recon-id-fix-engine 45 + recon-id-fix-engine-gateway 13 + recon-id-fix-io 13 + recon-id-fix-ipc-handlers 21 + recon-id-fix-end-to-end 6 + scenario-end-to-end 23 + bank-bu-recon 41 + biz-op-recon 154 + acquiring-bill-currency 172 + acquiring-bill-currency-progress 32 + acquiring-bill-currency-pragma 27 + render-status-box 25 + scenario-c1-defaults 21 + 其它）
- [x] `npm run preview`（主 UI 截图最新）
- [x] `npm run preview:account`（账号映射截图最新）
- [x] F1 C1 dialog AND/OR 切换：新建场景默认 AND radio 选中 + 编辑老场景 OR radio 选中（用户手测通过 round 5）
- [x] F2 C3 真实银行账单 + 网关账单端到端（1v1 防幽灵核销，用户手测通过 + reviewer round 3 反馈后 round 9 双方向修齐 = filter 排除空 gw + lock 已等值）
- [x] F3 大账号 multi-mode ≥ 20 文件场景：弹窗滚动条出现 + 文件名 grid 3 列正常（B4 round 6 DevTools 真根因 `.ba-scroll-container grid-template-rows: 1fr` 修齐）
- [x] F4 C2 场景：billTypes=1 + reconFields=0 端到端 + 文案"银行对账单字段赋值"（用户手测通过）
- [x] F6 收单单据币种校验：6 阶段进度文案显示 + 完成 Notification 弹出（用户手测通过）
- [x] F7-A1 启动后：`tool-data.sqlite` 同目录出现 `*.sqlite-wal` + `*.sqlite-shm` 旁文件（用户手测通过）
- [x] F8 银行账单导出：sheet 1 已处理 + sheet 2 未命中场景行 + 行数互斥无遗漏（用户手测通过 round 3，全未命中 edge case 在 round 7+8 修齐 — saveDialog 触发条件 + return 'empty' 条件双修）
- [x] R3 全模块状态栏：中文「：」后自动换行（acquiring / bankStatement / reconIdFix / bankBuRecon / bizOpRecon，用户手测通过；C-1 self-review 阻塞 bizOpRecon ID specificity 已 commit 7234c32 修）
- [x] B5 wiring：3 处历史漏接已改走 `updateStatusBox`，tone 颜色 + 换行均生效（用户手测通过 + smoke caseB5_wiringAudit 7 断言防再漏）
- [ ] DB 备份恢复：手动备份 `tool-data.sqlite` + WAL 旁文件，恢复后启动 DB 正常 — **用户合并后实操验证**（CHANGELOG/USER_GUIDE §5.1 提示已添加）

## 关联文档

- PRD：`docs/iterations/v2.1.7/PRD-v2.1.7.md` v0.11（含 §二十三 实施记录 50 commit 全列表 + 6 round 历程）
...
- ☑ T14-C PRD v0.11 实施记录（§二十三 50 commit 全列表 + 6 round 历程 + integrated:true）
