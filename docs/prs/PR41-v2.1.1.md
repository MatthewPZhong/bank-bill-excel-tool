...
head: v2.1.1
created: 2026-05-12
merged_at: 2026-05-12T09:23:59Z
merge_commit: 4e398f3
integrated: true
---
...
| 分支 | `v2.1.1 → main` |
| 起草日期 | 2026-05-12 |
| commits | 15（PM 1 + T1-T6 主 task 8 + PR 草稿 1 + 用户反馈 self-review fix 2 + 草稿改名 1 + PR review round-1/2 fix 2）|
| 改动量 | src 8 文件（+ styles.css 加 ReconID General 风格）+ CLAUDE.md + USER_GUIDE 顶部版本 + smoke 4 文件 + docs 三件套 + preview 4 张 Clear + 3 张 General |
...
| afa8c73 | T2-2 引擎 | BillDate ±N — c4-recon-id-fix.js: billDateMatches(L, R, mode, days=1) 参数化 + 5 处调用传 cfg._billDateDays + runC4Scenario 解析 cfg.billDateRange + smoke business 45/45 / gateway 13/13 — 3 文件 / +147 / -11 |
...
| 78a3640 | PR 草稿 | 起 PR #41 草稿到 docs/prs/ |
| 3576de7 | self-review fix #1 | T3 tooltip 修复（用户反馈）— renderer-dialogs.js L6881 label 加 `width:auto; white-space:nowrap;` 让"订单修复ID取值 ⓘ"不换行；tooltip 文案改写：业务用"主边/从边/主从都修复"，网关用"取自网关 ReconID / 渠道 ReconID / 自取值（拼接'加上'输入框文本）"，去掉用户不懂的"suffix"和错误的"两侧都修复"描述 |
| f3eaa12 | self-review fix #2 | General 风格补齐（用户反馈）— styles.css 加 8 处 .recon-id-fix-* CSS（之前 PR #39 只给 Clear 写了样式，General 完全没有）；grid 60/220/140 三列严格对齐 + label 同 .select-label + select 同 .template-select 风格；重跑 3 张 General preview |
...
- `scripts/smoke/recon-id-fix-engine.js`：billDateMatches 加 4 个 days 单测 + 端到端 `runBillDateRangeWithNDays` 3 sub-case + PR #41 review fix `runBillDateRangeDefensiveFallback`（7 异常值 + 1 正常值）→ 业务子模式 45/45 PASS
...
recon-id-fix-engine: 45/45 PASS    ← +2 (BillDate ±N + defensive fallback) (vs 43)
