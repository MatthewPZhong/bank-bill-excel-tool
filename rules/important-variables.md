...
- 🔴 变更 review 要点（资金红线）：
  ...
  - **侧库结构完整性**：主库与 side DB 的实例身份、generation、事务 token 和父 token 历史链必须满足 checkpoint 契约；缺失、旧库回滚、同代或更高代次分叉必须阻断，禁止静默重建，只有父链证明主库 checkpoint 是侧库祖先时才允许安全追平；备份恢复必须同时包含主库和 `run-data/`。银行、来源、链接、运行结果与血缘 JSON 不仅要求语法合法，还必须包含契约字段，运行 scope/snapshot/summary 还须与明细一致；`{}` 等缺字段对象必须 fail closed。
